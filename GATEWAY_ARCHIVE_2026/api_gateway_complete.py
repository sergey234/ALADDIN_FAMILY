...
# Add root to sys.path to find local routers
if os.getcwd() not in sys.path:
    sys.path.insert(0, os.getcwd())

# Import routers for Stage 2
try:
    from gamification_router import router as gamification_router
    gamification_available = True
except ImportError as e:
    print(f"⚠️ Gamification router not found: {e}")
    gamification_available = False

try:
    from subscription_sync_router import router as subscription_router
    subscription_available = True
except ImportError as e:
    print(f"⚠️ Subscription router not found: {e}")
    subscription_available = False

try:
    from parental_control_sync_router import router as parental_control_router
    parental_control_available = True
except ImportError as e:
    print(f"⚠️ Parental Control router not found: {e}")
    parental_control_available = False

try:
    from crash_detection_router import router as crash_detection_router
    crash_detection_available = True
except ImportError as e:
    print(f"⚠️ Crash Detection router not found: {e}")
    crash_detection_available = False
...
@app.middleware("http")
async def log_requests_and_metrics(request, call_next):
    start_time = time.time()
...
@app.get("/metrics")
def get_metrics():
    SFM_STATUS.set(1 if SFM_ADAPTER_AVAILABLE and sfm_adapter and sfm_adapter.available else 0)
...
def get_api_metrics():
    return {
        ...
        "sfm_status": 1 if SFM_ADAPTER_AVAILABLE and sfm_adapter and sfm_adapter.available else 0,
...
class ComponentRequest(BaseModel):
    component_id: str = Field(..., min_length=1, max_length=50, regex=r"^[a-zA-Z0-9_-]+$")
    
    @validator("component_id")
    def validate_component_id(cls, v):
        if not v.replace("_", "").replace("-", "").isalnum():
            raise ValueError("Component ID must be alphanumeric with underscores and hyphens")
        return v

class ComponentConfigRequest(BaseModel):
    component_id: str = Field(..., min_length=1, max_length=50)
    config: dict = Field(..., description="Configuration parameters")
...
class ParentalSettingsRequest(BaseModel):
    ...
    restrictions: dict = Field(..., description="Parental control settings")

class NotificationSettingsRequest(BaseModel):
    enabled: bool = True
    types: List[str] = Field(default_factory=lambda: ["security", "updates"])
