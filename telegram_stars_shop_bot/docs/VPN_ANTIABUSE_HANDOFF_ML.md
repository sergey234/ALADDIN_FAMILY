# Handoff: AiMonkeyVPN — бот, анти-абуз, приоритет WireGuard + Happ

**Для кого:** другая ML/агент-система, которая продолжит работу без доступа к прошлому чату.  
**Дата:** 2026-05-17 (статус реализации ниже)  
...
Пользователь **платит в Telegram-боте** за VPN на срок (например 30 дней). Сервер выдаёт доступ двумя путями:

1. **Основной — WireGuard** (файл `.conf` или QR → приложение WireGuard).
2. **Запасной — подписка по ссылке** `https://aladdin-ai.ru/sub/<токен>` → рекомендуемое приложение **Happ** (iPhone и Android); альтернативы — Streisand, V2Box, v2rayNG.
...
|---------|--------|---------------------------|
| 1 | Основной | **WireGuard** |
| 2 | Запасной | **Happ** (iPhone и Android) |
| 3 | Запасной альт. | Streisand, V2Box (iOS), v2rayNG (Android) |
