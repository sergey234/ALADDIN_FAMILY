...
| `onb:terms:yes` / `no` | `common.py` | Принятие оферты |
| `onb:ch:check` | `common.py` | Проверка подписки после экрана канала |
...
| `nav:sell_stars`, `nav:sells:*`, `sel:*` | `hub.py` / `admin.py` | Продажа Stars пользователем |
...
| `adm:*` | `admin.py` | Действия по заказам, топапам, выкупу и т.п. |
...
2. **Оферта** — пока нет `terms_accepted_at`: экран с ссылками на политику/соглашение (`marketing.py` / клавиатура `onboarding_terms_kb`), кнопки Принять/Отклонить.
3. **Канал** — если включён гейт (`REQUIRED_CHANNEL_ID`): пока пользователь не в канале — экран с инвайтом и `onb:ch:check`.
4. **Капча** — пока не завершён онбординг: **`emoji_captcha.send_onboarding_captcha_photo`** пишет задание в `captcha_challenges`, пользователь жмёт `onb:c:{challenge_id}:{idx}`; успех → **`users_repo.complete_onboarding`** → приветствие хаба.
5. **Хаб** — `send_hub_welcome`: опционально второе фото (`START_PHOTO_FILE_ID_2`), текст **`ONBOARDING_SCREEN_2`** из `ui_copy.py`, клавиатура **`hub_menu_kb(settings)`**.
...
Другие FSM: **`SellStates`**, **`TopupStates`**, **`ApiKeyStates`** — для сценариев продажи Stars оператору, произвольного топапа, заявки на API-ключ.
...
| 6 | Возможная капча | Перед созданием заказа в **`order_submit`**: **`users_repo.checkout_captcha_valid`**; если нет — **`emoji_captcha.prompt_checkout_captcha`**, callback **`chk:c:*`** в **`common.py`** | TTL **`CHECKOUT_CAPTCHA_TTL_SECONDS`** |
