...
def _scrub_headers(headers: dict[str, Any]) -> None:
    for k in list(headers.keys()):
        lk = k.lower()
        if lk in ("x-api-key", "authorization", "x-payment-signature", "x-partner-signature"):
            headers[k] = "[Filtered]"
...
def init_sentry_bot(*, dsn: str, environment: str | None, traces_sample_rate: float) -> None:
    if not (dsn or "").strip():
        return
    import sentry_sdk
