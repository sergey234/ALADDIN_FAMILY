...
struct ElderlyInterfaceScreen: View {
    ...
    @State private var isSecurityEnabled: Bool = true
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showMedicationReminder) {
            MedicationReminderModal(isPresented: $showMedicationReminder, medications: $medications)
        }
        .sheet(isPresented: $showDoctorAppointments) {
            DoctorAppointmentsModal(isPresented: $showDoctorAppointments, appointments: $appointments)
        }
        .sheet(isPresented: $showBloodPressure) {
            BloodPressureModal(isPresented: $showBloodPressure, bloodPressure: $bloodPressure)
        }
        ...
        .alert("Позвонить детям?", isPresented: $showCallChildrenAlert) {
            Button("Александр (сын)") {
                callFamilyMember("Александр", "+7 (999) 123-45-67")
            }
            Button("Елена (невестка)") {
                callFamilyMember("Елена", "+7 (999) 234-56-78")
            }
            Button("Алексей (внук)") {
                callFamilyMember("Алексей", "+7 (999) 345-67-89")
            }
            Button("Отмена", role: .cancel) { }
        } message: {
            Text("Выберите кому позвонить:")
        }
    }
        ...
        VStack(spacing: Spacing.l) {
            ...
            VStack(spacing: Spacing.m) {
                ...
                healthCard(
                    icon: "💊",
                    title: "Лекарства",
                    subtitle: "Следующий приём: \(medications.first?.time ?? "18:00") (\(medications.first?.name ?? "Аспирин"))",
                    color: .blue,
                ...
                healthCard(
                    ...
                    subtitle: "Следующий: \(appointments.first?.date ?? "12.10") (\(appointments.first?.doctor ?? "Терапевт"))",
                    color: .green,
                ...
                healthCard(
                    icon: "🩺",
                    title: "Давление",
                    subtitle: "Последнее: \(bloodPressure.systolic)/\(bloodPressure.diastolic) (\(bloodPressure.date))",
                    color: .red,
        ...
        VStack(spacing: Spacing.l) {
            ...
            VStack(spacing: Spacing.m) {
                ...
                securityCard(
                    ...
                    subtitle: "Безопасен ли этот сайт?",
                    color: .blue,
                ...
                securityCard(
                    ...
                    subtitle: isSecurityEnabled ? "✅ Включена и защищает" : "❌ Отключена",
                ...
                securityCard(
                    icon: "📞",
                    title: "Опасные контакты",
                    subtitle: "\(blockedContactsCount) заблокировано ✅",
                    color: .orange,
    ...
    private func callFamilyMember(_ name: String, _ phone: String) {
        ...
        let generator = UINotificationFeedbackGenerator()
        generator.notificationOccurred(.success)
        print("Звонок \(name): \(phone)")
    }
    
    private func callEmergencyService(_ number: String) {
        // Логика звонка в экстренную службу
        let generator = UINotificationFeedbackGenerator()
        generator.notificationOccurred(.success)
        print("Звонок в службу: \(number)")
    }
}
...
struct ElderlySettingsModal: View {
    ...
    @State private var autoCallEnabled: Bool = false
