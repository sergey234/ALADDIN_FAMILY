...
struct TariffsScreen: View {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ALADDINNavigationBar(
                    ...
                    showBackButton: true,
                    onBack: {
                        // Умная навигация назад - как в ProfileScreen
                        if navigationManager.canGoBack {
                            print("🔙 Возврат через NavigationManager.goBack() из TariffsScreen")
                            navigationManager.goBack()
                        } else {
                            print("🔙 Возврат через dismiss() из TariffsScreen")
                            dismiss()
                        }
                    }
                )
    ...
    enum ThreatCategory: String, CaseIterable {
        ...
        var threats: [String] {
            ...
            case .iotThreats:
                return ["IoT device compromise", "Smart home infiltration", "Compromised cameras", "Smart speaker eavesdropping", "Home network breaches", "Smart device data leaks", "Voice command manipulation", "Weak IoT passwords", "Default credential abuse", "Physical device theft"]
            }
        }
    }
}
