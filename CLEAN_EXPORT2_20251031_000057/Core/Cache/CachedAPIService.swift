...
class CachedAPIService: ObservableObject {
    ...
    func getVPNStatus() async -> Result<VPNStatusResponse, NetworkError> {
        ...
        if isCachingEnabled, let cachedStatus: VPNStatusResponse = cacheManager.retrieve(VPNStatusResponse.self, forKey: cacheKey) {
        ...
        case .success(let status):
            // Сохраняем в кэш
            if isCachingEnabled {
                cacheManager.store(status, forKey: cacheKey, ttl: 30, priority: .high)
            }
    ...
    func getFamilyMembers() async -> Result<FamilyMembersResponse, NetworkError> {
        ...
        // Пытаемся получить из кэша
        if isCachingEnabled, let cachedMembers: FamilyMembersResponse = cacheManager.retrieve(FamilyMembersResponse.self, forKey: cacheKey) {
        ...
        case .success(let members):
            // Сохраняем в кэш
            if isCachingEnabled {
                cacheManager.store(members, forKey: cacheKey, ttl: 300, priority: .normal)
            }
    ...
    func getAnalytics(_ request: AnalyticsRequest) async -> Result<AnalyticsResponse, NetworkError> {
        ...
        // Пытаемся получить из кэша
        if isCachingEnabled, let cachedAnalytics: AnalyticsResponse = cacheManager.retrieve(AnalyticsResponse.self, forKey: cacheKey) {
        ...
        case .success(let analytics):
            // Сохраняем в кэш
            if isCachingEnabled {
                cacheManager.store(analytics, forKey: cacheKey, ttl: 600, priority: .normal)
            }
     ...
     */
    func getNotifications() async -> Result<NotificationsResponse, NetworkError> {
        let cacheKey = "notifications"
        
        // Пытаемся получить из кэша
        if isCachingEnabled, let cachedNotifications: NotificationsResponse = cacheManager.retrieve(NotificationsResponse.self, forKey: cacheKey) {
        ...
        // Получаем с сервера
        let result = await retryManager.execute {
            try await self.apiService.getNotifications()
        }
        ...
        case .success(let notifications):
            // Сохраняем в кэш
            if isCachingEnabled {
                cacheManager.store(notifications, forKey: cacheKey, ttl: 180, priority: .high)
            }
    ...
    func getSubscriptionInfo() async -> Result<SubscriptionResponse, NetworkError> {
        ...
        // Пытаемся получить из кэша
        if isCachingEnabled, let cachedSubscription: SubscriptionResponse = cacheManager.retrieve(SubscriptionResponse.self, forKey: cacheKey) {
        ...
        case .success(let subscription):
            // Сохраняем в кэш
            if isCachingEnabled {
                cacheManager.store(subscription, forKey: cacheKey, ttl: 1800, priority: .normal)
            }
    ...
    func getDeviceInfo() async -> Result<DeviceInfoResponse, NetworkError> {
        ...
        // Пытаемся получить из кэша
        if isCachingEnabled, let cachedDeviceInfo: DeviceInfoResponse = cacheManager.retrieve(DeviceInfoResponse.self, forKey: cacheKey) {
