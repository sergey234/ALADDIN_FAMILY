    ...
    def validate_no_personal_data(data: Dict[str, Any]) -> ComplianceAudit:
        ...
        for key, value in data.items():
            if key.lower() in personal_data_indicators:
                ...
                risks.append(f"Обнаружено поле '{key}' которое может содержать ПД")
                recommendations.append(f"Удалить или анонимизировать поле '{key}'")
            
            if isinstance(value, str) and any(indicator in value.lower() for indicator in personal_data_indicators):
                personal_data_detected = True
                risks.append(f"Значение поля '{key}' может содержать ПД")
                recommendations.append(f"Проверить и анонимизировать значение поля '{key}'")
...
class AnonymousFamilyMember:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if audit.compliance_level == ComplianceLevel.NON_COMPLIANT:
            raise ValueError(f"Данные члена семьи содержат ПД: {audit.risks_identified}")
...
class AnonymousDevice:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        audit = AnonymousDataValidator.validate_no_personal_data(data)
        if audit.compliance_level == ComplianceLevel.NON_COMPLIANT:
            raise ValueError(f"Данные устройства содержат ПД: {audit.risks_identified}")
...
class AnonymousThreatEvent:
    ...
    def __init__(self, threat_type: str, severity: ThreatLevel, member_id: str = None, device_id: str = None):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        audit = AnonymousDataValidator.validate_no_personal_data(data)
        if audit.compliance_level == ComplianceLevel.NON_COMPLIANT:
            raise ValueError(f"Данные угрозы содержат ПД: {audit.risks_identified}")
...
class ComprehensiveAnonymousFamilySystem(SecurityBase):
    ...
    def create_family_profile(
        ...
        try:
            ...
            # Проверка соответствия 152-ФЗ
            audit = AnonymousDataValidator.validate_no_personal_data(family_profile)
            self.compliance_audits.append(audit)
            
            if audit.compliance_level == ComplianceLevel.NON_COMPLIANT:
                raise ValueError(f"Семейный профиль не соответствует 152-ФЗ: {audit.risks_identified}")
    ...
    def add_family_member(
        ...
        try:
            ...
            self.family_profiles[family_id]["last_activity"] = datetime.now().isoformat()
            
            # Проверка соответствия обновленного профиля
            audit = AnonymousDataValidator.validate_no_personal_data(self.family_profiles[family_id])
            self.compliance_audits.append(audit)
            
            if audit.compliance_level == ComplianceLevel.NON_COMPLIANT:
                # Откат изменений при нарушении соответствия
                self.family_profiles[family_id]["members"].pop()
                raise ValueError(f"Добавление члена семьи нарушает 152-ФЗ: {audit.risks_identified}")
            
            self.logger.info(f"Добавлен анонимный член семьи: {member_data['member_id']}")
            return member_data
    ...
    def register_device(
        ...
        try:
            ...
            if audit.compliance_level == ComplianceLevel.NON_COMPLIANT:
                ...
                raise ValueError(f"Регистрация устройства нарушает 152-ФЗ: {audit.risks_identified}")
            
            self.logger.info(f"Зарегистрировано анонимное устройство: {device_data['device_id']}")
            return device_data
    ...
    def record_threat_event(
        ...
        try:
            ...
            threat_event = AnonymousThreatEvent(threat_type, severity, member_id, device_id)
            threat_data = threat_event.to_dict()
            
            # Добавление в семейный профиль
            self.family_profiles[family_id]["threat_events"].append(threat_data)
            self.family_profiles[family_id]["last_activity"] = datetime.now().isoformat()
            
            # Обновление статистики
            self._update_threat_statistics(family_id, member_id, device_id, threat_data["action_taken"])
            ...
            if audit.compliance_level == ComplianceLevel.NON_COMPLIANT:
                ...
                raise ValueError(f"Запись угрозы нарушает 152-ФЗ: {audit.risks_identified}")
            
            self.logger.info(f"Записано анонимное событие угрозы: {threat_data['threat_id']}")
            return threat_data
    ...
    def _update_threat_statistics(self, family_id: str, member_id: str, device_id: str, action_taken: str):
    ...
    def get_family_analytics(self, family_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            total_threats = sum(member.get("threats_encountered", 0) for member in family["members"])
            blocked_threats = sum(member.get("threats_blocked", 0) for member in family["members"])
            ...
            for role in FamilyRole:
                role_members = [m for m in family["members"] if m["role"] == role.value]
                if role_members:
                    role_analytics[role.value] = {
                        "count": len(role_members),
                        "threats_encountered": sum(m.get("threats_encountered", 0) for m in role_members),
                        "threats_blocked": sum(m.get("threats_blocked", 0) for m in role_members),
                        "educational_lessons": sum(m.get("educational_lessons_completed", 0) for m in role_members)
                    }
            
            # Аналитика по устройствам
            device_analytics = {}
            for device_type in DeviceType:
                type_devices = [d for d in family["devices"] if d["device_type"] == device_type.value]
                if type_devices:
                    device_analytics[device_type.value] = {
                        "count": len(type_devices),
                        "threats_detected": sum(d.get("threats_detected", 0) for d in type_devices),
                        "threats_blocked": sum(d.get("threats_blocked", 0) for d in type_devices),
                        "software_updates": sum(d.get("software_updates", 0) for d in type_devices)
                    }
            ...
            if audit.compliance_level == ComplianceLevel.NON_COMPLIANT:
                raise ValueError(f"Аналитика содержит ПД: {audit.risks_identified}")
    ...
    def get_compliance_report(self) -> Dict[str, Any]:
        ...
        compliant_audits = len([a for a in self.compliance_audits if a.compliance_level == ComplianceLevel.FULL_COMPLIANCE])
        non_compliant_audits = len([a for a in self.compliance_audits if a.compliance_level == ComplianceLevel.NON_COMPLIANT])
        
        compliance_percentage = (compliant_audits / total_audits * 100) if total_audits > 0 else 100
        ...
        return {
            ...
            "last_audit": self.compliance_audits[-1].timestamp.isoformat() if self.compliance_audits else None,
            "overall_status": "FULL_COMPLIANCE" if compliance_percentage == 100 else "NEEDS_ATTENTION",
            "152_fz_compliant": compliance_percentage == 100,
            "personal_data_detected": any(a.personal_data_detected for a in self.compliance_audits),
    ...
    def cleanup_expired_data(self) -> Dict[str, int]:
        ...
        for family_id, family in self.family_profiles.items():
            ...
            if family.get("is_active", False):
                ...
                if (now - last_activity).days > self.data_retention_policy["session_data"]:
            ...
            family["threat_events"] = [
                ...
                if (now - datetime.fromisoformat(event["detected_at"])).days <= self.data_retention_policy["threat_events"]
            ]
            cleaned_counts["threat_events"] += original_count - len(family["threat_events"])
...
def main():
    ...
    print(f"Соответствие 152-ФЗ: {compliance_report['compliance_percentage']}%")
    
    print("✅ Комплексная анонимная семейная система работает с полным соответствием 152-ФЗ!")
