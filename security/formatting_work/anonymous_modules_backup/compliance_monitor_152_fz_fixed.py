...
class ComplianceRule:
    ...
    def __init__(
        self, rule_id: str, name: str, description: str, check_function
    ):
...
class ComplianceMonitor(SecurityBase):
        ...
        try:
                ...
                try:
                    ...
                    if rule_result["status"] == "passed":
                        ...
                        violation = ComplianceViolation(
                            ...
                            violation_type=ViolationType(
                                rule_result.get("violation_type", "unknown")
                            ),
                            severity=rule_result.get("severity", "medium"),
                            description=rule_result.get(
                                "description", "Нарушение правила соответствия"
                            ),
                            affected_data=rule_result.get("affected_data", []),
                            remediation_required=rule_result.get(
                                "remediation_required", True
                            ),
                            auto_fixable=rule_result.get(
                                "auto_fixable", False
                            ),
                        )
                    ...
                    self.logger.error(
                        f"Ошибка проверки правила {rule.name}: {e}"
                    )
                    failed_rules += 1
            ...
            if failed_rules == 0:
                check_results["overall_status"] = (
                    ComplianceStatus.FULLY_COMPLIANT.value
                )
                ...
                check_results["overall_status"] = (
                    ComplianceStatus.CRITICAL_VIOLATION.value
                )
            ...
            self.metrics.compliance_percentage = (
                (passed_rules / total_rules * 100) if total_rules > 0 else 100
            )
            ...
            self.logger.info(
                f"Проверка соответствия завершена. Статус: {check_results['overall_status']}"
            )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if critical_violations:
            recommendations.append(
                "КРИТИЧНО: Немедленно устраните критические нарушения"
            )

        if high_violations:
            recommendations.append(
                "ВАЖНО: Устраните нарушения высокой важности в течение 24 часов"
            )

        # Общие рекомендации
        if self.metrics.compliance_percentage < 100:
            recommendations.append(
                f"Улучшите общее соответствие с {self.metrics.compliance_percentage:.1f}% до 100%"
            )

        if self.metrics.critical_violations > 0:
            recommendations.append(
                f"Устраните {self.metrics.critical_violations} критических нарушений"
            )
    ...
    def get_violations_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unfixed_violations": len(
                [v for v in self.violations if not v.fixed]
            ),
    ...
    def fix_violation(self, violation_id: str) -> bool:
        ...
        try:
            for violation in self.violations:
                if violation.violation_id == violation_id:
                    if violation.auto_fixable:
                        ...
                        self.logger.info(
                            f"Нарушение {violation_id} исправлено автоматически"
                        )
                        return True
                    else:
                        self.logger.warning(
                            f"Нарушение {violation_id} требует ручного исправления"
                        )
                        return False
            ...
            self.logger.error(
                f"Ошибка исправления нарушения {violation_id}: {e}"
            )
            return False
