...
class AnonymousDataManager(SecurityBase):
    ...
    def register_anonymous_device(
        ...
        try:
            ...
            anonymous_device = {
                "device_id": device_id,
                "session_id": session_id,
                "device_type": device_info.get("type", "unknown"),  # "smartphone", "tablet", "laptop"
                "os_type": device_info.get("os_type", "unknown"),   # "iOS", "Android", "Windows"
            ...
            self.logger.info(f"Зарегистрировано анонимное устройство: {device_id}")
    ...
    def get_anonymous_analytics(self, session_id: str = None) -> Dict[str, Any]:
        """Получение анонимной аналитики без ПД"""
        try:
            analytics = {
                "general_statistics": {
                    "total_sessions": len(self.anonymous_sessions),
                    "active_sessions": len([s for s in self.anonymous_sessions.values() if s["is_active"]]),
                    "total_devices": len(self.anonymous_devices),
                    "total_threats_detected": sum(self.threat_statistics.values()),
            ...
            if session_id and session_id in self.anonymous_sessions:
                ...
                session_devices = [d for d in self.anonymous_devices.values() 
                                 if d["session_id"] == session_id]
                
                analytics["session_analytics"] = {
                    "session_duration": self._calculate_session_duration(session),
                    "devices_registered": len(session_devices),
                    "threats_encountered": sum(d["threats_detected"] for d in session_devices),
                    "threats_blocked": sum(d["threats_blocked"] for d in session_devices),
                    "security_score": self._calculate_security_score(session_devices)
                }
    ...
    def get_educational_content(
        ...
        try:
            ...
            return content_templates.get(content_type, content_templates["cybersecurity_basics"])
    ...
    def get_general_recommendations(
        self,
        threat_statistics: Dict[str, int] = None
    ) -> List[Dict[str, Any]]:
