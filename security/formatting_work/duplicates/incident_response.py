...
class IncidentResponseManager(SecurityBase):
    ...
    def _update_indexes(self, incident: "Incident", action: str = "add"):
        ...
        if action == "add":
            ...
            self._priority_index.setdefault(priority_key, set()).add(
                incident_id
            )
            ...
            if hasattr(incident, "assigned_to") and incident.assigned_to:
                self._assigned_to_index.setdefault(
                    incident.assigned_to, set()
                ).add(incident_id)
            ...
            for index in [
                ...
                self._type_index,
                self._created_at_index,
                self._assigned_to_index,
            ]:
                for key in index:
    ...
    def _check_rate_limit(self, user_id: str = "default") -> bool:
        ...
        if len(user_requests) >= self._max_requests_per_minute:
            self.log_activity(
                f"Rate limit exceeded for user {user_id}", "warning"
            )
            return False
    ...
    def _calculate_response_times(self) -> Dict[str, float]:
        ...
        for incident in self.incidents.values():
            if (
                hasattr(incident, "first_response_time")
                and incident.first_response_time
            ):
    ...
    def _analyze_escalation_patterns(self) -> Dict[str, Any]:
        ...
        escalation_data = {
            ...
            "escalation_rate": self.escalated_incidents
            / max(self.total_incidents, 1),
            "escalation_by_type": {},
        ...
        for incident in self.incidents.values():
            if incident.is_escalated():
                ...
                escalation_data["escalation_by_type"][type_key] = (
                    escalation_data["escalation_by_type"].get(type_key, 0) + 1
                )
                ...
                escalation_data["escalation_by_priority"][priority_key] = (
                    escalation_data["escalation_by_priority"].get(
                        priority_key, 0
                    )
                    + 1
                )
    ...
    def _calculate_resolution_efficiency(self) -> Dict[str, Any]:
        ...
        resolved_incidents = [
            inc for inc in self.incidents.values() if inc.is_resolved()
        ]
        ...
        resolution_times = [
            inc.get_resolution_time_hours()
            for inc in resolved_incidents
            if inc.get_resolution_time_hours()
        ]
        ...
        efficiency_score = max(
            0, 100 - (avg_resolution * 2)
        )  # 2 балла за каждый час
    ...
    def _analyze_team_performance(self) -> Dict[str, Any]:
            ...
            if hasattr(incident, "assigned_to") and incident.assigned_to:
                ...
                if incident.is_resolved():
                    ...
                    if incident.get_resolution_time_hours():
                        ...
                        ] += incident.get_resolution_time_hours()
        ...
        for team in team_stats:
            if team_stats[team]["resolved_incidents"] > 0:
                team_stats[team]["average_resolution_time"] /= team_stats[
                    team
                ]["resolved_incidents"]
    ...
    def _analyze_trends(self) -> Dict[str, Any]:
        ...
        if len(dates) < 2:
            return {
                ...
                "daily_average": sum(daily_incidents.values())
                / len(daily_incidents),
            }

        recent_avg = sum(list(daily_incidents.values())[-7:]) / min(
            7, len(daily_incidents)
        )
    ...
    def _assess_risk_levels(self) -> Dict[str, Any]:
        ...
        risk_incidents = {"critical": 0, "high": 0, "medium": 0, "low": 0}
        ...
        return {
            ...
            "overall_risk_score": min(
                100,
                risk_incidents["critical"] * 4
                + risk_incidents["high"] * 2
                + risk_incidents["medium"],
            ),
        }
    ...
    def _get_rate_limiting_stats(self) -> Dict[str, Any]:
        ...
        total_requests = sum(
            len(requests) for requests in self._rate_limits.values()
        )
        ...
        return {
            ...
            "rate_limited_users": len(
                [
                    user
                    for user, requests in self._rate_limits.items()
    ...
    async def get_incidents_batch_async(
        self, incident_ids: List[str]
    ) -> Dict[str, Any]:
        """Асинхронное получение нескольких инцидентов"""
        tasks = [
            self.get_incident_async(incident_id)
            for incident_id in incident_ids
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        return {
            incident_id: result
            for incident_id, result in zip(incident_ids, results)
        }

    async def create_incidents_batch_async(
