...
from security.types.security_types import SecurityEvent, IncidentSeverity
...
class ContextAwareAccess(SecurityBase):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """
        Инициализация системы контекстно-зависимого доступа

        Args:
            config: Конфигурация системы (опционально)

        Raises:
            TypeError: Если config не является словарем
            ValueError: Если config содержит некорректные значения
        """
        # Валидация параметров
        if config is not None and not isinstance(config, dict):
            raise TypeError("config должен быть словарем или None")
    ...
    def __str__(self) -> str:
        ...
        return (
            f"ContextAwareAccess(rules={len(self.access_rules)}, "
            f"decisions={len(self.access_decisions)})"
        )

    def __repr__(self) -> str:
        """Техническое представление объекта"""
        return (
            f"ContextAwareAccess(name='{self.name}', "
            f"rules={len(self.access_rules)})"
        )

    def __len__(self) -> int:
        """Количество активных правил доступа"""
        return len(self.access_rules)

    def __bool__(self) -> bool:
        """Проверка наличия активных правил"""
        return len(self.access_rules) > 0

    def __eq__(self, other) -> bool:
        """Сравнение на равенство"""
        if not isinstance(other, ContextAwareAccess):
            return False
        return (self.name == other.name and
                len(self.access_rules) == len(other.access_rules))

    def __lt__(self, other) -> bool:
        """Сравнение меньше"""
        if not isinstance(other, ContextAwareAccess):
            return NotImplemented
        return len(self.access_rules) < len(other.access_rules)
    ...
    async def async_evaluate_access_request(
        self, user_id: str, resource: str, context_data: ContextData
    ) -> AccessDecision:
        """Асинхронная версия оценки запроса на доступ"""
        import asyncio
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self.evaluate_access_request, user_id, resource, context_data
        )

    async def async_create_access_rule(
        self, rule_id: str, name: str, description: str,
        context_conditions: Dict[ContextFactor, Any],
        access_level: AccessLevel, priority: int = 100
    ) -> bool:
        """Асинхронная версия создания правила доступа"""
        import asyncio
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self.create_access_rule, rule_id, name, description,
            context_conditions, access_level, priority
        )

    async def async_get_status(self) -> Dict[str, Any]:
        """Асинхронная версия получения статуса"""
        import asyncio
        loop = asyncio.get_event_loop()
    ...
    def _initialize_default_rules(self) -> None:
        ...
        try:
            ...
            home_rule = AccessRule(
                ...
                description=(
                    "Разрешить полный доступ для доверенных устройств "
                    "в домашней сети"
                ),
                context_conditions={
            ...
            public_rule = AccessRule(
                ...
                context_conditions={ContextFactor.NETWORK: "public"},
            ...
            school_rule = AccessRule(
                ...
                description=(
                    "Заблокировать развлекательный контент в школьные часы"
                ),
                context_conditions={
            ...
            high_risk_rule = AccessRule(
                ...
                context_conditions={ContextFactor.RISK_LEVEL: 0.8},
                access_level=AccessLevel.DENIED,
                priority=95,
            )
            ...
            low_trust_rule = AccessRule(
                ...
                context_conditions={ContextFactor.TRUST_SCORE: 0.3},
                access_level=AccessLevel.RESTRICTED,
                priority=75,
            )
            ...
            self.logger.info(
                f"Инициализировано {len(self.access_rules)} правил доступа"
            )
        ...
        try:
            ...
            access_decision = AccessDecision(
                ...
                reasoning=self._generate_reasoning(
                    applicable_rules, context_score
                ),
            ...
            security_event = SecurityEvent(
                ...
                severity=(
                    ...
                    if decision == AccessDecisionType.ALLOW
                    else IncidentSeverity.MEDIUM
                ),
                description=(
                    f"Принято решение о доступе: {decision.value} "
                    f"для {user_id}"
                ),
            ...
            self.logger.info(
                (
                    f"Принято решение о доступе: {decision.value} "
                    f"для {user_id} (уверенность: {confidence:.2f})"
                )
            )
    ...
    def _analyze_context(self, context_data: ContextData) -> float:
        ...
        try:
            ...
            if context_data.location:
                ...
                context_score += (
                    location_score
                    * self.context_weights[ContextFactor.LOCATION]
                )
            ...
            context_score += (
                time_score * self.context_weights[ContextFactor.TIME]
            )
            ...
            context_score += (
                device_score * self.context_weights[ContextFactor.DEVICE]
            )
            total_weight += self.context_weights[ContextFactor.DEVICE]

            # Анализируем сеть
            if context_data.network_type:
                network_score = self._evaluate_network(
                    context_data.network_type
                )
                context_score += (
                    network_score * self.context_weights[ContextFactor.NETWORK]
                )
            ...
            context_score += (
                behavior_score
                * self.context_weights[ContextFactor.USER_BEHAVIOR]
            )
            ...
            context_score += (
                risk_score * self.context_weights[ContextFactor.RISK_LEVEL]
            )
            ...
            context_score += (
                trust_score * self.context_weights[ContextFactor.TRUST_SCORE]
            )
            ...
            context_score += (
                auth_score * self.context_weights[ContextFactor.AUTHENTICATION]
            )
    ...
    def _rule_matches_context(
        self, rule: AccessRule, context_data: ContextData
    ) -> bool:
            ...
            for factor, condition in rule.context_conditions.items():
                if factor == ContextFactor.NETWORK:
                    ...
                    if not self._check_time_condition(
                        condition, context_data.timestamp
                    ):
                        return False
    ...
    def _check_time_condition(
        self, condition: str, timestamp: datetime
    ) -> bool:
    ...
    def _make_access_decision(
        ...
        context_score: float,
    ) -> Tuple[AccessDecisionType, AccessLevel, float]:
    ...
    def _generate_reasoning(
        self, applicable_rules: List[AccessRule], context_score: float
    ) -> str:
        """Генерация обоснования решения"""
        try:
            if not applicable_rules:
                return (
                    f"Решение на основе контекстного балла: "
                    f"{context_score:.2f}"
                )

            rule_names = [rule.name for rule in applicable_rules]
            return (
                f"Применены правила: {', '.join(rule_names)}. "
                f"Контекстный балл: {context_score:.2f}"
            )
    ...
    def get_access_summary(
        ...
        try:
            ...
            user_decisions = [
                d
                for d in self.access_decisions
            ...
            for decision in user_decisions:
                ...
                access_level_counts[access_level] = (
                    access_level_counts.get(access_level, 0) + 1
                )
        ...
        try:
            ...
            status = {
                **base_status,
                ...
                "rules_by_priority": {
                    str(rule.priority): len(
                        [
                            r
                            for r in self.access_rules.values()
                ...
                "context_weights": {
                    k.value: v for k, v in self.context_weights.items()
                },
                "context_access_levels": {
                    k.value: v.value
                    for k, v in self.context_access_levels.items()
                },
