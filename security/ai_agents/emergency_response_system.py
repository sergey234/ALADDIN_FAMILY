...
class EmergencyResponseSystem(SecurityBase):
    ...
    def _initialize_family_contacts(self) -> Dict[str, FamilyContact]:
        ...
        return {
            ...
            "secondary_contact": FamilyContact(
                ...
                notification_channels=[
                    NotificationChannel.PUSH,
                    NotificationChannel.APP_NOTIFICATION,
                ],
                is_active=True,
            ),
    ...
    async def activate_emergency_mode(
        ...
        """
        Активация экстренного режима (алиас для trigger_emergency_mode)
        ...
        Returns:
            bool: Успешность активации экстренного режима
        """
        try:
            return await self.trigger_emergency_mode(elderly_id, alert)
    ...
    async def trigger_emergency_mode(
        ...
        try:
            self.logger.warning(
                f"Активация экстренного режима для {elderly_id}"
            )
            ...
            # Банковские алерты
            if alert.emergency_type in [
                EmergencyType.FINANCIAL_THEFT,
                EmergencyType.BANKING_SCAM,
            ]:
            ...
            self.logger.warning(
                f"Экстренный режим активирован для {elderly_id}"
            )
            return True
    ...
    async def _notify_family_emergency(
        self, elderly_id: str, alert: EmergencyAlert
    ):
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка уведомления семьи об экстренной ситуации: {e}"
            )
    ...
    async def _send_contact_notification(
        self, contact: FamilyContact, alert: EmergencyAlert
    ) -> bool:
        ...
        try:
            ...
            for channel in contact.notification_channels:
                if channel == NotificationChannel.PUSH:
                    ...
                    success |= await self._send_phone_notification(
                        contact, alert
                    )
                elif channel == NotificationChannel.APP_NOTIFICATION:
                    success |= await self._send_app_notification(
                        contact, alert
                    )
    ...
    async def _send_push_notification(
        ...
        try:
            ...
            self.logger.info(
                f"Push-уведомление отправлено {contact.name}: {alert.title}"
            )
            return True
        except Exception as e:
    ...
    async def _send_app_notification(
        ...
        try:
            ...
            self.logger.info(
                f"Уведомление в приложение {contact.name}: {alert.title}"
            )
            return True
        except Exception as e:
    ...
    async def _activate_protective_measures(
        self, elderly_id: str, alert: EmergencyAlert
    ):
        ...
        try:
            ...
            for action in protective_actions:
                await self._execute_protective_action(
                    elderly_id, action, alert
                )
    ...
    async def _execute_protective_action(
        self, elderly_id: str, action: str, alert: EmergencyAlert
    ):
    ...
    async def _integrate_with_blocking_system(self, phone_number: str):
        ...
        try:
            self.logger.info(
                f"Интеграция с системой блокировки для {phone_number}"
            )
    ...
    async def deactivate_emergency_mode(self, elderly_id: str) -> bool:
        ...
        try:
            if elderly_id in self.active_emergency_modes:
                ...
                self.logger.info(
                    f"Экстренный режим деактивирован для {elderly_id}"
                )
                return True
            return False
        except Exception as e:
