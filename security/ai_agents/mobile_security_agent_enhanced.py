...
class MobileSecurityAgent(SecurityBase):
    ...
    def spam_call_blocking(self, phone_number: str, call_context: dict) -> bool:
        ...
        try:
            ...
            caller_id_result = self.caller_id_verification(
                phone_number, call_context.get("caller_info", {})
            )
            if (caller_id_result["is_spoofed"] and
                    caller_id_result["verification_status"] in ["spoofed", "likely_spoofed"]):
                self.log_activity(
                    f"🚫 Звонок заблокирован (подделка): {phone_number}"
                )
                return True
