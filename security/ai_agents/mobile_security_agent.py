...
class MobileSecurityAgent(SecurityBase):
    ...
    def robocall_detection(self, phone_number: str, call_data: dict) -> dict:
        ...
        try:
            self.log_activity(
                f"Анализ роботов-звонков для номера: {phone_number}"
            )
            ...
            if "voice_analysis" in call_data:
                voice_result = self._analyze_voice_patterns(
                    call_data["voice_analysis"]
                )
            ...
            if "call_context" in call_data:
                context_result = self._analyze_call_context(
            ...
            result["analysis_details"]["total_indicators"] = len(
                robocall_indicators
            )
            ...
            if result["is_robocall"]:
                self.log_activity(
                    f"🚨 РОБОТ-ЗВОНОК ОБНАРУЖЕН: {phone_number} "
                    f"(уверенность: {result['confidence']:.2f})"
                )
            else:
                self.log_activity(
                    f"✅ Звонок не является роботом: {phone_number}"
                )
    ...
    def caller_id_verification(
        self, phone_number: str, caller_info: dict
    ) -> dict:
        """
        ...
        try:
            self.log_activity(
                f"Верификация Caller ID для номера: {phone_number}"
            )
            ...
            known_number_check = self._check_known_number_database(
                phone_number
            )
            ...
            cross_verification = self._cross_verify_caller_id(
            ...
            result["verification_details"]["total_indicators"] = len(
                spoofing_indicators
            )
            ...
            if result["is_spoofed"]:
                self.log_activity(
                    f"🚨 ПОДДЕЛКА CALLER ID ОБНАРУЖЕНА: {phone_number}"
                )
            else:
                self.log_activity(
                    f"✅ Caller ID верифицирован: {phone_number}"
                )
    ...
    def spam_call_blocking(
        self, phone_number: str, call_context: dict
    ) -> bool:
        """
        ...
        try:
            ...
            if self._check_blacklist(phone_number):
                self.log_activity(
                    f"🚫 Звонок заблокирован (черный список): {phone_number}"
                )
                return True
            ...
            if self._check_whitelist(phone_number):
                self.log_activity(
                    f"✅ Звонок разрешен (белый список): {phone_number}"
                )
                return False

            # 3. Анализ роботов-звонков
            robocall_result = self.robocall_detection(
                phone_number, call_context
            )
            if robocall_result["is_robocall"] and robocall_result[
                "threat_level"
            ] in ["high", "critical"]:
                self.log_activity(
                    f"🚫 Звонок заблокирован (робот): {phone_number}"
                )
                return True
            ...
            if caller_id_result["is_spoofed"] and caller_id_result[
                "verification_status"
            ] in ["spoofed", "likely_spoofed"]:
                self.log_activity(
            ...
            context_analysis = self._analyze_call_context_for_spam(
                call_context
            )
            if context_analysis["is_spam"]:
                self.log_activity(
                    f"🚫 Звонок заблокирован (спам-контекст): {phone_number}"
                )
                return True
            ...
            if reputation_check["is_spam"]:
                self.log_activity(
                    f"🚫 Звонок заблокирован (репутация): {phone_number}"
                )
                return True
            ...
            if ml_analysis["is_spam"] and ml_analysis["confidence"] >= 0.7:
                self.log_activity(
                    f"🚫 Звонок заблокирован (ML анализ): {phone_number}"
                )
                return True
    ...
    def phone_security_analysis(self, device_id: str) -> dict:
        ...
        try:
            self.log_activity(
                f"Анализ безопасности звонков для устройства: {device_id}"
            )
            ...
            security_score = self._calculate_call_security_score(
                security_settings, call_analysis, threats
            )
            result["security_score"] = security_score

            # 5. Генерация рекомендаций
            recommendations = self._generate_call_security_recommendations(
            ...
            # Логирование результата
            self.log_activity(
                f"📊 Анализ безопасности звонков завершен для {device_id}: "
                f"{security_score:.2f}"
            )
            ...
            self.log_activity(
                f"❌ Ошибка анализа безопасности звонков: {str(e)}"
            )
            return {
                "device_id": device_id,
                "error": str(e),
    ...
    def _check_known_number_database(self, phone_number: str) -> dict:
        ...
        try:
            return {"is_known": False, "source": "unknown", "confidence": 0.0}
        except Exception:
    ...
    def _check_phone_whitelist(self, phone_number: str) -> bool:
        """Проверка в белом списке телефонов"""
        try:
    ...
    def _calculate_call_security_score(
        self, settings: dict, call_analysis: dict, threats: list
    ) -> float:
        ...
        try:
            ...
            final_score = max(
                0.0, base_score - threat_penalty + settings_bonus
            )
            return min(100.0, final_score)
        except Exception:
            return 50.0

    def _generate_call_security_recommendations(
        self, security_score: float, threats: list
    ) -> list:
        ...
        try:
            ...
            if security_score < 70:
                recommendations.append(
                    "Включить блокировку неизвестных звонков"
                )
        ...
        try:
            ...
            for i in range(len(digits) - 2):
                if (
                    ...
                    and int(digits[i + 2]) == int(digits[i]) + 2
                ):
                    return True
            return False
        except Exception:
