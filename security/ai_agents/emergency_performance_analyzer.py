...
class EmergencyPerformanceAnalyzer:
    ...
    def measure_response_time(self, func, *args, **kwargs) -> tuple:
        ...
        try:
            ...
            self.logger.debug(
                f"Функция {func.__name__} выполнена за {response_time:.4f}с"
            )
    ...
    def measure_processing_time(self, events: List[EmergencyEvent]) -> float:
        ...
        try:
            ...
            self.logger.debug(
                f"Обработано {len(events)} событий за {processing_time:.4f}с"
            )
    ...
    def record_metrics(
        self, response_time: float, processing_time: float
    ) -> None:
        """
    ...
    def check_performance_issues(self) -> List[str]:
        ...
        try:
            ...
            if (
                ...
                issues.append(
                    f"Высокое время отклика: "
                    f"{stats['response_time']['avg']:.2f}с"
                )

            # Проверяем время обработки
            if (
                stats["processing_time"]["avg"]
                > self.performance_thresholds["processing_time"]
            ):
                issues.append(
                    f"Высокое время обработки: "
                    f"{stats['processing_time']['avg']:.2f}с"
                )
            ...
            if (
                ...
                issues.append(
                    f"Высокое использование памяти: "
                    f"{stats['memory_usage']['avg']:.2f}MB"
                )
            ...
            if (
                ...
                issues.append(
                    f"Высокое использование CPU: "
                    f"{stats['cpu_usage']['avg']:.2f}%"
                )
            ...
            self.logger.error(
                f"Ошибка проверки проблем производительности: {e}"
            )
            return []
