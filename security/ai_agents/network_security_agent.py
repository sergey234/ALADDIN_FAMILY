...
class NetworkSecurityAgent(SecurityBase):
    ...
    async def analyze_packet_async(
        ...
        Returns:
            Optional[NetworkThreat]: Объект угрозы если обнаружена,
            None если угроз нет
        """
        # Валидация входных параметров
        if not isinstance(packet_data, dict):
            self.log_activity(
                f"Ошибка валидации: packet_data должен быть словарем, "
                f"получен {type(packet_data)}",
                "error",
            )
            return None
        ...
        if not packet_data:
            self.log_activity(
                "Ошибка валидации: packet_data не может быть пустым", "error"
            )
            return None
        ...
        try:
            ...
            self.log_activity(
                f"Ошибка асинхронного анализа пакета: {e}", "error"
            )
            return None
    ...
    async def get_network_analysis_async(self) -> Optional[NetworkAnalysis]:
        ...
        Returns:
            Optional[NetworkAnalysis]: Объект анализа сети или
        ...
        try:
            ...
            self.log_activity(
                f"Ошибка асинхронного получения анализа сети: {e}", "error"
            )
            return None
    ...
    def analyze_network_flow(
        ...
        Returns:
            Optional[NetworkFlow]: Объект потока если создан,
            None в случае ошибки
        """
        try:
            with self.analysis_lock:
    ...
    def block_ip(
        ...
        if not isinstance(ip_address, str):
            self.log_activity(
                f"Ошибка валидации: ip_address должен быть строкой, "
                f"получен {type(ip_address)}",
                "error",
            )
            return False
        ...
        if not ip_address.strip():
            self.log_activity(
                "Ошибка валидации: ip_address не может быть пустым", "error"
            )
            return False

        if not isinstance(reason, str):
            self.log_activity(
                f"Ошибка валидации: reason должен быть строкой, "
                f"получен {type(reason)}",
                "error",
            )
            return False
