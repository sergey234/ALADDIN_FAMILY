...
class MessengerBotsIntegrationTest:
    ...
    async def setup_bots(self) -> None:
        """
        Инициализация всех ботов для интеграционного тестирования.

        Создает экземпляры всех доступных мессенджер-ботов и сохраняет их
        в словаре self.bots для последующего тестирования.

        Raises:
            ImportError: Если не удается импортировать класс бота
            RuntimeError: Если бот не может быть инициализирован

        Note:
            Включает следующие боты:
            - WhatsAppSecurityBot
            - TelegramSecurityBot
            - InstagramSecurityBot
            - MaxMessengerSecurityBot
            - AnalyticsBot
        ...
        for name, bot in self.bots.items():
            try:
                ...
                if success:
                    ...
                    logger.error(f"Bot {name} failed to start")
            except AttributeError as e:
                print(f"❌ {name}: Отсутствует метод start() - {e}")
                failed_bots.append(name)
                logger.error(f"Bot {name} missing start method: {e}")
            except ConnectionError as e:
                print(f"❌ {name}: Ошибка подключения - {e}")
                failed_bots.append(name)
                logger.error(f"Bot {name} connection error: {e}")
            except Exception as e:
                print(f"❌ {name}: Неожиданная ошибка - {e}")
                failed_bots.append(name)
                logger.error(f"Bot {name} unexpected error: {e}")

        print(f"📊 Запущено ботов: {started_bots}/{len(self.bots)}")

        if failed_bots:
            print(f"⚠️ Не запущены: {', '.join(failed_bots)}")

        return started_bots == len(self.bots)

    async def test_individual_functionality(self) -> None:
    ...
    async def _test_whatsapp_bot(self, bot: Any) -> None:
        """Тестирование WhatsApp бота"""
        if not bot:
            raise ValueError("Bot не может быть None")
        if not hasattr(bot, 'analyze_message'):
            raise AttributeError("Bot должен иметь метод analyze_message")
        ...
        try:
            ...
            logger.info("WhatsApp bot message analysis successful")

            # Тест отчета по безопасности
            if hasattr(bot, 'get_security_report'):
                report = await bot.get_security_report()
                print(f"✅ Отчет: {report.get('total_messages', 0)} сообщений")
                logger.info("WhatsApp bot security report generated")
            else:
                print("⚠️ Метод get_security_report недоступен")
                logger.warning("WhatsApp bot missing get_security_report "
                               "method")

        except Exception as e:
            logger.error(f"WhatsApp bot test failed: {e}")
            raise

    async def _test_telegram_bot(self, bot: Any) -> None:
    ...
    async def _test_instagram_bot(self, bot: Any) -> None:
    ...
    async def _test_max_messenger_bot(self, bot: Any) -> None:
    ...
    async def _test_analytics_bot(self, bot: Any) -> None:
    ...
    async def _test_website_navigation_bot(self, bot: Any) -> None:
    ...
    async def test_inter_bot_communication(self) -> None:
    ...
    def generate_test_report(self) -> Dict[str, Any]:
        """
        Генерирует подробный отчет о результатах тестирования.

        Returns:
            Dict[str, Any]: Словарь с результатами тестов, содержащий:
                - test_summary: общая статистика тестов
                - test_results: детальные результаты каждого теста
                - recommendations: рекомендации по улучшению

        Raises:
            RuntimeError: Если тесты не были запущены
            ValueError: Если данные тестов некорректны

        Note:
            Отчет сохраняется в файл 'messenger_bots_test_report.json'
            в формате JSON с кодировкой UTF-8.
        """
