...
# from datetime import datetime  # TODO: Добавить при реализации
# timestamp функций
...
class NotificationService:
    ...
    def _init_default_templates(self):
        ...
        templates = [
            NotificationTemplate(
                ...
                message="Ребенок {child_name} превысил лимит времени "
                "использования {device_type} на {excess_minutes} минут.",
            ...
            NotificationTemplate(
                ...
                message="Доступ к {url} был заблокирован для {child_name}. "
                "Причина: {reason}.",
            ...
            NotificationTemplate(
                ...
                message="Обнаружена подозрительная активность у "
                "{child_name}: {activity_description}.",
            ...
            NotificationTemplate(
                ...
                message="ЭКСТРЕННОЕ УВЕДОМЛЕНИЕ: {child_name} - "
                "{emergency_message}",
                channel=NotificationChannel.SMS,
    ...
    async def send_alert(
        self, alert: ActivityAlert, parent_contact: str = None
    ) -> bool:
        ...
        try:
            async with self._lock:
                ...
                if not template:
                    self.logger.warning(
                        f"Шаблон не найден для типа алерта: {alert.alert_type}"
                    )
                    return False
                ...
                if success:
                    ...
                    self.logger.info(
                        f"Уведомление отправлено: {alert.alert_type}"
                    )
                else:
                    self.stats.failed_deliveries += 1
                    self.logger.error(
                        f"Не удалось отправить уведомление: {alert.alert_type}"
                    )
    ...
    async def send_custom_notification(
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка отправки пользовательского уведомления: {e}"
            )
            return False
    ...
    def _prepare_template_data(
        self, alert: ActivityAlert, template: NotificationTemplate
    ) -> Dict[str, Any]:
        """Подготовка данных для шаблона"""
        data = {
    ...
    async def _send_notification(
        ...
        try:
            ...
            if template.channel == NotificationChannel.EMAIL:
                ...
                self.logger.error(
                    f"Неподдерживаемый канал: {template.channel}"
                )
                return False
    ...
    async def _send_email(
        self, subject: str, message: str, parent_contact: str
    ) -> bool:
    ...
    async def _send_in_app(
        self, subject: str, message: str, alert: ActivityAlert
    ) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка отправки внутриприложенческого уведомления: {e}"
            )
            return False

    async def _send_webhook(
        self, data: Dict[str, Any], parent_contact: str
    ) -> bool:
    ...
    async def get_notification_history(
        ...
        try:
            ...
            # Получение из Redis
            alerts_data = self.redis_client.lrange(
                "parental_alerts", 0, limit - 1
            )
            alerts = []
