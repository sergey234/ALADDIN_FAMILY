...
from security.types.security_types import (
    SecurityEvent, IncidentSeverity, FamilyRole, AgeGroup, 
    ContentCategory, ThreatLevel, FamilyProfileManager,
    ChildProtection, ElderlyProtection
)
...
class ParentalControls(SecurityBase):
    ...
    def __init__(
        ...
        Args:
            family_profile_manager: Менеджер семейных профилей
            child_protection: Система защиты детей
            elderly_protection: Система защиты пожилых
        ...
        if not isinstance(family_profile_manager, FamilyProfileManager):
            raise TypeError(
                "family_profile_manager должен быть экземпляром "
                "FamilyProfileManager"
            )
        if not isinstance(child_protection, ChildProtection):
            raise TypeError(
                "child_protection должен быть экземпляром ChildProtection"
            )
        if not isinstance(elderly_protection, ElderlyProtection):
            raise TypeError(
                "elderly_protection должен быть экземпляром ElderlyProtection"
            )
    ...
    def __len__(self) -> int:
        """
        Возвращает количество активных правил контроля.

        Returns:
            int: Количество активных правил
        """
        return len([
            rule for rule in self.control_rules.values() if rule.is_active
        ])

    def __bool__(self) -> bool:
        """
        Проверяет, есть ли активные правила контроля.

        Returns:
            bool: True если есть активные правила, False иначе
        """
        return len(self) > 0
    ...
    def __str__(self) -> str:
        ...
        Returns:
            str: Описание системы родительского контроля
        """
        active_rules = len(self)
        total_rules = len(self.control_rules)
        return (
            f"ParentalControls(активных правил: {active_rules}/{total_rules})"
        )
    ...
    def __repr__(self) -> str:
        ...
        Returns:
            str: Техническое описание объекта
        """
        return (
            f"<ParentalControls(правила={len(self.control_rules)}, "
            f"уведомления={len(self.notifications)})>"
        )

    async def async_create_control_rule(
        self,
        child_id: str,
        control_type: ControlType,
        settings: Dict[str, Any]
        ...
        """
        Асинхронное создание правила контроля.
        ...
        Returns:
            bool: True если правило создано успешно
        """
        import asyncio
        return await asyncio.get_event_loop().run_in_executor(
            None, self.create_control_rule, child_id, control_type, settings
        )

    async def async_check_time_limit(self, child_id: str) -> Tuple[bool, str]:
        """
        Асинхронная проверка ограничения времени.
        ...
        import asyncio
        return await asyncio.get_event_loop().run_in_executor(
            None, self.check_time_limit, child_id
        )

    async def async_get_status(self) -> Dict[str, Any]:
        """
        Асинхронное получение статуса системы.
