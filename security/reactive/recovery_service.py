...
class RecoveryService(SecurityBase):
    ...
    def __len__(self) -> int:
        ...
        try:
            return len(
                self.recovery_plans) if hasattr(
                self, 'recovery_plans') else 0
        except Exception:
            return 0
    ...
    def validate_recovery_plan(self, plan: 'RecoveryPlan') -> bool:
        ...
        try:
            ...
            if plan.estimated_duration <= 0:
                self.logger.warning(
                    "Некорректная продолжительность восстановления")
                return False

            self.logger.info(
                f"План восстановления {plan.plan_id} прошёл валидацию")
            return True
    ...
    def get_recovery_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, 'recovery_plans') and self.recovery_plans:
                completed = sum(
                    1 for plan in self.recovery_plans if getattr(
                ...
                stats["success_rate"] = (
                    (completed / len(self.recovery_plans)) * 100
                    if self.recovery_plans else 0)

                durations = [getattr(plan, 'estimated_duration', 0)
    ...
    def cleanup_old_plans(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            old_plans = []
            # Копия списка для безопасного удаления
            for plan in self.recovery_plans[:]:
            ...
            self.logger.info(
                f"Удалено {len(old_plans)} старых планов восстановления")
