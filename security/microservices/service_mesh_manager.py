...
class ServiceMeshManager(SecurityBase):
    ...
    def send_request(
        ...
        try:
            ...
            if not endpoint:
                self.log_activity(
                    f"Не удалось получить конечную точку для сервиса "
                    f"{service_id}",
                    "error",
                )
                return None
    ...
    async def health_check(self) -> Dict[str, Any]:
        """
        Проверка состояния сервисной сетки

        Returns:
            Dict[str, Any]: Статус здоровья сервисной сетки
        """
        try:
            import asyncio
            ...
            health_status = {
                ...
                "service": "ServiceMeshManager",
                "components": {
                    "mesh_initialized": self.status == ComponentStatus.ACTIVE,
                    "monitoring_active": self.monitoring_active,
                    "services_registered": len(self.services) > 0,
                    "load_balancers": len(self.load_balancers) > 0,
                    "circuit_breakers": len(self.circuit_breakers) > 0
                },
                "metrics": {
                    "total_services": len(self.services),
                    "total_requests": self.total_requests,
            ...
            for service_id in self.services:
                ...
                if service_status and service_status.get("status") == "healthy":
                    healthy_services += 1

            health_status["components"]["healthy_services"] = healthy_services
            health_status["components"]["unhealthy_services"] = len(self.services) - healthy_services
            ...
            if self.total_requests > 0:
                ...
                if success_rate < 80:
                    ...
                    health_status["components"]["low_success_rate"] = True

            # Проверка времени ответа
            if self.average_response_time > 5.0:  # 5 секунд
                health_status["status"] = "degraded"
                health_status["components"]["slow_response"] = True

            # Проверка зарегистрированных сервисов
            if len(self.services) == 0:
                health_status["status"] = "degraded"
                health_status["components"]["no_services"] = True
