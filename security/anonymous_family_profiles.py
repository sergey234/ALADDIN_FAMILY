...
class AnonymousFamilyProfile:
    ...
    def get_family_analytics(self) -> Dict[str, Any]:
        ...
        total_threats = sum(
            member["threats_encountered"] for member in self.members
        )
        blocked_threats = sum(
            member["threats_blocked"] for member in self.members
        )
            ...
            if role_members:
                role_analytics[role.value] = {
                    ...
                    "threats_encountered": sum(
                        m["threats_encountered"] for m in role_members
                    ),
                    "threats_blocked": sum(
                        m["threats_blocked"] for m in role_members
                    ),
                    "educational_lessons": sum(
                        m["educational_lessons_completed"]
            ...
            if type_devices:
                device_analytics[device_type.value] = {
                    ...
                    "threats_detected": sum(
                        d["threats_detected"] for d in type_devices
                    ),
                    "threats_blocked": sum(
                        d["threats_blocked"] for d in type_devices
                    ),
                    "software_updates": sum(
                        d["software_updates"] for d in type_devices
                    ),
                }
...
class AnonymousFamilyManager(SecurityBase):
    ...
    def create_family_profile(
        ...
        try:
            ...
            self.logger.info(
                f"Создан анонимный семейный профиль: {family_profile.family_id}"
            )
    ...
    def get_family_profile(
        self, family_id: str
    ) -> Optional[AnonymousFamilyProfile]:
    ...
    def get_all_family_analytics(self) -> Dict[str, Any]:
        ...
        total_members = sum(
            len(family.members) for family in self.family_profiles.values()
        )
        total_devices = sum(
            len(family.devices) for family in self.family_profiles.values()
        )
