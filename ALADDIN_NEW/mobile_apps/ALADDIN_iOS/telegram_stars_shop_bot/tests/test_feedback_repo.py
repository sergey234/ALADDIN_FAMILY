...
@pytest.mark.asyncio
async def test_feedback_repo_skips_negative_user_id(conn) -> None:
    await users_repo.upsert_user(conn, user_id=-1900000000012, username="web_guest", first_name="G")
    await orders_repo.create_order(
        conn,
        user_id=-1900000000012,
        product_id="vpn_1",
        product_title="VPN",
        payment_method="fiat",
        usd_base=1.0,
    ...
    ids = await feedback_repo.list_survey_candidates(conn, lookback_days=45, cooldown_days=30, limit=100)
    assert -1900000000012 not in ids


@pytest.mark.asyncio
async def test_feedback_repo_failed_prompt_cooldown(conn) -> None:
    await users_repo.upsert_user(conn, user_id=95021, username="u95021", first_name="C")
    await orders_repo.create_order(
        conn,
        user_id=95021,
        product_id="stars_100",
    ...
    await analytics_repo.log_event(
        ...
        meta={"via": "test", "error": "TelegramBadRequest"},
    )
    assert await feedback_repo.has_recent_prompt(conn, user_id=95021, cooldown_days=30) is True
    ids = await feedback_repo.list_survey_candidates(conn, lookback_days=45, cooldown_days=30, limit=100)
    assert 95021 not in ids
