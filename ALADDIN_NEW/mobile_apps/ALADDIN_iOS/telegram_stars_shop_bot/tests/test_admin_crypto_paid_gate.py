...
def test_gate_accepts_sqlite_row(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BOT_TOKEN", "9:row")
    monkeypatch.setenv("ADMIN_IDS", "1")
    monkeypatch.setenv("API_KEY_PEPPER", "p" * 32)
    monkeypatch.setenv("CRYPTO_PAY_ENABLED", "true")
    s = Settings()
    ...
    conn.execute("CREATE TABLE o (status TEXT, payment_method TEXT)")
    conn.execute("INSERT INTO o VALUES ('pending_payment', 'crypto')")
    row = conn.execute("SELECT status, payment_method FROM o").fetchone()
    assert crypto_manual_paid_gate_applies(row, s) is True
    conn.close()
