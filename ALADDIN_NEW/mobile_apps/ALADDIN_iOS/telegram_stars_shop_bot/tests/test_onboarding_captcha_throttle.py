...
def test_onboarding_combined_caption_and_kb(monkeypatch: pytest.MonkeyPatch) -> None:
    from bot.config import Settings
    from bot.keyboards.shop_kb import onboarding_combined_kb
    from bot.services import marketing

    monkeypatch.setenv("USD_RUB_RATE", "90")
    monkeypatch.setenv("REQUIRED_CHANNEL_INVITE_URL", "https://t.me/+xwj4zZo4bNphZjVi")
    monkeypatch.setenv("PRIVACY_POLICY_URL", "https://example.com/privacy")
    monkeypatch.setenv("TERMS_OF_SERVICE_URL", "https://example.com/terms")
    s = Settings()
    html = marketing.onboarding_combined_caption_html(s)
    assert "Добро пожаловать в Ai Monkey Stars" in html
    assert "Проверить и продолжить" in html
    assert 'href="https://example.com/terms"' in html
    assert 'href="https://example.com/privacy"' in html
    assert "Пользовательское соглашение" in html
    assert "Политику конфиденциальности" in html
    kb = onboarding_combined_kb(s)
    labels = [b.text for row in kb.inline_keyboard for b in row]
    assert labels == [
        "📢 Подписаться на канал",
        "✅ Проверить и продолжить",
    ]
    assert not any("Пользовательское соглашение" == t or "Политика конфиденциальности" == t for t in labels)
    datas = [b.callback_data for row in kb.inline_keyboard for b in row if b.callback_data]
    assert "onb:ch:check" in datas


def test_onboarding_terms_includes_channel_link(monkeypatch: pytest.MonkeyPatch) -> None:
    """Legacy terms caption still includes channel for old messages."""
    from bot.config import Settings
    from bot.services import marketing
