...
@pytest.fixture
def wata_client(tmp_path, monkeypatch):
    db = tmp_path / "watahook.db"
    monkeypatch.setenv("DATABASE_PATH", str(db))
    monkeypatch.setenv("BOT_TOKEN", "9:wata-test")
    monkeypatch.setenv("ADMIN_IDS", "1")
    monkeypatch.setenv("API_KEY_PEPPER", "wata_test_pepper_minimum_32_chars______")
    monkeypatch.setenv("PAYMENT_WEBHOOK_SECRET", "payment_fallback_secret_for_wata_tests")
    wata_secret = "wata_webhook_secret_for_tests_only_______"
    monkeypatch.setenv("WATA_WEBHOOK_SECRET", wata_secret)

    async def seed() -> int:
        conn = await connect(db)
        await users_repo.upsert_user(conn, user_id=777002, username="watau", first_name="W")
        cid, _ = await api_clients_repo.create_api_client(
            conn, owner_user_id=777002, pepper=os.environ["API_KEY_PEPPER"], label="wata", revoke_previous=False
        )
        idem = str(uuid.uuid4())
        oid, _ = await orders_repo.create_order_partner_api(
            conn,
            owner_user_id=777002,
            api_client_id=cid,
            idempotency_key=idem,
            external_ref="ext-wata-1",
            product_id="stars_100",
            product_title="Stars 100",
            payment_method="fiat",
            usd_base=1.0,
    ...
    order_id = asyncio.run(seed())
    with TestClient(create_app()) as client:
        yield client, wata_secret, order_id, db
...
def test_wata_webhook_marks_paid_with_wata_signature_header(wata_client) -> None:
    client, secret, order_id, _db = wata_client
    idem = str(uuid.uuid4())
    body = {"idempotency_key": idem, "order_id": order_id, "action": "mark_paid"}
    raw = json.dumps(body, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
    sig = hmac.new(secret.encode("utf-8"), raw, hashlib.sha256).hexdigest()
    r = client.post(
        "/v1/payments/wata-webhook",
        content=raw,
        headers={"Content-Type": "application/json", "X-Wata-Signature": f"sha256={sig}"},
    )
    assert r.status_code == 200, r.text
    data = r.json()
    assert data["ok"] is True
...
def test_wata_webhook_fallbacks_to_payment_secret(payment_client) -> None:
    client, secret, order_id, _db = payment_client
    idem = str(uuid.uuid4())
    body = {"idempotency_key": idem, "order_id": order_id, "action": "mark_paid"}
    raw = json.dumps(body, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
    sig = hmac.new(secret.encode("utf-8"), raw, hashlib.sha256).hexdigest()
...
def test_wata_webhook_bad_signature(wata_client) -> None:
    client, _secret, order_id, _db = wata_client
    body = {"idempotency_key": str(uuid.uuid4()), "order_id": order_id, "action": "mark_paid"}
    raw = json.dumps(body, separators=(",", ":")).encode("utf-8")
    r = client.post(
        "/v1/payments/wata-webhook",
        content=raw,
        headers={"Content-Type": "application/json", "X-Wata-Signature": "deadbeef"},
    )
    assert r.status_code == 401
