...
def test_products_yaml_loads_and_has_stars_and_premium() -> None:
    ...
    vpn_ids = {"vpn_7d", "vpn_30d", "vpn_90d", "vpn_180d", "vpn_270d", "vpn_365d"}
    assert by_id["vpn_7d"].price_rub == 50.0
    assert vpn_ids <= set(by_id)
    assert by_id["vpn_30d"].price_rub == 200.0
    assert by_id["vpn_90d"].price_rub == 500.0
    assert by_id["vpn_180d"].price_rub == 900.0
    assert by_id["vpn_270d"].price_rub == 1200.0
    assert by_id["vpn_365d"].price_rub == 1700.0
...
def test_vpn_referral_blurb_uses_env_percents(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "Пригласить друга" in html
...
def test_vpn_marketing_welcome_copy(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "VLESS" not in html and "REALITY" not in html
    assert "/sub" not in html
    assert "Happ" not in html
    assert "Ai Monkey VPN" not in html
    assert "обход блокировок" not in html.lower()
    assert "повседневного использования" not in html
    assert "Высокая скорость" in html
    assert "Работает в России" in html
    assert "Конфиденциальное соединение" in html
...
def test_vpn_marketing_screen_no_duplicate_benefits_when_active(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from bot.handlers.vpn import vpn_marketing_screen_html
    from bot.services.vpn_user_status import ACTIVE_STATUS_MARKER

    monkeypatch.setenv("BOT_TOKEN", "9:vpn-mkt2")
    monkeypatch.setenv("ADMIN_IDS", "1")
    monkeypatch.setenv("API_KEY_PEPPER", "m" * 32)
    monkeypatch.setenv("VPN_DOCS_PUBLIC_BASE", "https://aladdin-ai.ru/v1/legal")
    s = load_settings()
    ...
    screen = vpn_marketing_screen_html(s, active_card)
    assert screen.count("Высокая скорость") == 1
    assert "Работает в России" not in screen
    assert "Конфиденциальное соединение" not in screen
    assert "Политика конфиденциальности" not in screen
    inactive = "🔴 <b>Не активен</b>\nТариф: нет активной подписки"
    guest = vpn_marketing_screen_html(s, inactive)
    assert "Высокая скорость" in guest
    assert "Политика конфиденциальности" in guest


def test_vpn_tariffs_cta_labels() -> None:
...
def test_vpn_price_rub_fixed_against_usd_rate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert list_price_rub(p, s) == 500.0
    q = quote_product(p, s, is_first_order=True)
    assert q.rub_list == 500.0
    assert q.rub_referral_discount == 0.0
    assert q.rub_final == 500.0
    assert q.usd == pytest.approx(2.5)  # 500 / USD_RUB_RATE 200


def test_quote_stars_and_premium_no_discounts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert q_first.rub_referral_discount == 0.0
    assert q_first.rub_final == 100.0
    ...
    assert qw.rub_wholesale_discount == 0.0
    assert qw.rub_final == qw.rub_list
    prem = catalog.Product(
        id="premium_3m", kind="premium", title="3m", emoji="💎", price_usd=12.0, duration_months=3
    )
    qp = quote_product(prem, s, is_first_order=True)
    assert qp.rub_referral_discount == 0.0
    assert qp.rub_final == qp.rub_list
...
def test_fiat_checkout_options_kb_urls_and_home() -> None:
    ...
    assert m.inline_keyboard[0][0].url == "https://lava.example/i"
    assert m.inline_keyboard[1][0].url == "https://bc.example/x"
