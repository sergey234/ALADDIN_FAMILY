...
Версия: 2026-05-22 · **политика spend 2026-07-14:** бонусный баланс **только VPN**.  
Продуктовый принцип: **мы не предлагаем людям «зарабатывать деньги»** рефералкой.  
Техника: `balance_rub` = основной; `ref_balance_rub` = бонусный (**нельзя** Stars/Premium).  
Канон внедрения: `PLAN_REFERRAL_BONUS_VPN_ONLY_2026-07-14.md` · TODO `REFERRAL_BONUS_VPN_ONLY_TODO_TRACKER.md` (`rb-*`).

Связано: `ML_SYSTEM_HANDOFF_FINAL.md` §17, `EDGE_CASES.md`.

> **Устарело ниже (до внедрения `rb-*` + `pf-*`):** формулировки «бонус … можно оплатить Stars / Premium / VPN»; реф-блок внутри профиля (§3.1).  
> **Канон профиля (после `pf-*`):** только ID / регистрация / балансы / VPN-статус; рефка — только «👥 Пригласить друга». См. `PLAN_PROFILE_REF_DEDUP_VPN_STATUS_2026-07-14.md`.
...
| начисляется X% на реф. баланс | **бонус X%** на бонусный баланс; трата **только VPN** | `marketing.py`, `hub.py` |
...
**Канон после `pf-*`:** рефералка **не** в профиле. Профиль = карточка пользователя:
...
💳 Основной баланс: … ₽
🎁 Бонусный баланс: … ₽
   Используется только для оплаты VPN.
🛡️ VPN:
   ✅ Статус: Активен / ❌ Не активен (+ осталось / действует до)
```

Реф-ссылка, условия, статистика, FAQ — **только** «👥 Пригласить друга» (`nav:ref`) и `nav:reffaq`.  
Кнопку «👤 Полный профиль» из `nav:ref` можно оставить (навигация, не дубль реф-логики).

**Spend (после `rb-*`):** бонусный баланс → **только VPN** (bonus-first); Stars/Premium — только основной баланс.

Кнопки профиля: VPN copy/sub (если есть), sell/заявки, «В меню» — **без** реф-кнопок / `nav:reffaq`.

---

### 3.2 FAQ → «Как работает приглашение» (`marketing.referral_faq_html`)

```text
Как работает приглашение друзей

• Поделитесь ссылкой из раздела «👥 Пригласить друга».
• На первый заказ VPN у друга может быть скидка {RB}% (если ещё не было выданных покупок).
• Stars и Premium — каталожная цена, без реферальных скидок покупателю.
• После первой выданной покупки друга на бонусный баланс начисляется {RC}%.
  Бонусный баланс можно потратить только на VPN (списывается первым).
  Stars и Premium — только с основного баланса.
