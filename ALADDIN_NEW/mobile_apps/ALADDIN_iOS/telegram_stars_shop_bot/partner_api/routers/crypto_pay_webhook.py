...
from bot.services.topup_crypto_payload import decode_topup_crypto_payload, is_topup_crypto_payload
from bot.services.topup_payment_webhook import mark_topup_paid_from_provider, verify_topup_crypto_payload_row
...
async def crypto_pay_webhook(
    ...
    payload_s = payload_raw.strip()

    if is_topup_crypto_payload(payload_s):
        try:
            topup_decoded = decode_topup_crypto_payload(payload_s)
        except ValueError as exc:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={"code": "validation_error", "message": str(exc)},
            ) from exc
        topup_id = topup_decoded.topup_id
        idem = f"cryptobot:topup:{invoice_id}"
        conn = await connect(settings.database_path)
        try:
            await conn.execute("BEGIN IMMEDIATE")
            try:
                await verify_topup_crypto_payload_row(conn, topup_decoded)
            except ValueError as exc:
                await conn.rollback()
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            ...
            res = await mark_topup_paid_from_provider(
                conn, settings, topup_id=topup_id, idempotency_key=idem
            )
            ...
            await conn.rollback()
            _log.exception("crypto_pay_webhook_topup_failed")
            raise HTTPException(
            ...
            await conn.close()
        return {"ok": True, "topup_id": topup_id, "status": "completed"}

    try:
        decoded = decode_crypto_invoice_payload(payload_s)
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail={"code": "validation_error", "message": str(exc)},
    ...
    try:
        ...
        await orders_repo.set_payment_rail(conn, order_id=order_id, payment_rail="crypto_bot")
