...
def _webhook_alert_status(status_code: int) -> bool:
    """401 на вебхуке без подписи — ожидаемый отказ (health/probe), не шлём ops-алерт."""
    if status_code == 401:
        return False
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if _is_webhook_path(request.url.path) and response.status_code >= 400:
            if response.status_code == 401:
                _log.info(
                    "partner_api_webhook_auth_rejected status=401 path=%s",
                    request.url.path,
                )
            else:
                _log.warning(
            ...
            if settings.alerts_enabled and _webhook_alert_status(response.status_code):
                asyncio.create_task(
                    send_alert(
                        settings=settings,
                        severity="warning",
