...
async def save_feedback(
    ...
    *,
    ...
    if k not in ("nps", "csat", "suggestion"):
...
async def has_recent_prompt(conn: aiosqlite.Connection, *, user_id: int, cooldown_days: int) -> bool:
    ...
    cur = await conn.execute(
        ...
        WHERE user_id = ?
          AND event_type IN ('feedback_prompt_sent', 'feedback_prompt_failed')
          AND date(created_at) >= date('now', ?)
        LIMIT 1
        """,
        (int(user_id), f"-{d} days"),
        ...
        WHERE o.status = 'completed'
          ...
          AND NOT EXISTS (
              ...
              WHERE ae.user_id = o.user_id
                AND ae.event_type IN ('feedback_prompt_sent', 'feedback_prompt_failed')
