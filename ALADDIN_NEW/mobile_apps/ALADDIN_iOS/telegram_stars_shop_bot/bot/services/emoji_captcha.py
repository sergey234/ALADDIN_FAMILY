...
WORD_BY_EMOJI: dict[str, str] = {e: w for e, w in _CAPTCHA_TARGETS}
for _e, _w in list(WORD_BY_EMOJI.items()):
    WORD_BY_EMOJI[captcha_repo.normalize_emoji(_e)] = _w
...
def captcha_caption_html(*, word: str, kind: str) -> str:
    ...
    return (
        ...
        "<i>Если кнопка не срабатывает — нажмите именно тот эмодзи, что в тексте.</i>"
    )


def captcha_keyboard(*, challenge_id: int, purpose_prefix: str, emojis: list[str]) -> InlineKeyboardMarkup:
    """purpose_prefix: «onb» или «chk» → callback {prefix}:c:{id}:{emoji_hex}.

    В callback кладём сам эмодзи (hex), не индекс ряда — иначе на RTL/клиентах
    визуальная кнопка «яблоко» может уйти с чужим индексом.
    """
    row = [
        InlineKeyboardButton(
            text=e,
            callback_data=f"{purpose_prefix}:c:{challenge_id}:{captcha_repo.emoji_token(e)}",
        )
        for e in emojis
    ]
...
async def send_onboarding_captcha_photo(
    ...
    *,
    ...
    if photo is not None and len(cap) <= CHANNEL_GATE_PHOTO_CAPTION_MAX:
        sent = await bot.send_photo(chat_id, photo, caption=cap, reply_markup=kb)
...
async def prompt_checkout_captcha(cb: CallbackQuery, conn, settings: Settings) -> None:
    ...
    photo = branding_media.hero_photo_input(settings)
    if photo is not None and len(cap) <= CHANNEL_GATE_PHOTO_CAPTION_MAX:
        sent = await cb.message.answer_photo(photo, caption=cap, reply_markup=kb)
        branding_media.remember_hero_file_id_from_message(sent)
    else:
