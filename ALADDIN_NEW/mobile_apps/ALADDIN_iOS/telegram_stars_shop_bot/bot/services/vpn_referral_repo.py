...
async def reassign_referral_owner_on_telegram_link(
    ...
    *,
    ...
    """
    After web→Telegram link: keep one referral identity.

    Prefer existing Telegram code (bot SSOT). If only web synthetic had a code,
    move it to the Telegram user so site and bot show the same link/stats.
    Also move grant counters keyed by synthetic id.
    """
    from_id = int(from_subject_id)
    to_id = int(to_telegram_user_id)
    if from_id == 0 or to_id == 0 or from_id == to_id:
        return None

    cur = await conn.execute(
        "SELECT code FROM vpn_referral_codes WHERE user_id = ?", (to_id,)
    )
    to_row = await cur.fetchone()
    cur = await conn.execute(
        "SELECT code FROM vpn_referral_codes WHERE user_id = ?", (from_id,)
    )
    from_row = await cur.fetchone()

    kept: str | None = None
    if to_row and to_row["code"]:
        kept = str(to_row["code"])
        if from_row and from_row["code"] and str(from_row["code"]) != kept:
            # Drop orphan web code — Telegram code wins.
            await conn.execute(
                "DELETE FROM vpn_referral_codes WHERE user_id = ?", (from_id,)
            )
    elif from_row and from_row["code"]:
        await conn.execute(
            "UPDATE vpn_referral_codes SET user_id = ? WHERE user_id = ?",
            (to_id, from_id),
        )
        kept = str(from_row["code"])

    await conn.execute(
        """
        UPDATE vpn_referral_grants
    ...
    await conn.execute(
        """
        UPDATE accounts
        SET referrer_telegram_id = ?
        WHERE referrer_telegram_id = ?
        """,
        (to_id, from_id),
    )
    await conn.execute(
        "UPDATE orders SET referrer_id = ? WHERE referrer_id = ?",
        (to_id, from_id),
    )
    await conn.commit()
    return kept
...
async def user_vpn_referral_stats(conn: aiosqlite.Connection, user_id: int) -> dict[str, Any]:
    ...
    try:
        cur2 = await conn.execute(
            ...
            WHERE referrer_telegram_id = ? AND merged_into_account_id IS NULL
            """,
            (int(user_id),),
        )
        row2 = await cur2.fetchone()
        invited = int(row2["c"] or 0) if row2 else 0
    except Exception:
        invited = 0
    return {
        "vpn_referral_buyers": int(row["vpn_buyers"] or 0) if row else 0,
        "vpn_referral_days_earned": int(row["days_earned"] or 0) if row else 0,
        "vpn_referral_invited": invited,
    }
...
async def try_insert_vpn_referral_grant(
    ...
    """Один бонус на приглашённого за первую выданную VPN-покупку (grant_kind=paid)."""
    ...
    try:
        await conn.execute(
            ...
            INSERT INTO vpn_referral_grants (
                referred_user_id, referrer_user_id, order_id, friend_days, referrer_days, grant_kind
            ) VALUES (?, ?, ?, ?, ?, 'paid')
            """,
...
async def try_insert_vpn_trial_referral_grant(
    conn: aiosqlite.Connection,
    *,
    ...
    """
    Бонус за первую активацию trial по ref_.
    Друг уже получил trial (часы из VPN_TRIAL_HOURS) — friend_days в grant = 0 (без второго extend).
    Пригласившему — VPN_TRIAL_REFERRAL_REFERRER_DAYS (+1).
    """
    from bot.services.vpn_trial_service import trial_order_id

    ref_d = int(settings.vpn_trial_referral_referrer_days or 0)
    if ref_d <= 0:
        return None
    cur = await conn.execute(
    ...
    if referrer_id == user_id:
        return None
    order_id = trial_order_id(user_id)
    try:
        await conn.execute(
            """
            INSERT INTO vpn_referral_grants (
            ...
            """,
            (user_id, referrer_id, order_id, ref_d),
        )
        await conn.commit()
    except aiosqlite.IntegrityError:
        await conn.rollback()
