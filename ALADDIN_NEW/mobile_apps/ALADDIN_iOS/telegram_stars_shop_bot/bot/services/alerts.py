...
async def _send_telegram(settings: Settings, message: str) -> bool:
    from bot.services.ops_chat import send_ops_chat_plain

    return await send_ops_chat_plain(settings, message)
...
async def send_alert(
    ...
    *,
    ...
    cooldown_seconds — опционально свой интервал (например 8ч для low TON).
    """
    ...
    if settings.admin_bot_alerts_enabled:
        ...
        html = format_admin_bot_alert_html(severity=severity, title=title, body=body)
        tg_ok = await send_ops_chat_html(settings, html, dedupe_key=key)
    else:
        tg_ok = await _send_telegram(settings, msg)
    pd_ok = await _send_pagerduty(settings, severity=severity, title=title, body=body, dedupe_key=key)
