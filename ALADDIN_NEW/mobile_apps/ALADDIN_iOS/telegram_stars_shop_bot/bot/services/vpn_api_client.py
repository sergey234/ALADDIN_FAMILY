  ...
  """
    ...
    for attempt in range(max_retries + 1):
        ...
        if r.status_code in (200, 202):
            ...
            # Full body on success — truncation breaks json.loads for devices/list etc.
            return True, r, r.text
...
async def get_public_health(settings: Settings) -> tuple[bool, str]:
    """GET /health без HMAC (liveness vpn-api). Несколько попыток — переживает краткий restart."""
    base = (settings.vpn_api_base_url or "").strip().rstrip("/")
    if not base:
        return False, "VPN_API_BASE_URL not set"
    retries = max(1, min(int(getattr(settings, "vpn_ops_health_probe_retries", 3) or 3), 5))
    delay = max(0.2, float(getattr(settings, "vpn_ops_health_probe_delay_seconds", 1.0) or 1.0))
    last_err = "unknown error"
    url = f"{base}/health"
...
async def get_public_ready(settings: Settings) -> tuple[bool, dict[str, str] | str, int]:
    """GET /ready — WireGuard на ноде. Код 0 = transport error, -1 = не вызывали (нет base URL)."""
    base = (settings.vpn_api_base_url or "").strip().rstrip("/")
    if not base:
        return False, "VPN_API_BASE_URL not set", -1
    retries = max(1, min(int(getattr(settings, "vpn_ops_health_probe_retries", 3) or 3), 5))
    delay = max(0.2, float(getattr(settings, "vpn_ops_health_probe_delay_seconds", 1.0) or 1.0))
    last_err = "unknown error"
    url = f"{base}/ready"
    for attempt in range(retries):
        try:
            ...
            if r.status_code < 500:
                return False, last_err, r.status_code
        except httpx.HTTPError as e:
            last_err = str(e)[:200]
        if attempt + 1 < retries:
            await asyncio.sleep(delay)
...
async def post_provision(
    ...
    *,
    ...
    if shop_account_id:
        body_obj["shop_account_id"] = str(shop_account_id).strip()
...
async def post_provision_trial(
    settings: Settings,
    *,
    telegram_user_id: int,
    idempotency_key: str,
    ...
    path = "/internal/v1/provision_trial"
    body_obj: dict[str, Any] = {"telegram_user_id": int(telegram_user_id)}
    if trial_hours is not None and int(trial_hours) > 0:
        body_obj["trial_hours"] = int(trial_hours)
    body = json.dumps(body_obj, separators=(",", ":")).encode("utf-8")
    ok, _r, msg = await _request_vpn_api(
        settings,
        method="POST",
...
async def post_devices_list(
    settings: Settings,
    *,
    telegram_user_id: int,
) -> tuple[bool, dict[str, Any] | str]:
    path = "/internal/v1/devices/list"
    body = json.dumps(
        {"telegram_user_id": int(telegram_user_id)},
        separators=(",", ":"),
    ).encode("utf-8")
    ok, resp, msg = await _request_vpn_api(
        settings, method="POST", path=path, body=body, timeout=20.0
    )
    if not ok:
        return False, msg
    try:
...
async def post_devices_create(
    ...
    *,
    ...
    path = "/internal/v1/devices/create"
    body_obj: dict[str, Any] = {"telegram_user_id": int(telegram_user_id)}
    if display_name:
        body_obj["display_name"] = display_name
    if purchase_id:
        body_obj["purchase_id"] = purchase_id
    body = json.dumps(body_obj, separators=(",", ":")).encode("utf-8")
    ok, _r, msg = await _request_vpn_api(
        settings,
        method="POST",
...
async def post_devices_rename(
    ...
    *,
    ...
    path = "/internal/v1/devices/rename"
    body = json.dumps(
        {
            "telegram_user_id": int(telegram_user_id),
    ...
    ok, _r, msg = await _request_vpn_api(
        settings, method="POST", path=path, body=body, timeout=15.0
    )
    return ok, msg


async def post_devices_revoke(
    settings: Settings,
    *,
    telegram_user_id: int,
    device_id: int,
    ...
    path = "/internal/v1/devices/revoke"
    body = json.dumps(
        {"telegram_user_id": int(telegram_user_id), "device_id": int(device_id)},
        separators=(",", ":"),
    ).encode("utf-8")
    ok, _r, msg = await _request_vpn_api(
        settings,
...
async def post_devices_purchase(
    settings: Settings,
    *,
    telegram_user_id: int,
    purchase_id: str,
    ...
    path = "/internal/v1/devices/purchase"
    body_obj: dict[str, Any] = {
        "telegram_user_id": int(telegram_user_id),
        "purchase_id": str(purchase_id)[:128],
        "create_device": bool(create_device),
    }
    if display_name:
        body_obj["display_name"] = display_name
    body = json.dumps(body_obj, separators=(",", ":")).encode("utf-8")
...
async def post_bind_telegram(
    ...
    *,
    ...
    path = "/internal/v1/bind_telegram"
    body_obj: dict[str, Any] = {
        "shop_account_id": str(shop_account_id).strip(),
        "from_telegram_user_id": int(from_telegram_user_id),
        "to_telegram_user_id": int(to_telegram_user_id),
    }
    body = json.dumps(body_obj, separators=(",", ":")).encode("utf-8")
    ok, _r, msg = await _request_vpn_api(
        settings,
...
async def get_status_announce(settings: Settings) -> tuple[bool, str]:
    path = "/internal/v1/status-announce"
    ok, resp, _msg = await _request_vpn_api(
        settings,
        method="GET",
        path=path,
    ...
    if not data.get("active"):
        return True, ""
    return True, str(data.get("text") or "").strip()


async def post_status_announce(settings: Settings, *, text: str) -> tuple[bool, str]:
    path = "/internal/v1/status-announce"
    body = json.dumps({"text": (text or "").strip()}, ensure_ascii=False).encode("utf-8")
    ok, resp, msg = await _request_vpn_api(
        settings,
        method="POST",
        path=path,
    ...
    if resp.status_code != 200:
        return False, (resp.text or msg)[:500]
    return True, "ok"


async def clear_status_announce(settings: Settings) -> tuple[bool, str]:
    path = "/internal/v1/status-announce"
    ok, resp, msg = await _request_vpn_api(
        settings,
        method="DELETE",
