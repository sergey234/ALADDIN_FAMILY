...
async def fetch_vpn_account_user_facing(
    ...
    async with aiosqlite.connect(vpn_db_path) as db:
        ...
        cols_cur = await db.execute("PRAGMA table_info(vpn_accounts)")
        col_names = {str(r[1]) for r in await cols_cur.fetchall()}
        wanted = [
            "status",
            "paid_until",
            "opaque_token",
        ...
        select_cols = [c for c in wanted if c in col_names]
        if not select_cols:
            return None
        cur = await db.execute(
            f"SELECT {', '.join(select_cols)} FROM vpn_accounts WHERE telegram_user_id = ?",
            (telegram_user_id,),
        )
        ...
        return {
            ...
            "trial_used_at": str(d.get("trial_used_at") or ""),
            "account_kind": str(d.get("account_kind") or "paid"),
            "device_limit": str(d.get("device_limit") or ""),
        }
    ...
    async with aiosqlite.connect(vpn_db_path) as db:
        ...
        cur = await db.execute(
            ...
            SELECT id, telegram_user_id, status, paid_until, opaque_token,
                   wg_client_tunnel_ip, trial_used_at, account_kind, device_limit,
        ...
        curd = await db.execute(
            """
            SELECT substr(hwid_hash, 1, 12) AS hwid_prefix, bound_at, last_seen_at
            FROM vpn_device_bindings
            WHERE telegram_user_id = ?
            """,
            (telegram_user_id,),
        )
        dev = await curd.fetchone()
        row["device_binding"] = dict(dev) if dev else None
        return row


async def fetch_vpn_trial_admin_snapshot(
    vpn_db_path: Path,
    telegram_user_id: int,
) -> dict[str, Any] | None:
    snap = await fetch_vpn_account_admin_snapshot(vpn_db_path, telegram_user_id)
    if snap is None or snap.get("missing_account"):
        return snap
    return snap


def format_vpn_trial_status_html(d: dict[str, Any]) -> str:
    from bot.util_html import esc

    if d.get("missing_account"):
        tid = int(d["telegram_user_id"])
        return f"<b>Trial</b>: для <code>{esc(str(tid))}</code> записи в vpn.db нет."
    dev = d.get("device_binding") or {}
    hwid_prefix = str(dev.get("hwid_prefix") or "")
    lines = [
        "<b>VPN trial / device</b>\n",
        f"<code>telegram_user_id</code>: {esc(str(d.get('telegram_user_id')))}",
        f"<code>status</code>: {esc(str(d.get('status') or ''))}",
        f"<code>account_kind</code>: {esc(str(d.get('account_kind') or ''))}",
        f"<code>trial_used_at</code>: {esc(str(d.get('trial_used_at') or ''))}",
        f"<code>paid_until</code>: {esc(str(d.get('paid_until') or ''))}",
        f"<code>device_limit</code>: {esc(str(d.get('device_limit') or ''))}",
        f"<code>hwid_hash</code> (prefix): {esc(hwid_prefix) + '…' if hwid_prefix else '—'}",
        f"<code>device bound_at</code>: {esc(str(dev.get('bound_at') or ''))}",
        f"<code>device last_seen</code>: {esc(str(dev.get('last_seen_at') or ''))}",
    ]
    return "\n".join(lines)
...
def format_vpn_admin_snapshot_html(d: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"<code>telegram_user_id</code>: {esc(str(d.get('telegram_user_id')))}",
        f"<code>status</code>: {esc(str(d.get('status') or ''))}",
        f"<code>account_kind</code>: {esc(str(d.get('account_kind') or 'paid'))}",
        f"<code>trial_used_at</code>: {esc(str(d.get('trial_used_at') or ''))}",
        f"<code>paid_until</code>: {esc(str(d.get('paid_until') or ''))}",
