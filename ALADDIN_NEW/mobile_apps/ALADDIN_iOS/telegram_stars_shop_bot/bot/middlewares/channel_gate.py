...
def _callback_requires_channel(data: str | None) -> bool:
    ...
    if data == "order:submit":
        return True
    prefixes = ("buy:", "prem:", "pay:", "nav:buy_stars", "nav:premium", "nav:gifts", "nav:vpn")
...
class ChannelGateMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if ok:
            return await handler(event, data)
        await event.answer(
            "Чтобы покупать Stars / Premium / VPN / подарки, подпишитесь на канал магазина (кнопка на первом экране).",
            show_alert=True,
        )
        return None
