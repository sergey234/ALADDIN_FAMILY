                ...
                case .failure(let error):
                    ...
                    if let apiError = error as? APIError {
                        ...
                        case .networkError(let msg):
                            detailedErrorMessage = self.localized("payment_qr_error_network_format", msg)
                        case .serverError(let msg):
                            detailedErrorMessage = self.localized("payment_qr_error_server_with_message_format", msg)
                        case .invalidURL:
                            detailedErrorMessage = self.localized("payment_qr_error_config")
                        default:
                            detailedErrorMessage = self.localized("payment_qr_error_generic_format", error.localizedDescription)
                        }
                    } else {
                        detailedErrorMessage = self.localized("payment_qr_error_create_format", error.localizedDescription)
                    }
                    ...
                    if let apiError = error as? APIError {
                        ...
                        case .httpError(let statusCode, _):
                            if statusCode == 404 {
                                ...
                                detailedErrorMessage = self.localized("payment_qr_error_server_http_format", statusCode)
                            }
                        case .invalidURL:
                            detailedErrorMessage = self.localized("payment_qr_error_config")
                        default:
                            detailedErrorMessage = self.localized("payment_qr_error_check_format", error.localizedDescription)
                        }
                    } else {
                        detailedErrorMessage = self.localized("payment_qr_error_check_format", error.localizedDescription)
                    }
