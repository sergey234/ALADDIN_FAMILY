...
async def rate_limit_middleware(request, call_next):
    if hasattr(request.state, 'user_id'):
        ...
        level = getattr(request.state, 'subscription_level', 'free')
        allowed, remaining, retry_after = rate_limiter.is_allowed(user_id, level)
        if not allowed:
...
async def update_protection_settings(settings: dict):
    if SFM_ADAPTER_AVAILABLE and sfm_adapter:
...
async def start_protection_scan():
    if SFM_ADAPTER_AVAILABLE and sfm_adapter:
