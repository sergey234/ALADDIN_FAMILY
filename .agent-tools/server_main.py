...
# ✅ Точечный фикс для прод-API: если backend защиты отдал mock/fallback/error
# по GET /api/user/profile, возвращаем 503 вместо HTTP 200 с mock.
class UserProfileSfmMockTo503Middleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        response = await call_next(request)

        try:
            # Диагностируем только целевой endpoint, не меняя статус ответа.
            if request.method != "GET":
                return response
            if request.url.path != "/api/user/profile":
                return response

            # BaseHTTPMiddleware часто не заполняет response.body, поэтому
            # буферизуем body через response.body_iterator и возвращаем новый Response.
            body = b""
            try:
            ...
            # Важно: middleware не должен ломать обычные ответы.
            return response

        return response

app.add_middleware(UserProfileSfmMockTo503Middleware)
