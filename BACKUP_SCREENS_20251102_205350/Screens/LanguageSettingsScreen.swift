...
struct LanguageSettingsScreen: View {
    ...
    // Сохраняем выбранный язык в AppStorage
    @AppStorage("selected_language") private var selectedLanguageRaw: String = "ru"
    var selectedLanguage: Language {
        get {
            Language(rawValue: selectedLanguageRaw) ?? .russian
        }
        nonmutating set {
    ...
    private var autoLanguageSection: some View {
        VStack(spacing: Spacing.m) {
            ...
            HStack(spacing: Spacing.m) {
                ...
                ALADDINToggle(isOn: $isSystemLanguage, size: 40)
            }
            .padding(Spacing.m)
            .background(
