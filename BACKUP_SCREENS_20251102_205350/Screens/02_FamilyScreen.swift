...
struct FamilyScreen: View {
    ...
    private func loadFamilyMembers() {
        // Здесь можно загружать из UserDefaults или API
    ...
    var body: some View {
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 20) {
                            ...
                            // Динамическая сетка карточек (до 10 участников, со скроллом если больше 6)
                            ...
                            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 2), spacing: 8) {
                                ...
                                // Если участников больше 6, используем ScrollView
                                ScrollView {
                                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 2), spacing: 8) {
                                        ForEach(familyMembers) { member in
                                FamilyMemberCard(
                                ...
                                .frame(maxHeight: 400) // Максимальная высота для скролла
                            }
                        }
                        .padding(25)
                        .background(Color.secondaryGold.opacity(0.15))
        ...
        .onReceive(NotificationCenter.default.publisher(for: UserDefaults.didChangeNotification)) { _ in
            // Обновляем баланс при изменении UserDefaults
            unicornBalance = UserDefaults.standard.integer(forKey: "child_unicorn_balance")
        }
    }
}
...
struct FamilyLocationModal: View {
    ...
    private func loadLocationStatistics() {
        ...
        if let stats = UserDefaults.standard.dictionary(forKey: statsKey) {
            ...
            geofencesList = stats["geofencesList"] as? String ?? "Дом, Школа"
        }
...
struct FamilyParentalControlSettingsModal: View {
    ...
    // Выбор ребёнка (3 ребёнка) с сохранением в UserDefaults
    @AppStorage("parental_selected_child") private var selectedChild: String = "Маша"
