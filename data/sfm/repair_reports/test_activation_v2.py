#!/usr/bin/env python3
"""
Скрипт для тестовой активации критических функций (версия 2)
Активирует спящие функции по 10 штук за раз
"""
...
def get_sleeping_functions(data):
    """Получает спящие функции (исключая web и портовые)"""
    ...
    for func_id, func_data in functions.items():
        ...
        if any(exclude in name.lower() for exclude in ['web', 'interface', 'server', 'api', 'port']):
            continue
        if any(exclude in func_type.lower() for exclude in ['web', 'interface', 'server', 'api']):
            continue
        if any(exclude in file_path.lower() for exclude in ['web', 'interface', 'server', 'api']):
            continue
            
        # Ищем спящие функции
        if status == 'sleeping':
            sleeping.append((func_id, func_data))
...
def activate_functions(functions_to_activate):
    ...
    for func_id, func_data in functions_to_activate:
        if func_id in functions:
            ...
            activated.append((func_id, func_data))
...
def test_activation(batch_size=10):
    ...
    print(f"🚀 ТЕСТОВАЯ АКТИВАЦИЯ СПЯЩИХ ФУНКЦИЙ")
    print(f"================================================")
    
    # Загружаем реестр
    data = load_registry()
    sleeping_functions = get_sleeping_functions(data)
    
    print(f"Всего спящих функций (без web/портов): {len(sleeping_functions)}")
    print(f"Размер партии: {batch_size}")
    print("")
    
    if len(sleeping_functions) == 0:
        print("❌ Нет подходящих функций для активации!")
        return [], {}, {}
    ...
    # Активируем функции
    functions_to_activate = sleeping_functions[:batch_size]
    activated = activate_functions(functions_to_activate)
    
    print(f"✅ АКТИВИРОВАНО {len(activated)} ФУНКЦИЙ:")
    for func_id, func_data in activated:
        print(f"   • {func_data.get('name', func_id)} ({func_id})")
    ...
    report = {
        ...
        'activated_functions': [f[0] for f in activated],
