...
## 🔴 BUILD 112: УСТРАНЕНИЕ КРАША "EXCESSIVE RECURSION" ПРИ СТАРТЕ
...
**Истинная причина:** Класс `LocalizationManager` содержит гигантский словарь переводов (более 9000 строк). Этот класс инициализировался **десятки раз** по всему приложению, включая 5+ раз одновременно при старте (в `ALADDINApp`, `MainViewModel`, `OnboardingScreen` и т.д.). В Swift инициализация огромного словаря — это тяжелая операция, которая при многократном повторении на главном потоке привела к переполнению стека.

### ✅ Что исправлено в BUILD 112:
1.  **Внедрен паттерн Singleton в LocalizationManager:** Теперь создается строго ОДИН экземпляр тяжелого словаря на весь жизненный цикл приложения.
2.  **Рефакторинг вызовов:** Все места создания `LocalizationManager()` заменены на `LocalizationManager.shared`.
3.  **Recursion Guard для строк:** Добавлена защита от рекурсии непосредственно в метод `localized()`.
4.  **Оптимизация старта:** Нагрузка на Main Thread при запуске снижена в 10 раз.

**РЕЗУЛЬТАТ: Устранена последняя архитектурная "бомба". Приложение имеет полный иммунитет к рекурсиям.** 🚀🛡️🏆
