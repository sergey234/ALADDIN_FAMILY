...
struct RoleSelectionModal: View {
    ...
    private func roleName(for role: FamilyRole) -> String {
        switch role {
        case .parent: return "Родители"
        case .child: return "Дети"
        case .grandparent: return "Люди 60+"
        }
    }
    ...
    private func description(for role: FamilyRole) -> String {
        ...
        case .grandparent: return "Люди старшего возраста"
        }
    }
        ...
        Button(action: onTap) {
            HStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(roleName(for: role))
                        .font(.system(size: 16, weight: .semibold))
                        .foregroundColor(.primary)
                    
                    if !description(for: role).isEmpty {
