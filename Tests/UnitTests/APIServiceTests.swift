...
final class APIServiceTests: XCTestCase {
    ...
    func testGetVPNStatusReturnsStubbedResponse() {
        let expected = VPNStatusResponse(
            isConnected: true,
        ...
        mockNetwork.stubGET(endpoint: AppConfig.Endpoint.vpnStatus, response: expected)
        let expectation = expectation(description: "VPN status response")
        
        service.getVPNStatus { result in
            switch result {
        ...
        XCTAssertEqual(mockNetwork.lastGETEndpoint, AppConfig.Endpoint.vpnStatus)
    }
    
    func testConnectVPNReturnsStubbedResponse() {
        let expected = APIResponse(success: true, data: true, message: "connected", error: nil)
        mockNetwork.stubPOST(endpoint: AppConfig.Endpoint.vpnConnect, response: expected)
        let expectation = expectation(description: "VPN connect response")
        
        service.connectVPN { result in
            switch result {
            case .success(let response):
        ...
        wait(for: [expectation], timeout: 1.0)
        XCTAssertEqual(mockNetwork.lastPOSTEndpoint, AppConfig.Endpoint.vpnConnect)
    }
    ...
    func testRegisterDeviceTokenUsesCorrectEndpoint() {
        ...
        service.registerDeviceToken("test-token-123") { result in
            switch result {
            case .success(let response):
                XCTAssertTrue(response.success)
                expectation.fulfill()
