...
# Precision: /api/reports/* — те же handlers, что в main.py (БД), без отдельных заглушек SFM.
_gateway_root = os.path.dirname(os.path.abspath(__file__))
if _gateway_root not in sys.path:
    sys.path.insert(0, _gateway_root)
try:
    from security.api.routers.reports_router import router as _reports_precision_router

    app.include_router(_reports_precision_router)
    print("✅ Precision: reports_router mounted at /api/reports/*")
except Exception as _reports_exc:
    print(f"⚠️ reports_router not mounted: {_reports_exc}")
...
async def get_phishing_sensitivity():
    # ✅ ИСПРАВЛЕНА - функция 1/93
    # Заменено hardcoded/mock на реальный SFM вызов
    if SFM_ADAPTER_AVAILABLE and sfm_adapter:
        success, result, message = sfm_adapter.execute_function("get_phishing_protection_config", {})
        if success:
            return result
        else:
