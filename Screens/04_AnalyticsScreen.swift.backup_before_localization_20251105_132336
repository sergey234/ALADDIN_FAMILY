...
struct AnalyticsScreen: View {
    ...
    private func periodTitle(_ period: AnalyticsViewModel.TimePeriod) -> String {
        switch period { case .day: return "День"; case .week: return "Неделя"; case .month: return "Месяц" }
    }
    ...
    private var usageTotalTraffic: some View {
        HStack {
            Text("📊 Всего трафика:")
                ...
                .foregroundColor(.textPrimary)
            Spacer()
            Text(viewModel.usageAnalytics?.totalTraffic ?? "Загрузка...")
                .font(.bodyBold)
                .foregroundColor(.primaryBlue)
        }
    }
        ...
        VStack(alignment: .leading, spacing: Spacing.m) {
            ...
            if let status = viewModel.devicesAnalytics?.status {
                HStack {
                    ...
                    Spacer()
                    Text("\(status.online) устройств")
                }
                HStack {
                    Text("⭕ Офлайн:")
                        .foregroundColor(.textSecondary)
                    Spacer()
                    Text("\(status.offline) устройств")
                }
                HStack {
                    Text("🛡️ Защита:")
                        .foregroundColor(.primaryBlue)
