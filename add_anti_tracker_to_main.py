...
# -*- coding: utf-8 -*-
"""
Скрипт для добавления anti_tracker_router в main.py

Использование:
    python3 add_anti_tracker_to_main.py
...
BACKUP_PATH = MAIN_PY_PATH.with_suffix(f".py.backup_anti_tracker_{datetime.now().strftime('%Y%m%d_%H%M%S')}")

# Импорт и регистрация
IMPORT_LINE = "from security.api.routers.anti_tracker_router import router as anti_tracker_router"
ROUTER_LINE = "app.include_router(anti_tracker_router)"


def main():
    print("=" * 60)
    print("ДОБАВЛЕНИЕ ANTI-TRACKER ROUTER В MAIN.PY")
    print("=" * 60)
    ...
    if IMPORT_LINE in content or "anti_tracker_router" in content:
        print("\n⚠️ Импорт anti_tracker_router уже существует")
    else:
        # Поиск места для импорта (после других router импортов)
        import_patterns = [
            "from security.api.routers.driving_reports_router import",
    ...
    if ROUTER_LINE in content or "anti_tracker_router" in content:
        print("\n⚠️ Регистрация anti_tracker_router уже существует")
    else:
        ...
        print("✅ Регистрация router добавлена: " + ROUTER_LINE)
