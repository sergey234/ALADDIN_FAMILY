#!/bin/bash
# 🚀 СКРИПТ РАЗВЕРТЫВАНИЯ ФУНКЦИИ 3/93: /api/components/status/{component_id}
# Выполнить после исправления функции в api_gateway_server_current.py

echo "🔧 НАЧИНАЕМ РАЗВЕРТЫВАНИЕ ФУНКЦИИ 3/93"
...
sshpass -p 'Sergio675' ssh -o StrictHostKeyChecking=no root@149.154.65.180 \
    "cd /opt/aladdin-backend && cp api_gateway.py api_gateway_backup_3_\$(date +%Y%m%d_%H%M%S).py && echo '✅ Backup создан'"
...
echo "🧪 Тестирование функции /api/components/status/{component_id}..."
sshpass -p 'Sergio675' ssh -o StrictHostKeyChecking=no root@149.154.65.180 \
    "curl -s http://127.0.0.1:8002/api/components/status/crash_detection_agent | python3 -m json.tool"
...
echo ""
echo "📊 РЕЗУЛЬТАТЫ:"
echo "- Функция 3/93 исправлена: /api/components/status/{component_id}"
echo "- Теперь возвращает реальные данные из SFM вместо mock"
echo "- Следующая функция: 4/93 - /api/components/enable/{component_id}"