...
# -*- coding: utf-8 -*-
#
# 🛡️ ДЕПЛОЙ ANTI-TRACKER AGENT НА СЕРВЕР
#
# Автоматический деплой Anti-Tracker Agent на сервер
...
# Пути
LOCAL_AGENT="security/ai_agents/anti_tracker_agent.py"
LOCAL_ROUTER="security/api/routers/anti_tracker_router.py"
LOCAL_REGISTRY="security/ai_agents/function_registry_entry_anti_tracker.json"
LOCAL_REGISTER_SCRIPT="register_anti_tracker_in_sfm.py"
LOCAL_MAIN_SCRIPT="add_anti_tracker_to_main.py"
...
echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}🛡️ ДЕПЛОЙ ANTI-TRACKER AGENT НА СЕРВЕР${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
echo ""
...
expect << EOF
set timeout 60
spawn ssh ${SERVER_USER}@${SERVER} "cd ${REMOTE_TMP} && python3 register_anti_tracker_in_sfm.py"
expect {
    "password:" {
        send "${SERVER_PASS}\r"
        exp_continue
...
expect << EOF
set timeout 60
spawn ssh ${SERVER_USER}@${SERVER} "cd ${REMOTE_TMP} && python3 add_anti_tracker_to_main.py"
expect {
    "password:" {
        send "${SERVER_PASS}\r"
        exp_continue
...
expect << EOF
set timeout 30
spawn ssh ${SERVER_USER}@${SERVER} "cd ${REMOTE_BACKEND} && python3 -c 'from security.ai_agents.anti_tracker_agent import AntiTrackerAgent; print(\"✅ Импорт агента успешен\")'"
expect {
    "password:" {
        send "${SERVER_PASS}\r"
        exp_continue
...
expect << EOF
set timeout 30
spawn ssh ${SERVER_USER}@${SERVER} "cd ${REMOTE_BACKEND} && python3 -c 'from security.api.routers.anti_tracker_router import router; print(\"✅ Импорт router успешен\")'"
expect {
    "password:" {
        send "${SERVER_PASS}\r"
        exp_continue
...
cat > /tmp/check_sfm_stats_anti_tracker.py << 'PYEOF'
import json
import sys
...
expect << EOF
set timeout 30
spawn scp /tmp/check_sfm_stats_anti_tracker.py ${SERVER_USER}@${SERVER}:${REMOTE_TMP}/
expect {
    "password:" {
        send "${SERVER_PASS}\r"
        exp_continue
...
expect << EOF
set timeout 30
spawn ssh ${SERVER_USER}@${SERVER} "cd ${REMOTE_TMP} && python3 check_sfm_stats_anti_tracker.py"
expect {
    "password:" {
        send "${SERVER_PASS}\r"
        exp_continue
...
rm -f /tmp/check_sfm_stats_anti_tracker.py
...
echo "2. Проверьте health endpoint: curl http://localhost:8000/api/anti-tracker/health"
echo "3. Перезапустите сервис (если нужно): systemctl restart aladdin-backend"
echo ""
echo -e "${GREEN}🛡️ Anti-Tracker Agent готов к использованию!${NC}"
