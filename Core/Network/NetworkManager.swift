...
class NetworkManager: NSObject, ObservableObject {
    ...
    func get<T: Decodable>(
        ...
        Task {
            ...
            performRequest(request: request, requiresAuth: false, onHeaders: onHeaders, completion: completion)
        }
    }
    ...
    private func performRequest<T: Decodable>(
        ...
        session.dataTask(with: request) { [weak self] data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                // Передаём заголовки ответа наверх (если запрошено) — только для первого ответа (не retry)
                if !isRetry {
                    onHeaders?(httpResponse.allHeaderFields)
                }
