...
class SecurityRule:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_triggered": (
                self.last_triggered.isoformat() if self.last_triggered is not None else None),
...
class AdvancedSecurityBase(SecurityBase):
    ...
    def _execute_rule_actions_dict(
        ...
        for action in actions:
            try:
                if action == "log":
                    self.log_activity(
                        f"Правило {rule.get('name', 'Unknown')} сработало для события {event.id}")
                elif action == "alert":
    ...
    def _execute_rule_actions(self, rule: SecurityRule, event: SecurityEvent):
        ...
        for action in rule.actions:
            try:
                if action == "log":
                    self.log_activity(
                        f"Правило {rule.name} сработало для события {event.id}")
                elif action == "alert":
    ...
    def _send_alert_dict(self, rule: Dict[str, Any], event: SecurityEvent):
        ...
        alert_message = f"Сработало правило {rule.get('name', 'Unknown')} для события {event.id}"
    ...
    def get_security_report(self) -> Dict[str, Any]:
        ...
        additional_info = {
            "security_events_count": len(
                ...
                self.security_rules), "active_events": len(
                [
                    e for e in self.security_events if not e.resolved]), "encryption_keys_count": len(
                        self.encryption_keys), "blocked_sources_count": len(
                            [
                                s for s in self.access_control_list.values() if s.get(
                                    "blocked", False)]), "audit_trail_entries": len(
                                        self.audit_trail), "threat_intelligence_sources": len(
    ...
    def initialize(self) -> bool:
        ...
        try:
            self.log_activity(
                f"Расширенная инициализация компонента безопасности {self.name}")
