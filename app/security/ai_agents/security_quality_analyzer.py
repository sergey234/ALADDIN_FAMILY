...
class SecurityQualityAnalyzer:
    """Анализатор качества системы безопасности ALADDIN"""

    def __init__(self, config: Dict[str, Any] = None):
        """Инициализация анализатора качества"""
        self.config = config or {}
        self.max_line_length = self.config.get('max_line_length', MAX_LINE_LENGTH)
        self.flake8_safe_codes = self.config.get('flake8_safe_codes', FLAKE8_SAFE_CODES)
        self.security_keywords = self.config.get('security_keywords', SECURITY_KEYWORDS)
        self.analysis_results = {}
        self.start_time = None

    def analyze_security_components(self, components: List[str]) -> Dict[str, Any]:
        """Анализ компонентов системы безопасности"""
        ...
        for component in components:
            try:
                score = self._analyze_component(component)
                results['quality_scores'][component] = score
                results['analyzed_components'] += 1
            except Exception as e:
                results['quality_scores'][component] = {'error': str(e)}

        results['analysis_time'] = time.time() - self.start_time
        return results

    def _analyze_component(self, component_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'quality_rating': self._get_quality_rating(security_score)
            }
    ...
    def _calculate_security_score(self, content: str) -> float:
        ...
        # Проверка наличия ключевых слов безопасности
        for keyword in self.security_keywords:
            if keyword in content_lower:
                score += 1.0

        # Нормализация оценки
        max_possible_score = len(self.security_keywords)
...
def run_command(cmd: List[str]) -> Tuple[int, str, str]:
    """Выполняет команду в терминале и возвращает код выхода,
    stdout, stderr."""
    try:
...
def is_security_file(file_path: str) -> bool:
    ...
    if any(
        exclude in file_path.lower()
        for exclude in ["backup", "script", "test", "temp", "old"]
    ):
        return False
...
def get_flake8_suggestion(error: Dict[str, Any]) -> str:
    ...
    if code == "E501":
        return (f"Строка {line} слишком длинная. Разбейте на несколько "
                f"строк используя \\ или скобки")
    elif code == "E302":
        ...
        return ("Файл не заканчивается переводом строки. "
                "Добавьте \\n в конец файла")
    elif code == "W293":
        ...
        return (
            f"Строка {line}: Импорт не используется. Удалите или используйте"
        )
    elif code.startswith("F8"):  # F821
        return (f"Строка {line}: Неопределенное имя. "
                f"Проверьте объявление или импорт")
...
def analyze_security_classes(file_path: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "message": f"Найдено {len(found_classes)} классов "
        f"системы безопасности",
    }
...
def analyze_sfm_functions(file_path: str) -> Dict[str, Any]:
    ...
    if os.path.exists(sfm_registry_path):
        try:
            ...
            function_data = (
                functions.get(file_name, {}) if is_registered else {}
            )

            # Статистика по статусам
            active_functions = len(
                [f for f in functions.values() if f.get("status") == "active"]
            )
            critical_functions = len(
                [f for f in functions.values() if f.get("is_critical", False)]
            )
            ...
            return {
                ...
                "message": (f"SFM реестр: {total_functions} функций, "
                            f"{active_functions} активных, "
                            f"{critical_functions} критических"),
            }
        except Exception as e:
            return {
                "status": "error",
        ...
        if isinstance(
            ...
            if not (ast.get_docstring(node)):
                missing_docstrings.append(
                    f"Отсутствует docstring для {node.name} "
                    f"(строка {node.lineno})"
                )
    return {
        "status": "success" if not missing_docstrings else "warning",
        "missing_docstrings": missing_docstrings,
        "message": f"Найдено {len(missing_docstrings)} "
        f"отсутствующих docstring",
    }
        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            if not node.returns:
                missing_type_hints.append(
                    f"Отсутствует тип возвращаемого значения для "
                    f"функции {node.name} (строка {node.lineno})"
                )
            for arg in node.args.args:
                if arg.arg != "self" and not arg.annotation:
                    missing_type_hints.append(
                        f"Отсутствует тип для аргумента '{arg.arg}' "
                        f"в функции {node.name} (строка {node.lineno})"
                    )
    return {
        "status": "success" if not missing_type_hints else "warning",
        "missing_type_hints": missing_type_hints,
        "message": f"Найдено {len(missing_type_hints)} "
        f"отсутствующих type hints",
    }
...
def analyze_dependencies(file_path: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "message": f"Зависимости: {len(dependencies)}",
    }
...
def security_quality_analyzer(file_path: str) -> Dict[str, Any]:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_analysis = {
            executor.submit(task_func, file_path): name
            for name, task_func in analysis_tasks.items()
        }
        for future in concurrent.futures.as_completed(future_to_analysis):
            ...
            try:
                ...
                results[name] = {
                    "status": "error",
                    "message": f"Ошибка при выполнении {name}: {exc}",
                }
    ...
    safe_to_fix_errors = len(
        [
            e
            for e in results.get("flake8", {}).get("errors", [])
            if e.get("security_level") == "safe"
        ]
    )
    manual_review_errors = total_errors - safe_to_fix_errors

    security_keywords_found = len(
        results.get("security_keywords", {}).get("found_keywords", [])
    )
    security_classes_found = len(
        results.get("security_classes", {}).get("found_classes", [])
    )
    sfm_total_functions = results.get("sfm_functions", {}).get(
        "total_sfm_functions", 0
    )
    sfm_active_functions = results.get("sfm_functions", {}).get(
        "active_functions", 0
    )
    sfm_critical_functions = results.get("sfm_functions", {}).get(
        "critical_functions", 0
    )
    is_sfm_registered = results.get("sfm_functions", {}).get(
        "is_current_registered", False
    )

    # Расчет общего балла качества для системы безопасности
    quality_score = 100.0
    if results.get("syntax", {}).get("status") == "error":
        quality_score -= 30
    if results.get("imports", {}).get("status") == "error":
        quality_score -= 20
    quality_score -= total_errors * 1.5  # Каждая ошибка flake8 снижает балл
    if results.get("docstrings", {}).get("missing_docstrings"):
        quality_score -= len(results["docstrings"]["missing_docstrings"]) * 0.5
    if results.get("type_hints", {}).get("missing_type_hints"):
        quality_score -= len(results["type_hints"]["missing_type_hints"]) * 0.3
    ...
    report = {
        ...
        "summary": {
            ...
            "imports_ok": results.get("imports", {}).get("status")
            == "success",
            "docstrings_ok": results.get("docstrings", {}).get("status")
            == "success",
            "type_hints_ok": results.get("type_hints", {}).get("status")
            == "success",
        },
        "recommendations": [],
    ...
    if not report["summary"]["syntax_ok"]:
        report["recommendations"].append(
            "Исправьте критические синтаксические ошибки."
        )
    if not report["summary"]["imports_ok"]:
        report["recommendations"].append(
            "Исправьте критические ошибки импорта."
        )
    if total_errors > 0:
        report["recommendations"].append(
            f"Исправьте {total_errors} ошибок flake8."
        )
    if security_classes_found == 0:
        report["recommendations"].append(
            "Добавьте классы системы безопасности."
        )
    if not is_sfm_registered:
        report["recommendations"].append(
            "Зарегистрируйте функцию в SFM реестре."
        )
    ...
    if not report["summary"]["type_hints_ok"]:
        report["recommendations"].append(
            "Добавьте отсутствующие type hints для улучшения "
            "читаемости и надежности."
        )

    if not report["recommendations"]:
        report["recommendations"].append(
            "Компонент системы безопасности в отличном состоянии!"
        )
    ...
    print(
        f"📊 Размер: "
        f"{report['analysis_details']['file_info']['size_bytes']} байт"
    )
    ...
    print(
        f"🔧 Текущая функция в SFM: {'✅ Да' if is_sfm_registered else '❌ Нет'}"
    )
    print(f"🔑 Ключевых слов безопасности: {security_keywords_found}")

    # Сохранение детального отчета
    report_filename = (f"security_quality_report_"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Использование: python3 security_quality_analyzer.py <file_path>"
        )
