...
class SafeQualityAnalyzer:
    ...
    def __new__(cls):
        """Singleton pattern для анализатора качества"""
    ...
    def __init__(self):
        ...
        self.security_keywords = [
            'encrypt', 'decrypt', 'hash', 'password', 'token', 'auth',
            'permission', 'access', 'security', 'validate', 'sanitize',
            'audit', 'log', 'monitor', 'threat', 'vulnerability'
        ]
        self.critical_functions = [
            'authenticate', 'authorize', 'encrypt', 'decrypt', 'validate',
            'sanitize', 'audit', 'log_security', 'check_permissions'
        ]

        # Регистрируем в SFM (временно отключено)
        # if self.sfm:
        #     self.sfm.register_function(
        #         function_name="safe_quality_analyzer",
        #         function_obj=self,
        #         critical=False,
        #         sleep_mode=True
        #     )

        print("Безопасный анализатор качества инициализирован")
    ...
    async def analyze_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_result = {
                ...
                "analysis": {
                    "syntax_check": await self._check_syntax(file_path),
                ...
                "suggestions": self._generate_safe_suggestions(file_path, content),
                "safety_report": self._generate_safety_report(file_path, content),
                "recommendations": self._generate_recommendations(file_path, content)
            }

            print(f"✅ Анализ завершен для файла: {file_path}")
    ...
    def _check_imports(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                [sys.executable, '-c', f"import {file_path.replace('/', '.').replace('.py', '')}"],
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def _analyze_flake8(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "errors": errors,
                "safe_errors": [e for e in errors if e["safety_level"] == SafetyLevel.SAFE],
                "manual_errors": [e for e in errors if e["safety_level"] == SafetyLevel.MANUAL],
                "dangerous_errors": [e for e in errors if e["safety_level"] == SafetyLevel.DANGEROUS],
                "critical_errors": [e for e in errors if e["safety_level"] == SafetyLevel.CRITICAL]
            }
        except Exception as e:
            return {
                "total_errors": 0,
        ...
        for i, line in enumerate(lines, 1):
            ...
            if 'eval(' in line_lower:
                security_issues.append({
                    "line": i,
                    "type": "dangerous_eval",
                    "description": "Использование eval() может быть опасно",
                    "severity": "high"
                })
            ...
            if 'exec(' in line_lower:
                security_issues.append({
                    "line": i,
                    "type": "dangerous_exec",
                    "description": "Использование exec() может быть опасно",
                    "severity": "high"
                })
            ...
            if 'subprocess.call(' in line_lower and 'shell=True' in line_lower:
                security_issues.append({
                    ...
                    "description": "Потенциальная уязвимость shell injection",
                    "severity": "critical"
                })
        ...
        for i, line in enumerate(lines, 1):
            ...
            if 'for i in range(len(' in line_lower:
                performance_issues.append({
                    ...
                    "description": "Использование range(len()) неэффективно",
                    "suggestion": "Используйте enumerate() или прямую итерацию"
                })
            ...
            if '.keys()' in line_lower and 'for' in line_lower:
                performance_issues.append({
                    ...
                    "description": "Не нужно вызывать .keys() для итерации по словарю",
    ...
    def _is_auto_fixable(self, error_code: str) -> bool:
        ...
        auto_fixable_codes = ['E501', 'W293', 'W292', 'E302', 'E305', 'E128', 'E129']
        return error_code in auto_fixable_codes

    def _generate_safe_suggestions(self, file_path: str, content: str) -> Dict[str, Any]:
        ...
        for error in flake8_result.get("errors", []):
            if error["safety_level"] == SafetyLevel.SAFE:
                suggestions["safe_formatting"].append({
                    ...
                    "suggested_fix": self._get_safe_fix_suggestion(error["code"])
                })
            else:
    ...
    def _get_safe_fix_suggestion(self, error_code: str) -> str:
        ...
        suggestions = {
            ...
            'E302': 'Добавьте 2 пустые строки перед определением функции/класса',
            'E305': 'Добавьте 2 пустые строки после определения функции/класса',
    ...
    def _generate_safety_report(self, file_path: str, content: str) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, file_path: str, content: str) -> List[str]:
        ...
        if flake8_result["total_errors"] > 0:
            recommendations.append(f"Исправьте {flake8_result['total_errors']} ошибок flake8")
    ...
    def enable_sleep_mode(self):
        ...
        print("Спящий режим включен для Safe Quality Analyzer")

    def disable_sleep_mode(self):
        """Выключение спящего режима"""
        self.sleep_mode = False
        print("Спящий режим выключен для Safe Quality Analyzer")

    async def run_tests(self):
        """Запуск тестов для проверки функциональности"""
        print("🧪 Запуск тестов Safe Quality Analyzer...")

        try:
            # Тест 1: Анализ синтаксиса
            test_file = __file__  # Анализируем сам себя
            result = await self.analyze_file(test_file)

            if "error" in result:
                print(f"❌ Ошибка в тесте анализа: {result['error']}")
                return False

            print(f"✅ Тест анализа файла: {result['line_count']} строк")

            # Тест 2: Генерация отчета
            report = self.generate_report(result)
            print(f"✅ Тест генерации отчета: {len(report)} символов")

            print("🎉 Все тесты Safe Quality Analyzer прошли успешно!")
            return True
    ...
    def generate_report(self, analysis_result: Dict[str, Any]) -> str:
        ...
        report.append(f"   Статус: {'✅ Успешно' if syntax['status'] == 'success' else '❌ Ошибка'}")
        ...
        report.append(f"   Статус: {'✅ Успешно' if imports['status'] == 'success' else '❌ Ошибка'}")
        ...
        report.append(f"   Безопасных для исправления: {len(flake8.get('safe_errors', []))}")
        report.append(f"   Требуют ручного анализа: {len(flake8.get('manual_errors', []))}")
        report.append(f"   Опасных: {len(flake8.get('dangerous_errors', []))}")
        report.append(f"   Критических: {len(flake8.get('critical_errors', []))}")
        report.append("")
...
async def analyze_file_safely(file_path: str) -> Dict[str, Any]:
    """Безопасный анализ файла - ТОЛЬКО АНАЛИЗ"""
    return await safe_quality_analyzer.analyze_file(file_path)


async def generate_quality_report(file_path: str) -> str:
    """Генерация отчета о качестве"""
    analysis = await analyze_file_safely(file_path)
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        # Запускаем тесты
        await safe_quality_analyzer.run_tests()

        result = await analyze_file_safely(file_path)
        if "error" in result:
            print(f"❌ Ошибка: {result['error']}")
        else:
            report = await generate_quality_report(file_path)
            print(report)
    else:
