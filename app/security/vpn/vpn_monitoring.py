...
class VPNMonitoring:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.metrics: deque = deque(
            maxlen=self.config.get("max_metrics", 10000)
        )
    ...
    async def _collect_system_metrics(self, timestamp: datetime) -> None:
        ...
        try:
            ...
            self._check_system_thresholds(
                cpu_percent, memory.percent, disk.percent, timestamp
            )
    ...
    def _check_system_thresholds(
        self, cpu: float, memory: float, disk: float, timestamp: datetime
    ) -> None:
    ...
    def _check_server_thresholds(
        self, health: ServerHealth, timestamp: datetime
    ) -> None:
    ...
    def _create_alert(
        ...
        for existing_alert in self.alerts.values():
            if (
                ...
                and existing_alert.source == source
    ...
    async def get_alerts(
        ...
        if resolved is not None:
            filtered_alerts = [
                a for a in filtered_alerts if a.resolved == resolved
            ]
    ...
    async def get_system_summary(self) -> Dict[str, Any]:
        ...
        online_servers = len(
            [h for h in self.server_health.values() if h.is_online]
        )

        if total_servers > 0:
            avg_health_score = statistics.mean(
                [h.get_health_score() for h in self.server_health.values()]
            )
        else:
            avg_health_score = 0
...
async def main():
    ...
    print(
        f"Активных оповещений: {len([a for a in alerts if not a['resolved']])}"
    )
