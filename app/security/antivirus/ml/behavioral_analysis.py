...
if __name__ == "__main__":
    async def main():
        ...
        if "error" not in behavior:
            print(f"Risk: {behavior.get('risk_score', 0.0):.2f}")
            print(f"Pattern: {behavior.get('pattern', 'unknown')}")
            print(f"Recommendations: {behavior.get('recommendations', [])}")
        else:
            print(f"Status: {behavior['error']}")
