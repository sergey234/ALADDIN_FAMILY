...
from .core.antivirus_core import AntivirusCore
from .engines.clamav_engine import ClamAVEngine
from .scanners.malware_scanner import MalwareScanner

try:
    from .ml.malware_ml_model import malware_ml_model, MLScanResult
    ML_AVAILABLE = True
except ImportError:
    ML_AVAILABLE = False
    malware_ml_model = None
...
class AntivirusSecuritySystem:
    ...
    def __init__(
        self,
        name: str = "AntivirusSecuritySystem",
        ...
        self.ml_model = malware_ml_model if ML_AVAILABLE else None
    ...
    async def scan_file(
        self, file_path: str, engine: AntivirusEngine = AntivirusEngine.ALL
    ) -> Dict[str, Any]:
        """Сканирование файла"""
        try:
            ...
            if (
                engine in [AntivirusEngine.INTERNAL, AntivirusEngine.ALL]
                and self.antivirus_core
            ):
                try:
                    result = await self.antivirus_core.scan_file(
                        file_path, "internal"
                    )
            ...
            if (
                engine in [AntivirusEngine.CLAMAV, AntivirusEngine.ALL]
                and self.clamav_engine
            ):
                try:
            ...
            if (
                engine
                in [AntivirusEngine.MALWARE_SCANNER, AntivirusEngine.ALL]
                and self.malware_scanner
            ):
                try:
            ...
            # Сканирование через ML модель
            if (
                engine in [AntivirusEngine.ML_MODEL, AntivirusEngine.ALL]
                and self.ml_model
            ):
                try:
                    ml_result = await self.ml_model.predict_threat(file_path)
                    scan_results["ml_model"] = {
                        "threat_probability": ml_result.threat_probability.value,
                        "confidence": ml_result.confidence,
                        "features": ml_result.features,
                        "recommendations": ml_result.recommendations,
                        "scan_time": ml_result.scan_time,
                        "model_version": ml_result.model_version,
                    }
                    # Если ML считает файл критичным - добавляем в угрозы
                    if ml_result.threat_probability.value in ["high", "critical"]:
                        threats_found.append({
                            "name": f"ML_{ml_result.threat_probability.value}",
                            "type": "ml_detection",
                            "level": ml_result.threat_probability.value,
                            "confidence": ml_result.confidence
                        })
                    logger.info(f"ML analysis: {ml_result.threat_probability.value} ({ml_result.confidence:.2f})")
                except Exception as e:
                    logger.error(f"Ошибка ML сканирования: {e}")
                    scan_results["ml_model"] = {"error": str(e)}
            ...
            if (
                ...
                try:
                    await self.antivirus_core.quarantine_file(
                        file_path, threats_found
                    )
    ...
    async def scan_directory(
        self,
        directory_path: str,
        engine: AntivirusEngine = AntivirusEngine.ALL,
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Сканирование директории завершено: "
                f"{total_threats} угроз найдено"
            )
        ...
        return [
            {
                ...
                "threats": [
                    {"name": t.name, "type": t.threat_type.value}
                    for t in item.threat_signatures
                ],
    ...
    def get_system_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "quarantine_items": len(self.get_quarantine_items()),
            "engines": {
                "internal": self.antivirus_core is not None,
                "clamav": self.clamav_engine is not None
                and self.clamav_engine.available,
