...
class CloudStorageSecurityBot:
    ...
    async def _detect_file_threats(
        ...
        if (
            ...
            threat = FileThreat(
                threat_id=f"blocked_ext_{file_hash}",
                file_path=file_path,
                file_name=file_name,
                file_type=file_type,
                threat_level=ThreatLevel.HIGH,
        ...
        if file_size_mb > max_size_mb:
            file_hash = hashlib.md5(file_path.encode()).hexdigest()[:8]
            threat = FileThreat(
                threat_id=f"oversized_{file_hash}",
                file_path=file_path,
                file_name=file_name,
                file_type=file_type,
        ...
        for signature in self.threat_database["malware_signatures"]:
            if signature.upper() in content_hex:
                file_hash = hashlib.md5(file_path.encode()).hexdigest()[:8]
                threat = FileThreat(
                    threat_id=f"malware_{file_hash}",
                    file_path=file_path,
                    file_name=file_name,
                    file_type=file_type,
        ...
        for pattern in self.threat_database["sensitive_patterns"]:
            if re.search(pattern, content_str):
                file_hash = hashlib.md5(file_path.encode()).hexdigest()[:8]
                threat = FileThreat(
                    threat_id=f"sensitive_{file_hash}",
                    file_path=file_path,
                    file_name=file_name,
                    file_type=file_type,
                    threat_level=ThreatLevel.HIGH,
        ...
        for keyword in self.threat_database["suspicious_keywords"]:
            if keyword.lower() in content_str.lower():
                file_hash = hashlib.md5(file_path.encode()).hexdigest()[:8]
                threat = FileThreat(
                    threat_id=f"suspicious_{file_hash}",
                    file_path=file_path,
                    file_name=file_name,
                    file_type=file_type,
                    threat_level=ThreatLevel.MEDIUM,
    ...
    def _generate_file_message(
        ...
        if action == StorageAction.BLOCK:
            return (
                f"🚫 Файл заблокирован: {threat_level.value.upper()} "
                f"уровень угрозы"
            )
        elif action == StorageAction.ENCRYPT:
            return (
                f"🔒 Файл зашифрован: {threat_level.value.upper()} "
                f"уровень угрозы"
            )
        elif action == StorageAction.QUARANTINE:
            return (
                f"⚠️ Файл помещен в карантин: {threat_level.value.upper()} "
                f"уровень угрозы"
            )
        else:
    ...
    async def _block_file(
        ...
        try:
            ...
            cursor.execute(
                ...
                (file_path, file_name, file_type, reason, block_time, "
                "threat_level)
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                (
    ...
    async def _save_session(self, session: StorageSession):
        ...
        try:
            ...
            cursor.execute(
                ...
                (session_id, user_id, provider, start_time, end_time, "
                "files_uploaded, files_downloaded, files_shared, "
