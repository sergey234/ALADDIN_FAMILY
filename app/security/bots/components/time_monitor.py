...
# from datetime import timedelta  # TODO: Добавить при реализации
# временных интервалов
from typing import Any, Dict, List, Optional, Tuple

from security.bots.parental_control_bot import ActivityAlert, TimeLimitData
...
class TimeMonitor:
    ...
    def __init__(self, logger: logging.Logger):
        self.logger = logger
        self.active_sessions: Dict[str, Dict[str, TimeUsage]] = (
            {}
        )  # child_id -> device_type -> TimeUsage
        self.time_limits: Dict[str, Dict[str, int]] = (
            {}
        )  # child_id -> device_type -> minutes
    ...
    async def end_session(
        self, child_id: str, device_type: str
    ) -> Optional[int]:
        """Завершение сессии использования устройства"""
        try:
            async with self._lock:
                if (
                    child_id not in self.active_sessions
                    or device_type not in self.active_sessions[child_id]
                ):
                    return None
                ...
                await self._update_daily_usage(
                    child_id, device_type, duration_minutes
                )
                ...
                self.logger.info(
                    f"Завершена сессия {device_type} для {child_id}: "
    ...
    async def set_time_limit(
        self, child_id: str, device_type: str, minutes: int
    ) -> bool:
        ...
        try:
            ...
            async with self._lock:
                ...
                self.logger.info(
                    f"Установлен лимит {device_type}: {minutes}м "
                    f"для {child_id}"
                )
                return True
            ...
            async with self._lock:
                ...
                if current_usage > limit:
                    ...
                    alert = ActivityAlert(
                        ...
                        severity="medium",
                        message=f"Превышен лимит времени {device_type}: "
    ...
    async def get_current_usage(self, child_id: str, device_type: str) -> int:
        ...
        try:
            async with self._lock:
                ...
                if (
                    child_id in self.active_sessions
                    and device_type in self.active_sessions[child_id]
                ):
                    session = self.active_sessions[child_id][device_type]
    ...
    async def get_active_sessions(self, child_id: str) -> List[TimeUsage]:
        ...
        return [
            session
            for session in self.active_sessions[child_id].values()
            if session.is_active
        ]
    ...
    async def reset_daily_usage(self, child_id: str = None):
        ...
        async with self._lock:
            ...
            self.logger.info(
                f"Сброшено дневное использование для {child_id or 'всех'}"
            )
    ...
    async def validate_time_limit_data(
        self, device_type: str, minutes: int
