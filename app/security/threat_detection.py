...
class ThreatDetection:
    ...
    def analyze_data(
        ...
        for indicator in self.indicators:
            ...
            # Простая проверка по паттерну (в реальной системе здесь был бы
            # regex)
            if indicator.pattern in data:
                detection = ThreatDetection(
                    detection_id=(
                        f"threat_{int(time.time())}_{len(detections)}"
                    ),
                ...
                print(
                    f"Обнаружена угроза: {indicator.name} "
                    f"(уверенность: {indicator.confidence})"
                )
    ...
    def mark_false_positive(self, detection_id: str) -> bool:
        ...
        for detection in self.detections:
            if detection.detection_id == detection_id:
                ...
                detection.analyzed_at = datetime.datetime.now().isoformat()
                print(
                    f"Обнаружение '{detection_id}' помечено как "
