...
class SafeFunctionManager(SecurityBase):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Настройка обработчика IoT Security Agent
            self._setup_iot_security_agent_handler()
            # Настройка обработчика Self Harm Detection Agent
            self._setup_self_harm_detection_handler()
            # Настройка обработчика Online Predators Agent
            self._setup_online_predators_handler()
            # Настройка обработчика Grooming Detection Agent
            self._setup_grooming_detection_handler()
            # Настройка обработчика Fake News Detection Agent
            self._setup_fake_news_detection_handler()
            # Настройка обработчика Fake Documents Agent
            self._setup_fake_documents_handler()
    ...
    def _register_basic_functions(self):
        ...
        basic_functions = [
            ...
            {
                "function_id": "iot_security_agent",
                "name": "IoTSecurityAgent",
                "description": "Агент мониторинга и защиты IoT-устройств",
                "function_type": "ai_agent",
                "security_level": SecurityLevel.HIGH,
                "is_critical": True,
                "auto_enable": True,
            },
            {
                ...
                "name": "SelfHarmDetectionAgent",
                "description": "BERT агент мониторинга self-harm контента",
                "function_type": "ai_agent",
                "security_level": SecurityLevel.HIGH,
                "is_critical": True,
                "auto_enable": True,
            ...
            {
                "function_id": "online_predators_agent",
                "name": "OnlinePredatorsAgent",
                "description": "CNN+RNN агент выявления онлайн-хищников",
                "function_type": "ai_agent",
                "security_level": SecurityLevel.HIGH,
                "is_critical": True,
                "auto_enable": True,
            ...
            {
                "function_id": "grooming_detection_agent",
                "name": "GroomingDetectionAgent",
                "description": "Transformer агент детекции grooming (ухаживания/манипуляций)",
                "function_type": "ai_agent",
                "security_level": SecurityLevel.HIGH,
                "is_critical": True,
                "auto_enable": True,
            ...
            {
                "function_id": "fake_news_detection_agent",
                "name": "FakeNewsDetectionAgent",
                "description": "BERT агент детекции фейковых новостей и дезинформации",
                "function_type": "ai_agent",
                "security_level": SecurityLevel.HIGH,
                "is_critical": True,
                "auto_enable": True,
            ...
            {
                "function_id": "fake_documents_agent",
                "name": "FakeDocumentsAgent",
                "description": "OpenCV агент проверки поддельных документов",
                "function_type": "ai_agent",
                "security_level": SecurityLevel.HIGH,
                "is_critical": True,
                "auto_enable": True,
    ...
    def _setup_dependencies(self):
        ...
        dependencies = {
            ...
            "self_harm_detection_agent": ["security_base", "emergencymlanalyzer"],
            "online_predators_agent": ["security_base", "emergencymlanalyzer"],
            "grooming_detection_agent": ["security_base", "emergencymlanalyzer"],
            "fake_news_detection_agent": ["security_base", "emergencymlanalyzer"],
            "fake_documents_agent": ["security_base", "emergencymlanalyzer"],
        }
        ...
        if function_id in self.function_handlers:
            ...
            if isinstance(params, dict):
                try:
                    ...
                    # Совместимость со старыми обработчиками, ожидающими один аргумент
                    return handler(params)
            return handler(params)
        else:
    ...
    def _setup_iot_security_agent_handler(self):
        """Настройка обработчика для IoT Security Agent"""
        try:

            def iot_security_agent_handler(*args, **kwargs):
                """Обработчик IoT Security Agent"""
                try:
                    ...
                    if action == "detect_threats":
                        ...
                        threats = agent.detect_threats(devices)
                        return {
                            "status": "success",
                            "function_id": "iot_security_agent",
                            "handler_name": "IoTSecurityAgent",
                            "message": f"Обработано устройств: {len(devices)}",
                            "threats": threats,
                        }
                    if action == "analyze_device":
                        device = kwargs.get("device", {})
                        analysis = agent.analyze_device(device)
                        return {
                            "status": "success",
                            "function_id": "iot_security_agent",
                            "handler_name": "IoTSecurityAgent",
                            "message": f"Устройство {analysis.get('device_name', 'unknown')} проанализировано",
                            "analysis": analysis,
                        }
                    ...
                    return {
                        ...
                        "function_id": "iot_security_agent",
                        "handler_name": "IoTSecurityAgent",
                        "message": "IoT Security Agent инициализирован",
                    }
                except Exception as e:
                    return {
                        "status": "error",
                        "function_id": "iot_security_agent",
                        "handler_name": "IoTSecurityAgent",
                        "message": f"Ошибка исполнения IoT Security Agent: {e}",
                    }

            self.register_function_handler("iot_security_agent", iot_security_agent_handler)
            self.log_activity("Обработчик IoT Security Agent настроен")

        except Exception:
            self.log_activity("Ошибка при настройке IoT Security Agent", "error")

    def _setup_self_harm_detection_handler(self):
        """Настройка обработчика для Self Harm Detection Agent"""
        try:

            def self_harm_detection_handler(*args, **kwargs):
                """Обработчик SelfHarmDetectionAgent"""
                try:
                    from security.ai_agents.self_harm_detection_agent import SelfHarmDetectionAgent

                    model_name = kwargs.get("model_name")
                    agent = SelfHarmDetectionAgent(model_name=model_name)
                    action = kwargs.get("action", "analyze_text")
                    ...
                    if action == "analyze_batch":
                        ...
                        return {
                            "status": "success",
                            "function_id": "self_harm_detection_agent",
                            "handler_name": "SelfHarmDetectionAgent",
                            "message": f"Проанализировано сообщений: {len(texts)}",
                            "results": results,
                        }
                    ...
                    if action == "health_check":
                        return {
                            ...
                            "function_id": "self_harm_detection_agent",
                            "handler_name": "SelfHarmDetectionAgent",
                            "message": "SelfHarmDetectionAgent готов к работе",
                        }

                    text = kwargs.get("text", "")
                    if not text:
                        raise ValueError("Parameter 'text' is required for analyze_text action")

                    analysis = agent.detect_self_harm(text)
                    return {
                        "status": "success",
                        "function_id": "self_harm_detection_agent",
                        "handler_name": "SelfHarmDetectionAgent",
                    ...
                    return {
                        ...
                        "function_id": "self_harm_detection_agent",
                        "handler_name": "SelfHarmDetectionAgent",
                        "message": f"Ошибка исполнения SelfHarmDetectionAgent: {e}",
                    }

            self.register_function_handler("self_harm_detection_agent", self_harm_detection_handler)
            self.log_activity("Обработчик Self Harm Detection Agent настроен")

        except Exception:
            self.log_activity("Ошибка при настройке Self Harm Detection Agent", "error")

    def _setup_online_predators_handler(self):
        """Настройка обработчика для Online Predators Agent"""
        try:

            def online_predators_handler(*args, **kwargs):
                """Обработчик OnlinePredatorsAgent"""
                try:
                    from pathlib import Path

                    from security.ai_agents.online_predators_agent import OnlinePredatorsAgent

                    cnn_weights = kwargs.get("cnn_weights_path")
                    rnn_weights = kwargs.get("rnn_weights_path")
                    agent = OnlinePredatorsAgent(
                        cnn_weights_path=Path(cnn_weights) if cnn_weights else None,
                        rnn_weights_path=Path(rnn_weights) if rnn_weights else None,
                    )

                    action = kwargs.get("action", "analyze_combined")

                    if action == "analyze_images":
                        image_paths = [Path(p) for p in kwargs.get("image_paths", [])]
                        results = agent.predict_batch(image_paths)
                        return {
                            "status": "success",
                            "function_id": "online_predators_agent",
                            "handler_name": "OnlinePredatorsAgent",
                            "mode": "images",
                            "results": results,
                        }

                    if action == "analyze_texts":
                        texts = kwargs.get("texts", [])
                        agent.build_vocab(texts)
                        results = agent.predict_text_batch(texts)
                        return {
                            "status": "success",
                            "function_id": "online_predators_agent",
                            "handler_name": "OnlinePredatorsAgent",
                    ...
                    # Combined: анализируем оба набора если переданы
                    texts = kwargs.get("texts", [])
                    images = [Path(p) for p in kwargs.get("image_paths", [])]
                    combined = {}
                    if images:
                        combined["image_results"] = agent.predict_batch(images)
                    if texts:
                        agent.build_vocab(texts)
                        combined["text_results"] = agent.predict_text_batch(texts)
                    ...
                    return {
                        ...
                        "function_id": "online_predators_agent",
                        "handler_name": "OnlinePredatorsAgent",
                        "message": f"Ошибка выполнения OnlinePredatorsAgent: {e}",
                    }

            self.register_function_handler("online_predators_agent", online_predators_handler)
            self.log_activity("Обработчик Online Predators Agent настроен")

        except Exception:
            self.log_activity("Ошибка при настройке Online Predators Agent", "error")

    def _setup_grooming_detection_handler(self):
        """Настройка обработчика для Grooming Detection Agent"""
        try:

            def grooming_detection_handler(*args, **kwargs):
                """Обработчик GroomingDetectionAgent"""
                try:
                    from security.ai_agents.grooming_detection_agent import GroomingDetectionAgent

                    model_name = kwargs.get("model_name")
                    agent = GroomingDetectionAgent(model_name=model_name)
                    action = kwargs.get("action", "analyze_text")
                    ...
                    if action == "analyze_batch":
                        ...
                        return {
                            "status": "success",
                            "function_id": "grooming_detection_agent",
                            "handler_name": "GroomingDetectionAgent",
                            "message": f"Проанализировано сообщений: {len(texts)}",
                            "results": results,
                        }
                    ...
                    if action == "health_check":
                        return {
                            ...
                            "function_id": "grooming_detection_agent",
                            "handler_name": "GroomingDetectionAgent",
                            "message": "GroomingDetectionAgent готов к работе",
                        }
                    ...
                    analysis = agent.detect_grooming(text)
                    return {
                        "status": "success",
                        "function_id": "grooming_detection_agent",
                        "handler_name": "GroomingDetectionAgent",
                    ...
                    return {
                        ...
                        "function_id": "grooming_detection_agent",
                        "handler_name": "GroomingDetectionAgent",
                        "message": f"Ошибка исполнения GroomingDetectionAgent: {e}",
                    }

            self.register_function_handler("grooming_detection_agent", grooming_detection_handler)
            self.log_activity("Обработчик Grooming Detection Agent настроен")

        except Exception:
            self.log_activity("Ошибка при настройке Grooming Detection Agent", "error")

    def _setup_fake_news_detection_handler(self):
        """Настройка обработчика для Fake News Detection Agent"""
        try:
            ...
            def fake_news_detection_handler(*args, **kwargs):
                ...
                try:
                    ...
                    if action == "analyze_batch":
                        ...
                        results = agent.analyze_batch(texts, metadata_list)
                        return {
                            "status": "success",
                            "function_id": "fake_news_detection_agent",
                            "handler_name": "FakeNewsDetectionAgent",
                            "message": f"Проанализировано текстов: {len(texts)}",
                            "results": results,
                        }
                    ...
                    if action == "health_check":
                        return {
                            ...
                            "function_id": "fake_news_detection_agent",
                            "handler_name": "FakeNewsDetectionAgent",
                            "message": "FakeNewsDetectionAgent готов к работе",
                        }
                    ...
                    analysis = agent.detect_fake_news(text, metadata)
                    return {
                        "status": "success",
                        "function_id": "fake_news_detection_agent",
                        "handler_name": "FakeNewsDetectionAgent",
                        "message": "Текст проанализирован",
                        "analysis": analysis,
                    }
                except Exception as e:
                    return {
                        ...
                        "function_id": "fake_news_detection_agent",
                        "handler_name": "FakeNewsDetectionAgent",
                        "message": f"Ошибка исполнения FakeNewsDetectionAgent: {e}",
                    }

            self.register_function_handler("fake_news_detection_agent", fake_news_detection_handler)
            self.log_activity("Обработчик Fake News Detection Agent настроен")

        except Exception:
            self.log_activity("Ошибка при настройке Fake News Detection Agent", "error")

    def _setup_fake_documents_handler(self):
        """Настройка обработчика для Fake Documents Agent"""
        try:

            def fake_documents_handler(*args, **kwargs):
                """Обработчик FakeDocumentsAgent"""
                try:
                    from pathlib import Path

                    from security.ai_agents.fake_documents_agent import FakeDocumentsAgent

                    agent = FakeDocumentsAgent()
                    action = kwargs.get("action", "analyze_document")

                    if action == "analyze_batch":
                        image_paths = [Path(p) for p in kwargs.get("image_paths", [])]
                        results = agent.analyze_batch(image_paths)
                        return {
                            "status": "success",
                            "function_id": "fake_documents_agent",
                            "handler_name": "FakeDocumentsAgent",
                            "message": f"Проанализировано документов: {len(image_paths)}",
                            "results": results,
                        }
                    ...
                    if action == "health_check":
                        return {
                            ...
                            "function_id": "fake_documents_agent",
                            "handler_name": "FakeDocumentsAgent",
                            "message": "FakeDocumentsAgent готов к работе",
                        }

                    image_path = kwargs.get("image_path")
                    if not image_path:
                        raise ValueError("Parameter 'image_path' is required for analyze_document action")

                    analysis = agent.detect_fake_document(image_path)
                    return {
                        "status": "success",
                        "function_id": "fake_documents_agent",
                        "handler_name": "FakeDocumentsAgent",
                        "message": "Документ проанализирован",
                        "analysis": analysis,
                    }
                except Exception as e:
                    return {
                        ...
                        "function_id": "fake_documents_agent",
                        "handler_name": "FakeDocumentsAgent",
                        "message": f"Ошибка исполнения FakeDocumentsAgent: {e}",
                    }

            self.register_function_handler("fake_documents_agent", fake_documents_handler)
            self.log_activity("Обработчик Fake Documents Agent настроен")

        except Exception:
            self.log_activity("Ошибка при настройке Fake Documents Agent", "error")
