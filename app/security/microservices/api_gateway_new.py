...
безопасности, включающую безопасную маршрутизацию, аутентификацию, мониторинг и
защиту.
...
# Prometheus метрики (будут инициализированы в классе)
...
class APIGateway:
    ...
    def _init_metrics(self) -> None:
        """Инициализация Prometheus метрик"""
        try:
            # Проверяем, не инициализированы ли уже метрики
            ...
            self.logger.info("Prometheus метрики инициализированы")
        except Exception as e:
            self.logger.error(f"Ошибка инициализации метрик: {e}")
    ...
    async def register_route(self, route_config: RouteConfig) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Маршрут зарегистрирован: {route_config.method} "
                f"{route_config.path}"
            )
            return True
    ...
    async def process_request(self, request: APIRequest) -> APIResponse:
        ...
        try:
            ...
            if not route:
                raise HTTPException(
                    status_code=404,
                    detail=f"Маршрут не найден: {request.method} "
                    f"{request.path}",
                )
