                                ...
                                connections.append(
                                    {
                                        "from": service_name,
                                        "to": (
                                            f"External:{conn.raddr.ip}:{conn.raddr.port}"
                                            if conn.raddr
                                            else "Local"
                                        ),
                                        "status": conn.status,
...
async def filter_services(
    status: Optional[str] = None, service_type: Optional[str] = None
):
    ...
    return {
        ...
        "filters": {"status": status, "service_type": service_type},
    }
        ...
        history.append(
            {
                ...
                "running_services": len(
                    [
                        p
                        for p in arch_manager.aladdin_ports.values()
                        if arch_manager._check_port_status(p)
                    ]
                ),
                "alerts_count": 0,
            }
