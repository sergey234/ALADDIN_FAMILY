...
class PhishingProtectionAgent:
    ...
    def email_breach_monitoring(self, email_address: str) -> dict:
        ...
        try:
            ...
            if result["breach_detected"]:
                print(
                    f"🚨 УТЕЧКА ОБНАРУЖЕНА: {email_address} - {result['breach_count']} утечек, риск: {result['risk_level']}"
                )
            else:
    ...
    def email_security_assessment(self, email: str) -> dict:
        ...
        try:
            ...
            # Логирование результата
            print(
                f"📊 Оценка безопасности завершена: {email} - {result['security_score']:.1f}/100, риск: {result['overall_risk']}"
            )
