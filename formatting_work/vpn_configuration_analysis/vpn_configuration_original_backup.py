...
class VPNConfiguration:
    ...
    async def get_all_servers(self, enabled_only: bool = False) -> List[ServerConfig]:
    ...
    async def get_servers_by_location(self, location: ServerLocation) -> List[ServerConfig]:
        """Получение серверов по локации"""
        return [s for s in self.servers.values() if s.location == location]

    async def set_client_config(self, user_id: str, client_config: ClientConfig) -> bool:
    ...
    async def export_config(self, format: str = "json",
    ...
    async def import_config(self, file_path: str, merge: bool = False) -> bool:
        ...
        try:
            ...
            if not self._validate_imported_config(new_config):
                logger.error("Импортированная конфигурация не прошла валидацию")
                return False
    ...
    async def generate_client_config_file(self, user_id: str,
                                         server_id: str) -> Optional[str]:
        """
        ...
        try:
            ...
            config_content = self._generate_openvpn_config(client_config, server_config)
    ...
    def _generate_openvpn_config(self, client_config: ClientConfig,
                                server_config: ServerConfig) -> str:
