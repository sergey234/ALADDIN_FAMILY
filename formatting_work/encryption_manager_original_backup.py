...
class EncryptionManager:
    ...
    def _initialize_encryption(self):
        ...
        try:
            if not CRYPTOGRAPHY_AVAILABLE:
                self.logger.warning("Cryptography не доступна, используется упрощенное шифрование")
                return
    ...
    async def decrypt_data(self, encrypted_data: EncryptedData) -> Union[str, bytes, Dict[str, Any]]:
        """Расшифровка данных"""
        try:
            ...
            if encrypted_data.algorithm == EncryptionAlgorithm.FERNET:
                decrypted_bytes = await self._decrypt_fernet(encrypted_data, key)
            elif encrypted_data.algorithm == EncryptionAlgorithm.AES_256_GCM:
                decrypted_bytes = await self._decrypt_aes_gcm(encrypted_data, key)
            elif encrypted_data.algorithm == EncryptionAlgorithm.AES_256_CBC:
                decrypted_bytes = await self._decrypt_aes_cbc(encrypted_data, key)
            else:
                # Fallback к простому расшифрованию
                decrypted_bytes = await self._decrypt_simple(encrypted_data, key)
    ...
    async def _encrypt_aes_gcm(self, data: bytes, key: EncryptionKey) -> EncryptedData:
    ...
    async def _encrypt_simple(self, data: bytes, key: EncryptionKey) -> EncryptedData:
    ...
    async def _decrypt_simple(self, encrypted_data: EncryptedData, key: EncryptionKey) -> bytes:
    ...
    async def _rotate_key(self, old_key_id: Optional[str] = None):
        ...
        try:
            ...
            self.logger.info(f"Ключ повернут: {old_key_id} -> {self.active_key_id}")
            
        except Exception as e:
            self.logger.error(f"Ошибка ротации ключа: {e}")
    
    async def encrypt_sensitive_field(self, field_name: str, value: Any, 
                                    child_id: Optional[str] = None) -> EncryptedData:
    ...
    async def decrypt_sensitive_field(self, encrypted_data: EncryptedData) -> Tuple[str, Any]:
        ...
        try:
            ...
            field_name = encrypted_data.metadata.get("field_name", "unknown") if encrypted_data.metadata else "unknown"
    ...
    async def decrypt_file(self, encrypted_file_path: str, output_path: Optional[str] = None) -> str:
        ...
        try:
            ...
            self.logger.info(f"Файл расшифрован: {encrypted_file_path} -> {output_path}")
            return output_path
    ...
    async def cleanup_expired_keys(self) -> int:
        ...
        try:
            ...
            if expired_keys:
                self.logger.info(f"Очищено {len(expired_keys)} истекших ключей")
    ...
    async def export_key(self, key_id: str, password: str) -> str:
        ...
        try:
            ...
            export_data = {
                "key_id": key_id,
                "algorithm": key.algorithm.value,
                "encrypted_key": base64.b64encode(bytes(encrypted_key)).decode('utf-8'),
    ...
    async def import_key(self, exported_key: str, password: str) -> str:
        ...
        try:
            ...
            key = EncryptionKey(
                ...
                expires_at=datetime.fromisoformat(export_data["expires_at"]) if export_data["expires_at"] else None
            )
