...
class SmartMonitoringSystem:
    ...
    Этот класс предоставляет продвинутую систему мониторинга метрик с защитой от спама,
    адаптивными порогами и асинхронной обработкой.
    ...
    __slots__ = (
        ...
        'total_alerts_generated', 'total_alerts_suppressed',
        'total_alerts_resolved', 'config_version', 'max_alerts_in_memory',
        'max_metrics_per_name', 'cleanup_interval_hours', 'performance_stats',
        'log_level', 'log_file', 'enable_debug', 'enable_validation',
                ...
                for rule_id, rule_config in config["rules"].items():
                    if isinstance(rule_config, dict):
                        rule = AlertRule(
                            ...
                            severity=AlertSeverity(rule_config.get("severity", "info")),
                            ...
                            adaptive_threshold=rule_config.get("adaptive_threshold", True)
                        )
                        system.add_rule(rule)
    ...
    async def _check_rules_async(
        ...
        try:
            ...
            for rule_id, rule in self.rules.items():
                ...
                await self._generate_alert_async(rule, value, tags, current_time)
        except Exception as e:
        ...
        try:
            ...
            with self.lock:
                ...
                if len(self.alerts) > self.max_alerts_in_memory:
                    self.alerts = self.alerts[-self.max_alerts_in_memory//2:]

            # Асинхронная отправка уведомлений
            await self._send_alert_async(alert)
        except Exception as e:
            print(f"Ошибка асинхронной генерации алерта для {rule.rule_id}: {e}")
            raise
        ...
        try:
            for callback in self.alert_callbacks:
                try:
                    ...
                    print(
                        f"Ошибка в асинхронном callback для алерта {alert.alert_id}: {e}"
                    )
    ...
    @private_method
    async def _adapt_threshold_async(self, rule: AlertRule, current_value: float) -> None:
        """
        ...
        try:
            ...
            print(f"Ошибка асинхронной адаптации порога для {rule.rule_id}: {e}")
    ...
    async def _cleanup_old_data_async(self) -> None:
        ...
        try:
            with self.lock:
                ...
                print(f"Асинхронная очистка: удалено {removed_alerts} старых алертов")
        except Exception as e:
    ...
    def _validate_metric_name(self, metric_name: str) -> None:
        ...
        for char in invalid_chars:
            if char in metric_name:
                raise ValueError(f"metric_name содержит недопустимый символ: {repr(char)}")
        ...
        for keyword in sql_keywords:
            if keyword in metric_upper:
                raise ValueError(f"metric_name содержит потенциально опасный SQL ключевое слово: {keyword}")
    ...
    def _validate_tags(self, tags: Optional[Dict[str, str]]) -> None:
        ...
        for key, value in tags.items():
            ...
            if len(key) > 100 or len(value) > 500:
                raise ValueError("Ключи тегов не должны превышать 100 символов, значения - 500")
            
            if not key.strip() or not value.strip():
                raise ValueError("Ключи и значения тегов не могут быть пустыми")
            
            # Проверяем на недопустимые символы в ключах
            invalid_chars = ['=', ' ', '\n', '\r', '\t']
            for char in invalid_chars:
                if char in key:
                    raise ValueError(f"Ключ тега содержит недопустимый символ: {repr(char)}")
    ...
    def _validate_rule(self, rule: AlertRule) -> None:
        ...
        if not isinstance(rule.cooldown, int) or rule.cooldown < 0:
            raise ValueError("cooldown должен быть неотрицательным целым числом")
        
        if rule.cooldown > 86400:  # Максимум 24 часа
            raise ValueError("cooldown не может превышать 86400 секунд (24 часа)")
        
        # Валидируем min_occurrences
        if not isinstance(rule.min_occurrences, int) or rule.min_occurrences < 1:
            raise ValueError("min_occurrences должен быть положительным целым числом")
        
        # Валидируем max_alerts_per_hour
        if not isinstance(rule.max_alerts_per_hour, int) or rule.max_alerts_per_hour < 1:
            raise ValueError("max_alerts_per_hour должен быть положительным целым числом")
    ...
    def _validate_alert_id(self, alert_id: str) -> None:
        ...
        if '_' not in alert_id:
            raise ValueError("alert_id должен содержать символ '_' для разделения rule_id и timestamp")
        
        parts = alert_id.split('_', 1)
        if len(parts) != 2:
            raise ValueError("alert_id должен иметь формат 'rule_id_timestamp'")
    ...
    def _check_memory_limits(self) -> bool:
        ...
        try:
            ...
            if total_metrics > self.max_metrics_per_name * 10:  # 10x лимит для всех метрик
                return False
            ...
            with self.lock:
                ...
                for metric_name in list(self.metrics.keys()):
                    if len(self.metrics[metric_name]) > self.max_metrics_per_name:
                        ...
                        self.metrics[metric_name] = self.metrics[metric_name][-keep_count:]
                
                # Очищаем историю алертов
                for rule_id in self.alert_history:
                    self.alert_history[rule_id] = [
                        alert_time for alert_time in self.alert_history[rule_id]
                        if alert_time > cutoff_time
                    ]
                ...
                for metric_name in list(self.baseline_metrics.keys()):
                    if len(self.baseline_metrics[metric_name]) > 50:
                        self.baseline_metrics[metric_name] = self.baseline_metrics[metric_name][-25:]
                
                removed_alerts = old_count - len(self.alerts)
                print(f"Принудительная очистка памяти: удалено {removed_alerts} алертов")
    ...
    def _estimate_memory_usage(self) -> Dict[str, int]:
        ...
        try:
            ...
            memory_usage = {
                ...
                "metrics_mb": (sum(len(values) for values in self.metrics.values()) * metric_size) / 1024 / 1024,
        ...
        try:
            with self.lock:
                ...
                return {
                    ...
                    "memory_pressure": memory_usage["total_estimated_mb"] > self.max_memory_usage_mb * 0.8,
        ...
        try:
            self._log_event(
                ...
                performance_metrics={
                    "total_alerts": len(self.alerts),
                    "total_metrics": sum(len(values) for values in self.metrics.values()),
                    "memory_usage_mb": self._estimate_memory_usage().get("total_estimated_mb", 0),
                    **kwargs
                }
            )
        except Exception as e:
    ...
    @private_method
    def _log_security_event(self, event_type: str, details: str, severity: str = "INFO") -> None:
        """
    ...
    @validate_input
    def set_logging_config(self, log_level: str = "INFO", log_file: Optional[str] = None, enable_debug: bool = False) -> bool:
        """
        Настройка конфигурации логирования
        
        Args:
            log_level: Уровень логирования (DEBUG, INFO, WARNING, ERROR, CRITICAL, DISABLED)
        ...
        try:
            ...
            self._log_event("INFO", "Конфигурация логирования обновлена", 
                          log_level=log_level, log_file=log_file, enable_debug=enable_debug)
    ...
    @private_method
    def _handle_error(self, error: Exception, context: str, severity: str = "ERROR") -> None:
        """
    ...
    def _emergency_recovery(self) -> None:
        ...
        try:
            ...
            if self.callback_error_count > self.max_callback_errors:
                ...
                self.callback_error_count = 0
                self._log_event("WARNING", "Очищены все callback'ы из-за ошибок")
    ...
    def get_system_health_detailed(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._handle_error(e, "получение детального здоровья системы", "ERROR")
            return {
