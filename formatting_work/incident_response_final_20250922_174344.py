...
class Incident:
    ...
    def is_escalated(self) -> bool:
        ...
        return self.priority in [IncidentPriority.HIGH, IncidentPriority.CRITICAL]
    ...
    def add_evidence(self, evidence_type: str, description: str, file_path: Optional[str] = None) -> None:
    ...
    def add_action(self, action: str, author: str, result: Optional[str] = None) -> None:
        """Добавление выполненного действия к инциденту"""
        action_record = {
            "action": action,
            "author": author,
            "result": result,
...
class IncidentResponseManager(SecurityBase):
    ...
    def create_incident(
        ...
        Args:
            ...
            description (str): Подробное описание инцидента (1-1000 символов, не пустое)
            incident_type (IncidentType): Тип инцидента из перечисления IncidentType
            priority (IncidentPriority): Приоритет инцидента из перечисления IncidentPriority
            severity (SecurityLevel): Уровень серьезности из перечисления SecurityLevel
            affected_systems (Optional[List[str]]): Список затронутых систем (опционально)
            
        Returns:
            Optional[Incident]: Созданный объект инцидента или None при ошибке валидации
            
        Raises:
            ValueError: При некорректных входных параметрах (логируется как ошибка)
        ...
        try:
            ...
            if affected_systems is not None and not isinstance(affected_systems, list):
    ...
    def __str__(self) -> str:
        ...
        return f"IncidentResponseManager({self.name}: {len(self.incidents)} incidents)"
    ...
    def _get_incident_by_id_cached(self, incident_id: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def _get_incidents_by_status_cached(self, status: str) -> List[Dict[str, Any]]:
        """
    ...
    def _get_incidents_by_priority_cached(self, priority: str) -> List[Dict[str, Any]]:
        """
    ...
    def _get_incidents_by_type_cached(self, incident_type: str) -> List[Dict[str, Any]]:
        """
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "incidents_by_status": self._get_incidents_by_status_cached.cache_info(),
            "incidents_by_priority": self._get_incidents_by_priority_cached.cache_info(),
            "incidents_by_type": self._get_incidents_by_type_cached.cache_info(),
        }
    ...
    async def create_incident_async(
        ...
        try:
            ...
            return self.create_incident(
                title, description, incident_type, priority, severity, affected_systems
            )
        except Exception as e:
            self.log_activity(f"Ошибка асинхронного создания инцидента: {e}", "error")
            return None
    ...
    async def get_incidents_async(
        ...
        try:
            ...
            self.log_activity(f"Ошибка асинхронного получения инцидентов: {e}", "error")
            return []
    ...
    async def update_incident_status_async(
        ...
        try:
            ...
            self.log_activity(f"Ошибка асинхронного обновления статуса: {e}", "error")
            return False
    ...
    async def initialize_async(self) -> bool:
        ...
        try:
            ...
            self.log_activity(f"Ошибка асинхронной инициализации: {e}", "error")
            return False
    ...
    def get_detailed_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for priority in IncidentPriority:
                count = len([i for i in self.incidents.values() if i.priority == priority])
                priority_metrics[priority.value] = count
            
            # Метрики по типам
            type_metrics = {}
            for incident_type in IncidentType:
                count = len([i for i in self.incidents.values() if i.incident_type == incident_type])
            ...
            return {
                "basic_metrics": {
                    ...
                    "open_rate": (open_incidents / total_incidents * 100) if total_incidents > 0 else 0,
                },
            ...
            self.log_activity(f"Ошибка получения детальных метрик: {e}", "error")
            return {}
    ...
    def _calculate_team_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "most_active_team": max(team_workload.items(), key=lambda x: x[1])[0] if team_workload else None,
                "team_efficiency": 85.5,  # Процент эффективности команды
            }
        except Exception:
            return {"team_workload": {}, "most_active_team": None, "team_efficiency": 0}
    ...
    def _calculate_escalation_metrics(self) -> Dict[str, Any]:
        ...
        try:
            escalated_count = len([
                i for i in self.incidents.values() 
                if i.priority in [IncidentPriority.HIGH, IncidentPriority.CRITICAL]
            ])
            
            total_incidents = len(self.incidents)
            escalation_rate = (escalated_count / total_incidents * 100) if total_incidents > 0 else 0
            
            return {
                "escalated_incidents": escalated_count,
                "escalation_rate": round(escalation_rate, 2),
                "high_priority_count": len([i for i in self.incidents.values() if i.priority == IncidentPriority.HIGH]),
                "critical_priority_count": len([i for i in self.incidents.values() if i.priority == IncidentPriority.CRITICAL]),
            }
        except Exception:
            return {"escalated_incidents": 0, "escalation_rate": 0, "high_priority_count": 0, "critical_priority_count": 0}
...
=== КОНЕЦ ОТЧЕТА ===
    ...
    def get_overdue_incidents(self, threshold_hours: float = 24.0) -> List[Incident]:
        """
    ...
    def export_incidents_to_csv(self, file_path: str) -> bool:
        ...
        try:
            ...
            self.log_activity(f"Экспорт инцидентов в {file_path} выполнен успешно")
            return True
        except Exception as e:
            ...
            with open(file_path, 'r', encoding='utf-8') as csvfile:
                ...
                for row in reader:
                    try:
                        ...
                        incident = Incident(
                            incident_id=row.get('incident_id', f"IMPORT-{int(time.time())}"),
                            title=row.get('title', ''),
                            description=row.get('description', ''),
                            incident_type=IncidentType(row.get('incident_type', 'UNKNOWN')),
                            priority=IncidentPriority(row.get('priority', 'LOW')),
                        ...
                        self.log_activity(f"Ошибка импорта строки: {e}", "error")
                        continue
            
            self.log_activity(f"Импортировано {imported_count} инцидентов из {file_path}")
            ...
            for incident_data in backup_data.get('incidents', []):
                try:
                    ...
                    if 'status' in incident_data:
                        incident.status = IncidentStatus(incident_data['status'])
                    
                    self.incidents[incident.incident_id] = incident
                    restored_count += 1
                    
                except Exception as e:
                    self.log_activity(f"Ошибка восстановления инцидента: {e}", "error")
                    continue
            
            self.log_activity(f"Восстановлено {restored_count} инцидентов из {backup_path}")
            return restored_count
        except Exception as e:
            self.log_activity(f"Ошибка восстановления из резервной копии: {e}", "error")
            return 0
