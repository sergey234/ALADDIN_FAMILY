...
class NetworkMonitoringService(SecurityBase):
    ...
    def __init__(
        self, name: str = "NetworkMonitoring", config: Optional[Dict[str, Any]] = None
    ):
    ...
    def get_network_statistics(
        ...
        try:
            if user_id:
                ...
                user_connections = [
                    conn for conn in self.connection_history if conn.user_id == user_id
                ]
            else:
                # Общая статистика
        ...
        try:
            status = {
                ...
                "family_specific_rules": len(
                    [r for r in self.monitoring_rules.values() if r.family_specific]
                ),
