...
class EnhancedSafeAnalyzer(SafeQualityAnalyzer):
    ...
    def generate_detailed_report(self, analysis_result: Dict[str, Any]) -> str:
        ...
        report.append("-" * 40)
        report.append(
            f"Функции безопасности сохранены: {'✅' if safety['security_functions_preserved'] else '❌'}"
        )
        report.append(
            f"Шифрование не затронуто: {'✅' if safety['encryption_intact'] else '❌'}"
        )
        report.append(
            f"Аутентификация сохранена: {'✅' if safety['authentication_preserved'] else '❌'}"
        )
        report.append(
            f"Валидация не затронута: {'✅' if safety['validation_intact'] else '❌'}"
        )
        report.append(
            f"Логирование сохранено: {'✅' if safety['logging_preserved'] else '❌'}"
        )
        report.append(
            f"Проблем безопасности найдено: {safety['security_issues_found']}"
        )
