...
class MFAService(SecurityBase):
    """
    Многофакторная аутентификация для семей
    Поддерживает различные методы аутентификации в зависимости от возраста
    и роли
    """
    ...
    def create_user_profile(
        ...
        try:
            ...
            event = SecurityEvent(
                ...
                description=f"Создан профиль MFA для пользователя {user_id} "
                f"с ролью {role.value}",
                source="MFAService",
            )
    ...
    def start_mfa_session(
        ...
        """
        try:
            # Обработка context по умолчанию
            if context is None:
                context = {}
    ...
    def send_mfa_code(self, session_id: str, method: MFAMethod) -> bool:
        ...
        try:
            ...
            if method not in session.methods:
                self.logger.warning(
                    f"Метод {method.value} не поддерживается для сессии "
                    f"{session_id}"
                )
                return False
            ...
            if success:
                ...
                event = SecurityEvent(
                    ...
                    severity=IncidentSeverity.LOW,
                    description=f"Отправлен код MFA методом {method.value} "
                ...
                self.logger.info(
                    f"Отправлен код MFA методом {method.value} "
                    f"для сессии {session_id}"
                )
                return True
            else:
        ...
        try:
            ...
            if self._verify_code(valid_code, code):
                ...
                event = SecurityEvent(
                    ...
                    severity=IncidentSeverity.LOW,
                    description=f"Подтвержден код MFA методом {method.value} "
                    f"для сессии {session_id}",
                    source="MFAService",
                )
                self.activity_log.append(event)
    ...
    def cleanup_expired_sessions(self) -> int:
        ...
        try:
            ...
            if expired_sessions or expired_codes:
                self.logger.info(
                    f"Очищено {len(expired_sessions)} сессий и "
                    f"{len(expired_codes)} кодов"
                )
    ...
    def _deliver_code(
        ...
        try:
            ...
            if method == MFAMethod.SMS and profile.phone_number:
                ...
                self.logger.info("Запрос биометрической аутентификации")
                return True
            elif method == MFAMethod.BACKUP_CODE:
                # Резервные коды уже есть у пользователя
                self.logger.info("Использован резервный код")
                return True
        ...
        try:
            ...
            if profile:
                ...
                event = SecurityEvent(
                    ...
                    description=f"Пользователь {user_id} заблокирован из-за "
                    f"превышения попыток MFA",
                    source="MFAService",
                )
                self.activity_log.append(event)

                self.logger.warning(
                    f"Пользователь {user_id} заблокирован до "
                    f"{profile.lock_until}"
                )
