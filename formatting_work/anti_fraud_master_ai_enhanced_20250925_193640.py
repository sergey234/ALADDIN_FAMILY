...
class AntiFraudMasterAI(SecurityBase):
    ...
    def __iter__(self):
        ...
        self._iter_index = 0
        self._iter_attrs = [
            'name', 'status', 'fraud_detections', 'blocked_attempts',
            'family_notifications', 'emergency_alerts', 'protected_amount'
        ]
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            ...
            self.logger.error(f"Ошибка при выходе из контекстного менеджера: {e}")
        finally:
            # Не вызываем shutdown автоматически, только логируем
            self.logger.info("Завершение контекстного менеджера AntiFraudMasterAI")
