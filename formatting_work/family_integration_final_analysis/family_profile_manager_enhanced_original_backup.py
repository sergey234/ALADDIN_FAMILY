...
class DashboardWidget:
    ...
    size: Dict[str, int] = field(default_factory=lambda: {"width": 200, "height": 150})
...
class FamilyProfileManagerEnhanced(SecurityBase):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Dashboard данные
        self.dashboards: Dict[str, Dict[str, Any]] = {}  # member_id -> dashboard_config
        self.widgets: Dict[str, List[DashboardWidget]] = {}  # member_id -> widgets
        self.notifications: Dict[str, List[FamilyNotification]] = {}  # family_id -> notifications
        self.quick_actions: Dict[str, List[Dict[str, Any]]] = {}  # family_id -> actions
        self.emergency_contacts: Dict[str, List[str]] = {}  # family_id -> contacts
...
# ==================== DASHBOARD METHODS ====================
    ...
    def add_family_member_dashboard(self, member_id: str, name: str, role: UserRole, 
                                   age: int = 0, avatar: str = "", 
        ...
        try:
            ...
            family_member = FamilyMember(
                ...
                name=name,
                age=age,
                role=FamilyRole(role.value),  # Конвертируем UserRole в FamilyRole
                age_group=self._determine_age_group(age),
    ...
    def _create_personal_dashboard(self, member_id: str, role: UserRole) -> None:
    ...
    def _create_default_widgets(self, member_id: str, role: UserRole) -> List[DashboardWidget]:
        """Создание виджетов по умолчанию"""
        widgets = []
        
        # Базовые виджеты для всех ролей
        base_widgets = [
            (WidgetType.SECURITY_STATUS, "Статус безопасности", {"x": 0, "y": 0}),
        ...
        if role == UserRole.PARENT:
            base_widgets.extend([
                (WidgetType.FAMILY_MEMBERS, "Члены семьи", {"x": 200, "y": 150}),
                (WidgetType.PARENTAL_CONTROLS, "Родительский контроль", {"x": 0, "y": 300})
            ])
        elif role == UserRole.CHILD:
            base_widgets.extend([
                (WidgetType.ACTIVITY_FEED, "Активность", {"x": 200, "y": 150})
            ])
        elif role == UserRole.ELDERLY:
            base_widgets.extend([
                (WidgetType.HEALTH_MONITOR, "Мониторинг здоровья", {"x": 200, "y": 150}),
                (WidgetType.EMERGENCY, "Экстренные функции", {"x": 0, "y": 300})
            ])
    ...
    def get_family_members_dashboard(self, family_id: str) -> List[Dict[str, Any]]:
    ...
    def get_notifications(self, member_id: Optional[str] = None, 
                         family_id: Optional[str] = None,
            ...
            if family_id and family_id in self.notifications:
                for notification in self.notifications[family_id]:
                    if unread_only and notification.read:
                        continue
                    if member_id and notification.target_member and notification.target_member != member_id:
                        continue
    ...
    def get_metrics_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "total_groups": sum(len(family.groups) for family in self.families.values()),
                "total_messages": len(self.messages),
                "active_families": len([f for f in self.families.values() if f.is_active]),
                "active_members": sum(len([m for m in family.members if m.is_active]) 
                                    for family in self.families.values()),
                "online_members": sum(len([m for m in family.members if m.is_online]) 
                                    for family in self.families.values())
            }
        except Exception as e:
...
# ==================== CHILD PROFILE METHODS ====================
    ...
    def add_child_profile(self, child_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if family_id and family_id in self.families:
                ...
                self.families[family_id].metadata["child_profiles"][child_id] = child_profile
    ...
    def get_child_profile(self, child_id: str, family_id: str) -> Optional[Dict[str, Any]]:
        """Получение профиля ребенка"""
        try:
            if family_id in self.families:
                child_profiles = self.families[family_id].metadata.get("child_profiles", {})
    ...
    def update_child_profile(self, child_id: str, family_id: str, updates: Dict[str, Any]) -> bool:
    ...
    def get_all_child_profiles(self, family_id: str) -> Dict[str, Dict[str, Any]]:
        """Получение всех профилей детей семьи"""
        try:
            if family_id in self.families:
                return self.families[family_id].metadata.get("child_profiles", {})
            return {}
        except Exception as e:
            self.log_activity(f"Ошибка получения профилей детей: {e}", "error")
            return {}

    def get_child_profiles_by_parent(self, parent_id: str, family_id: str) -> List[Dict[str, Any]]:
        """Получение профилей детей по родителю"""
        try:
            child_profiles = self.get_all_child_profiles(family_id)
            return [profile for profile in child_profiles.values() 
                   if profile.get("parent_id") == parent_id]
        except Exception as e:
            self.log_activity(f"Ошибка получения профилей по родителю: {e}", "error")
            return []

    def get_child_profiles_by_age_group(self, age_group: str, family_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.log_activity(f"Ошибка получения профилей по возрасту: {e}", "error")
            return []

    def search_child_profiles(self, query: str, family_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for profile in child_profiles.values():
                if (query_lower in profile.get("name", "").lower() or
                    query_lower in profile.get("age_group", "").lower()):
    ...
    def get_child_profile_stats(self, family_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "active_profiles": len([p for p in child_profiles.values() if p.get("is_active", True)]),
            ...
            for profile in child_profiles.values():
                ...
                stats["profiles_by_age_group"][age_group] = stats["profiles_by_age_group"].get(age_group, 0) + 1
                
                parent_id = profile.get("parent_id", "unknown")
                stats["profiles_by_parent"][parent_id] = stats["profiles_by_parent"].get(parent_id, 0) + 1
        ...
        try:
            ...
            for field in required_fields:
                if field not in child_data:
                    self.log_activity(f"Отсутствует обязательное поле: {field}", "error")
                    return False
