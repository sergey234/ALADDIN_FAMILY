...
class DeviceSecurityService(SecurityBase):
    ...
    def _scan_system_devices(self):
        ...
        try:
            ...
            self.log_activity(
                f"Обнаружено системное устройство: {device_profile.device_name}"
            )
            ...
            for software in device_profile.installed_software:
                if self._is_software_outdated(software):
                    vulnerability = SecurityVulnerability(
                        ...
                        remediation=f"Обновить {software} до последней версии",
                    )
            ...
            # Проверяем устройства пожилых
                ...
                if self._has_suspicious_activity(device_profile):
                    vulnerability = SecurityVulnerability(
                        ...
                        remediation="Проверить активность и усилить мониторинг",
                    )
                    vulnerabilities.append(vulnerability)
            return vulnerabilities
        except Exception as e:
        ...
        try:
            for action in rule.actions:
                if action == SecurityAction.SCAN:
                    ...
                    self.log_activity(
                        f"Обновление устройства: {device_profile.device_name}"
                    )
                elif action == SecurityAction.QUARANTINE:
                    self.quarantined_devices.add(device_profile.device_id)
                    self.log_activity(
                        f"Устройство помещено в карантин: {device_profile.device_name}"
                    )
                elif action == SecurityAction.BLOCK:
                    self.blocked_devices.add(device_profile.device_id)
                    self.log_activity(
                        f"Устройство заблокировано: {device_profile.device_name}"
                    )
                elif action == SecurityAction.ALERT:
                    self.log_activity(
                        f"Алерт безопасности: {rule.name} - {device_profile.device_name}"
                    )
