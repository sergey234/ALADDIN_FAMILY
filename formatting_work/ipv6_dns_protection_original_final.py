...
class IPv6DNSProtectionSystem(SecurityBase):
    ...
    def get_leak_statistics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "leak_types": {
                    leak_type.value: sum(1 for leak in self.leak_detections 
                                       if leak.leak_type == leak_type)
                    for leak_type in LeakType
                },
                "severity_distribution": {
                    severity: sum(1 for leak in self.leak_detections 
    ...
    def is_protection_active(self) -> bool:
        ...
        try:
            return (
                self.status == ComponentStatus.RUNNING and
                (self.dns_protection_enabled or 
                 self.ipv6_blocking_enabled or 
