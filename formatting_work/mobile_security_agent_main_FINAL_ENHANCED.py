...
class MobileSecurityAgentMain:
    ...
    def validate_threat_data(self, threat_data: Any) -> bool:
        ...
        try:
            ...
            required_fields = [
                ...
                'confidence', 'timestamp', 'device_id', 'details'
            ]
    ...
    def get_threat_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_confidence": sum(t.confidence for t in self.threats.values()) / len(self.threats) if self.threats else 0
            }
        except Exception as e:
        ...
        try:
            ...
            for threat_data in import_data.get("threats", []):
                try:
                    ...
                    self.logger.warning(f"Ошибка импорта угрозы {threat_data.get('threat_id', 'unknown')}: {e}")
            
            self.logger.info(f"Успешно импортировано {imported_count} угроз из {file_path}")
            return True
        except Exception as e:
    ...
    def update_security_policy(self, policy_name: str, policy_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for field in required_fields:
                if field not in policy_data:
                    self.logger.error(f"Отсутствует обязательное поле {field} в политике")
                    return False
