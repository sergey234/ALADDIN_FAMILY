...
class VoiceAnalysisEngine(SecurityBase):
    ...
    def _initialize_suspicious_phrases(self) -> Dict[str, List[str]]:
        ...
        return {
            "urgency": [
                "срочно", "немедленно", "прямо сейчас", "в течение часа",
                "сегодня", "завтра", "быстро", "скорее"
            ],
            "authority": [
                "ФСБ", "прокуратура", "суд", "полиция", "следственный комитет",
                "налоговая", "банк", "государство", "правительство"
            ],
            "financial": [
                "деньги", "карта", "счет", "перевод", "платеж", "рубли",
                "доллары", "евро", "криптовалюта", "биткоин"
            ],
            "threats": [
                "блокировка", "заморозка", "арест", "конфискация", "штраф",
                "уголовная ответственность", "суд", "тюрьма"
            ],
            "rewards": [
                "наследство", "выигрыш", "приз", "компенсация", "возврат",
                "бонус", "скидка", "подарок"
            ],
            "technical": [
                "вирус", "взлом", "обновление", "техподдержка", "программа",
    ...
    def _initialize_manipulation_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            "urgency_pressure": [
                "срочно", "немедленно", "прямо сейчас", "в течение часа",
                "последний шанс", "не упустите", "ограниченное время"
            ],
            "authority_appeal": [
                "я из", "представляю", "уполномочен", "имею право",
                "по приказу", "согласно закону", "от имени"
            ],
            "social_proof": [
                "все делают", "многие уже", "популярно", "рекомендуют",
                "используют", "доверяют", "выбирают"
            ],
            "scarcity_tactics": [
                "ограниченное количество", "последний", "единственный",
                "редкий", "эксклюзивный", "только для вас"
            ],
            "fear_appeal": [
                "опасно", "риск", "потеряете", "проблемы", "штраф",
                "блокировка", "арест", "уголовная ответственность"
            ],
            "guilt_tripping": [
                "вы должны", "обязаны", "не можете отказаться",
                "неправильно", "плохо", "стыдно"
            ],
            "love_bombing": [
                "забочусь", "хочу помочь", "для вашего блага",
                "люблю", "дорогой", "милый"
            ],
            "gaslighting": [
                "вы не помните", "забыли", "не понимаете", "ошибаетесь",
                "не так", "неправильно", "неверно"
            ]
        }
    ...
    async def _extract_voice_features(self, audio_data: bytes) -> VoiceFeatures:
        ...
        try:
            ...
            return VoiceFeatures(
                ...
                energy_mean=0, energy_std=0, zero_crossing_rate=0,
                mfcc_features=[0] * self.n_mfcc,
                spectral_centroid=0, spectral_rolloff=0, spectral_bandwidth=0,
                tempo=0, rhythm=0
            )
    ...
    async def _detect_manipulation(self, voice_features: VoiceFeatures) -> ManipulationIndicators:
        ...
        try:
            ...
            return ManipulationIndicators(
                urgency_pressure=0, authority_appeal=0, social_proof=0,
                scarcity_tactics=0, fear_appeal=0, guilt_tripping=0,
                love_bombing=0, gaslighting=0, total_manipulation_score=0
            )
