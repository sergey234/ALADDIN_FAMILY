    ...
    def _personalize_notification(self, notification: Notification) -> None:
        ...
        try:
            if notification.user_id in self.user_settings:
                ...
                current_hour = datetime.now().hour
                if (
                    settings.quiet_hours[0] <= current_hour
                    or current_hour < settings.quiet_hours[1]
                ):
    ...
    def search_notifications(
        ...
        try:
            ...
            for notification in self.notifications.values():
                ...
                if (
                    query_lower in notification.title.lower()
                    or query_lower in notification.message.lower()
                ):
    ...
    def process_scheduled_notifications(self) -> int:
        ...
        try:
            ...
            for notification in self.bot.notifications.values():
                if (
                    notification.status == NotificationStatus.PENDING
                    and "scheduled_for" in notification.metadata
                ):
