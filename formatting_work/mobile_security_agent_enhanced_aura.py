...
class MobileSecurityAgent(SecurityBase):
    ...
    # ============================================================================
    # НОВЫЕ МЕТОДЫ AURA: ROBOCALL FILTERING + CALLER ID PROTECTION
    # ============================================================================

    def robocall_detection(self, phone_number: str, call_data: dict) -> dict:
        """
        Обнаружение роботов-звонков с AI анализом
        ...
        Returns:
            dict: Результат анализа роботов-звонков
        """
        try:
            self.log_activity(f"Анализ роботов-звонков для номера: {phone_number}")
            ...
            # Анализ паттернов роботов-звонков
            robocall_indicators = []
            
            # 1. Анализ номера телефона
            if self._analyze_phone_number_pattern(phone_number):
                robocall_indicators.append("suspicious_number_pattern")
                result["confidence"] += 0.3
            
            # 2. Анализ времени звонка
            if "call_time" in call_data:
                if self._analyze_call_timing(call_data["call_time"]):
                    robocall_indicators.append("unusual_call_timing")
                    result["confidence"] += 0.2
            
            # 3. Анализ частоты звонков
            if "call_frequency" in call_data:
                if self._analyze_call_frequency(call_data["call_frequency"]):
                    robocall_indicators.append("high_call_frequency")
                    result["confidence"] += 0.25
            
            # 4. Анализ голосовых паттернов (если доступно)
            if "voice_analysis" in call_data:
                voice_result = self._analyze_voice_patterns(call_data["voice_analysis"])
                if voice_result["is_robotic"]:
                    robocall_indicators.append("robotic_voice")
                    result["confidence"] += 0.4
            
            # 5. Проверка базы данных известных роботов-звонков
            if self._check_robocall_database(phone_number):
                robocall_indicators.append("known_robocall_number")
                result["confidence"] += 0.5
            
            # 6. Анализ контекста звонка
            if "call_context" in call_data:
                context_result = self._analyze_call_context(call_data["call_context"])
                if context_result["is_suspicious"]:
                    robocall_indicators.append("suspicious_context")
                    result["confidence"] += 0.2
            
            # Определение результата
            result["is_robocall"] = result["confidence"] >= 0.6
            result["analysis_details"]["indicators"] = robocall_indicators
            result["analysis_details"]["total_indicators"] = len(robocall_indicators)
            ...
            if result["is_robocall"]:
                self.log_activity(f"🚨 РОБОТ-ЗВОНОК ОБНАРУЖЕН: {phone_number} (уверенность: {result['confidence']:.2f})")
            else:
                self.log_activity(f"✅ Звонок не является роботом: {phone_number}")
            
            return result
            
        except Exception as e:
            self.log_activity(f"❌ Ошибка анализа роботов-звонков: {str(e)}")
            return {
                "phone_number": phone_number,
                "is_robocall": False,
                "confidence": 0.0,
    ...
    def caller_id_verification(self, phone_number: str, caller_info: dict) -> dict:
        """
        Верификация Caller ID с проверкой на подделку
        
        Args:
            phone_number (str): Номер телефона
            caller_info (dict): Информация о звонящем
            
        Returns:
            dict: Результат верификации Caller ID
        """
        try:
            self.log_activity(f"Верификация Caller ID для номера: {phone_number}")
            ...
            if not self._validate_phone_format(phone_number):
                result["verification_status"] = "invalid_format"
                result["confidence"] = 0.0
                return result
            
            # 2. Проверка в базе данных известных номеров
            known_number_check = self._check_known_number_database(phone_number)
            if known_number_check["is_known"]:
                result["verification_status"] = "verified"
                result["confidence"] = 0.9
                result["verification_details"]["source"] = "known_database"
                return result
            
            # 3. Анализ Caller ID информации
            caller_id_analysis = self._analyze_caller_id_info(caller_info)
            
            # 4. Проверка на спуфинг
            spoofing_indicators = []
            
            # Проверка несоответствия информации
            if caller_id_analysis["info_mismatch"]:
                spoofing_indicators.append("information_mismatch")
                result["confidence"] += 0.3
            
            # Проверка географического несоответствия
            if caller_id_analysis["geo_mismatch"]:
                spoofing_indicators.append("geographical_mismatch")
                result["confidence"] += 0.25
            
            # Проверка временных паттернов
            if caller_id_analysis["time_pattern_suspicious"]:
                spoofing_indicators.append("suspicious_timing")
                result["confidence"] += 0.2
            
            # Проверка технических характеристик
            if caller_id_analysis["technical_suspicious"]:
                spoofing_indicators.append("technical_anomaly")
                result["confidence"] += 0.3
            
            # 5. Анализ репутации номера
            reputation_check = self._check_number_reputation(phone_number)
            if reputation_check["is_malicious"]:
                spoofing_indicators.append("malicious_reputation")
                result["confidence"] += 0.4
            
            # 6. Перекрестная проверка с другими источниками
            cross_verification = self._cross_verify_caller_id(phone_number, caller_info)
            if cross_verification["inconsistency_found"]:
                spoofing_indicators.append("cross_verification_failed")
                result["confidence"] += 0.35
            
            # Определение результата
            result["is_spoofed"] = result["confidence"] >= 0.6
            result["verification_details"]["indicators"] = spoofing_indicators
            result["verification_details"]["total_indicators"] = len(spoofing_indicators)
            
            # Определение статуса верификации
            if result["confidence"] >= 0.8:
                result["verification_status"] = "spoofed"
            elif result["confidence"] >= 0.6:
                result["verification_status"] = "likely_spoofed"
            ...
            if result["is_spoofed"]:
                self.log_activity(f"🚨 ПОДДЕЛКА CALLER ID ОБНАРУЖЕНА: {phone_number}")
            else:
                self.log_activity(f"✅ Caller ID верифицирован: {phone_number}")
            
            return result
            
        except Exception as e:
            self.log_activity(f"❌ Ошибка верификации Caller ID: {str(e)}")
            return {
                "phone_number": phone_number,
                "is_spoofed": False,
    ...
    def spam_call_blocking(self, phone_number: str, call_context: dict) -> bool:
        """
        Блокировка спам-звонков с машинным обучением
        
        Args:
            phone_number (str): Номер телефона
            call_context (dict): Контекст звонка
            
        Returns:
            bool: True если звонок должен быть заблокирован
        """
        try:
            self.log_activity(f"Анализ спам-звонка для номера: {phone_number}")
            
            # 1. Проверка в черном списке
            if self._check_blacklist(phone_number):
                self.log_activity(f"🚫 Звонок заблокирован (черный список): {phone_number}")
                return True
            
            # 2. Проверка в белом списке
            if self._check_whitelist(phone_number):
                self.log_activity(f"✅ Звонок разрешен (белый список): {phone_number}")
                return False
            
            # 3. Анализ роботов-звонков
            robocall_result = self.robocall_detection(phone_number, call_context)
            if robocall_result["is_robocall"] and robocall_result["threat_level"] in ["high", "critical"]:
                self.log_activity(f"🚫 Звонок заблокирован (робот): {phone_number}")
                return True
            
            # 4. Верификация Caller ID
            caller_id_result = self.caller_id_verification(phone_number, call_context.get("caller_info", {}))
            if caller_id_result["is_spoofed"] and caller_id_result["verification_status"] in ["spoofed", "likely_spoofed"]:
                self.log_activity(f"🚫 Звонок заблокирован (подделка): {phone_number}")
                return True
            
            # 5. Анализ контекста звонка
            context_analysis = self._analyze_call_context_for_spam(call_context)
            if context_analysis["is_spam"]:
                self.log_activity(f"🚫 Звонок заблокирован (спам-контекст): {phone_number}")
                return True
            
            # 6. Проверка репутации номера
            reputation_check = self._check_number_reputation(phone_number)
            if reputation_check["is_spam"]:
                self.log_activity(f"🚫 Звонок заблокирован (репутация): {phone_number}")
                return True
            
            # 7. Машинное обучение анализ
            ml_analysis = self._ml_spam_detection(phone_number, call_context)
            if ml_analysis["is_spam"] and ml_analysis["confidence"] >= 0.7:
                self.log_activity(f"🚫 Звонок заблокирован (ML анализ): {phone_number}")
                return True
            
            # Звонок разрешен
            self.log_activity(f"✅ Звонок разрешен: {phone_number}")
            return False
            
        except Exception as e:
            self.log_activity(f"❌ Ошибка анализа спам-звонков: {str(e)}")
            # В случае ошибки разрешаем звонок (fail-open)
            return False

    def phone_security_analysis(self, device_id: str) -> dict:
        """
        Анализ безопасности телефонных вызовов
        
        Args:
            device_id (str): ID устройства
            
        Returns:
            dict: Результат анализа безопасности звонков
        """
        try:
            self.log_activity(f"Анализ безопасности звонков для устройства: {device_id}")
            ...
            if device_id not in self.devices:
                result["error"] = f"Устройство {device_id} не найдено"
                return result
            
            device = self.devices[device_id]
            
            # 1. Анализ настроек безопасности
            security_settings = self._analyze_call_security_settings(device)
            result["protection_status"]["settings"] = security_settings
            
            # 2. Анализ истории звонков
            call_history = self._get_call_history(device_id)
            call_analysis = self._analyze_call_history(call_history)
            result["call_statistics"] = call_analysis
            
            # 3. Обнаружение угроз
            threats = []
            
            # Проверка на роботов-звонки
            robocall_threats = self._detect_robocall_threats(call_history)
            threats.extend(robocall_threats)
            
            # Проверка на подделку Caller ID
            spoofing_threats = self._detect_caller_id_spoofing(call_history)
            threats.extend(spoofing_threats)
            
            # Проверка на спам-звонки
            spam_threats = self._detect_spam_calls(call_history)
            threats.extend(spam_threats)
            
            result["threats_detected"] = threats
            
            # 4. Расчет балла безопасности
            security_score = self._calculate_call_security_score(security_settings, call_analysis, threats)
            result["security_score"] = security_score
            
            # 5. Генерация рекомендаций
            recommendations = self._generate_call_security_recommendations(security_score, threats)
            result["recommendations"] = recommendations
            
            # Логирование результата
            self.log_activity(f"📊 Анализ безопасности звонков завершен для {device_id}: {security_score:.2f}")
            
            return result
            
        except Exception as e:
            self.log_activity(f"❌ Ошибка анализа безопасности звонков: {str(e)}")
            return {
                "device_id": device_id,
                "error": str(e),
                "analysis_timestamp": datetime.now().isoformat()
    ...
    # ============================================================================
    # ВСПОМОГАТЕЛЬНЫЕ МЕТОДЫ ДЛЯ ROBOCALL FILTERING + CALLER ID PROTECTION
    # ============================================================================

    def _analyze_phone_number_pattern(self, phone_number: str) -> bool:
        """Анализ паттернов номера телефона на подозрительность"""
        try:
            # Проверка на последовательные цифры
            if self._has_sequential_pattern(phone_number):
                return True
            
            # Проверка на повторяющиеся цифры
            if self._has_repeating_pattern(phone_number):
                return True
            
            # Проверка на известные префиксы роботов-звонков
            if self._has_robocall_prefix(phone_number):
                return True
    ...
    def _analyze_call_timing(self, call_time: str) -> bool:
        """Анализ времени звонка на подозрительность"""
        try:
            # Проверка на необычное время (например, очень раннее утро)
            # Логика анализа времени
            return False
        except Exception:
            return False

    def _analyze_call_frequency(self, call_frequency: dict) -> bool:
        """Анализ частоты звонков"""
        try:
            # Проверка на высокую частоту звонков
            if call_frequency.get("calls_per_hour", 0) > 10:
                return True
            return False
        except Exception:
            return False

    def _analyze_voice_patterns(self, voice_data: dict) -> dict:
        ...
        try:
            ...
            return {"is_robotic": False, "confidence": 0.0}

    def _check_robocall_database(self, phone_number: str) -> bool:
        """Проверка в базе данных известных роботов-звонков"""
        try:
            # Здесь должна быть логика проверки базы данных
            return False
        except Exception:
            return False

    def _analyze_call_context(self, context: dict) -> dict:
    ...
    def _check_known_number_database(self, phone_number: str) -> dict:
        """Проверка в базе данных известных номеров"""
        try:
            return {
                "is_known": False,
                "source": "unknown",
    ...
    def _analyze_caller_id_info(self, caller_info: dict) -> dict:
        """Анализ информации Caller ID"""
        try:
            return {
                "info_mismatch": False,
                "geo_mismatch": False,
                "time_pattern_suspicious": False,
                "technical_suspicious": False
            }
        except Exception:
            return {
                "info_mismatch": False,
    ...
    def _check_number_reputation(self, phone_number: str) -> dict:
    ...
    def _cross_verify_caller_id(self, phone_number: str, caller_info: dict) -> dict:
        """Перекрестная проверка Caller ID"""
        try:
            return {
            ...
            return {"inconsistency_found": False, "confidence": 0.0}

    def _check_blacklist(self, phone_number: str) -> bool:
        """Проверка в черном списке"""
        try:
            # Здесь должна быть логика проверки черного списка
            return False
        except Exception:
            return False

    def _check_whitelist(self, phone_number: str) -> bool:
        """Проверка в белом списке"""
        try:
            # Здесь должна быть логика проверки белого списка
            return False
        except Exception:
            return False

    def _analyze_call_context_for_spam(self, context: dict) -> dict:
        """Анализ контекста звонка на спам"""
        try:
            return {
                "is_spam": False,
                "confidence": 0.0,
    ...
    def _ml_spam_detection(self, phone_number: str, call_context: dict) -> dict:
        """Машинное обучение для детекции спама"""
        try:
            return {
                "is_spam": False,
                "confidence": 0.0,
    ...
    def _analyze_call_security_settings(self, device) -> dict:
        """Анализ настроек безопасности звонков"""
        try:
            return {
                "call_blocking_enabled": True,
                "unknown_caller_blocking": True,
    ...
    def _get_call_history(self, device_id: str) -> list:
        """Получение истории звонков"""
        try:
            # Здесь должна быть логика получения истории звонков
            return []
        except Exception:
            return []

    def _analyze_call_history(self, call_history: list) -> dict:
    ...
    def _detect_robocall_threats(self, call_history: list) -> list:
        """Обнаружение угроз роботов-звонков"""
        try:
            return []
        except Exception:
            return []

    def _detect_caller_id_spoofing(self, call_history: list) -> list:
        """Обнаружение подделки Caller ID"""
        try:
            return []
        except Exception:
            return []

    def _detect_spam_calls(self, call_history: list) -> list:
        """Обнаружение спам-звонков"""
        try:
            return []
        except Exception:
            return []

    def _calculate_call_security_score(self, settings: dict, call_analysis: dict, threats: list) -> float:
        """Расчет балла безопасности звонков"""
        try:
            base_score = 100.0
            
            # Штрафы за угрозы
            threat_penalty = len(threats) * 10.0
            
            # Бонусы за настройки безопасности
            settings_bonus = 0.0
            if settings.get("call_blocking_enabled"):
                settings_bonus += 5.0
            if settings.get("spam_protection"):
                settings_bonus += 5.0
            
            final_score = max(0.0, base_score - threat_penalty + settings_bonus)
            return min(100.0, final_score)
        except Exception:
            return 50.0

    def _generate_call_security_recommendations(self, security_score: float, threats: list) -> list:
        """Генерация рекомендаций по безопасности звонков"""
        try:
            recommendations = []
            
            if security_score < 70:
                recommendations.append("Включить блокировку неизвестных звонков")
                recommendations.append("Активировать защиту от спама")
            
            if any("robocall" in str(threat).lower() for threat in threats):
                recommendations.append("Обновить базу данных роботов-звонков")
                recommendations.append("Включить детекцию роботизированных голосов")
            
            if any("spoofing" in str(threat).lower() for threat in threats):
                recommendations.append("Включить верификацию Caller ID")
                recommendations.append("Использовать перекрестную проверку номеров")
            
            return recommendations
        except Exception:
            return []

    def _has_sequential_pattern(self, phone_number: str) -> bool:
        """Проверка на последовательные цифры"""
        try:
    ...
    def _has_repeating_pattern(self, phone_number: str) -> bool:
        """Проверка на повторяющиеся цифры"""
        try:
            digits = ''.join(filter(str.isdigit, phone_number))
            for digit in '0123456789':
                if digits.count(digit) >= 4:
                    return True
            return False
        except Exception:
            return False

    def _has_robocall_prefix(self, phone_number: str) -> bool:
        """Проверка на известные префиксы роботов-звонков"""
        try:
            # Список известных префиксов роботов-звонков
            robocall_prefixes = ['555', '000', '111', '123']
            for prefix in robocall_prefixes:
