...
class NetworkMonitoringService(SecurityBase):
    ...
    def __init__(
        ...
        self.monitoring_config = (
            monitoring_config or NetworkMonitoringConfig()
        )
    ...
    def _validate_ip_address(self, ip: str) -> bool:
        ...
        if not ip or not isinstance(ip, str):
            raise InvalidIPAddressError(
                f"IP адрес не может быть пустым: {ip}"
            )
    ...
    async def detect_network_anomaly_async(
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка асинхронного обнаружения аномалии: {e}"
            )
            if self.metrics:
    ...
    async def get_network_statistics_async(
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка асинхронного получения статистики: {e}"
            )
            if self.metrics:
    ...
    async def _apply_rule_actions_async(
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка асинхронного применения действий: {e}"
            )
    ...
    async def _add_security_event_async(
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка асинхронного добавления события: {e}"
            )
        ...
        try:
            ...
            if rule.family_specific:
                if (
                    ...
                    and connection.metadata.get("user_age", 0) >= 18
                ):
                    return False
                elif (
                    rule.age_group == "elderly"
                    and connection.metadata.get("user_age", 0) < 65
                ):
                    return False
            # Проверка типа трафика
            if "traffic_type" in conditions:
                if (
                    connection.traffic_type.value
                    != conditions["traffic_type"]
                ):
                    return False
            # Проверка типа сети
            if "network_type" in conditions:
                if (
                    connection.network_type.value
            ...
            if (
                ...
                if self._is_inappropriate_content(
                    connection.destination_ip
                ):
                    return True
            # Проверка финансовых сайтов
            if (
                "financial_site" in conditions
                and conditions["financial_site"]
            ):
                if self._is_financial_site(connection.destination_ip):
                    return True
            # Проверка утечки данных
            if (
                "data_exfiltration" in conditions
                and conditions["data_exfiltration"]
            ):
        ...
        try:
            for action in rule.actions:
                if action == MonitoringAction.BLOCK:
                    ...
                    self.log_activity(
                        f"Уведомление родителям: {rule.name}"
                    )
    ...
    def get_family_network_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка получения статуса семейной сети: {e}"
            )
            return {}
    ...
    def enable_child_monitoring(self, user_id: str) -> bool:
        ...
        try:
            if user_id in self.family_network_history:
                self.child_monitoring_mode = True
                self.log_activity(
                    f"Включен детский мониторинг для {user_id}"
                )
                return True
            return False
        except Exception as e:
            self.logger.error(
                f"Ошибка включения детского мониторинга: {e}"
            )
            return False
    ...
    def disable_child_monitoring(self, user_id: str) -> bool:
        ...
        try:
            if user_id in self.family_network_history:
                ...
                self.log_activity(
                    f"Отключен детский мониторинг для {user_id}"
                )
                return True
            return False
        except Exception as e:
            self.logger.error(
                f"Ошибка отключения детского мониторинга: {e}"
            )
            return False
    ...
    def enable_elderly_monitoring(self, user_id: str) -> bool:
        ...
        try:
            if user_id in self.family_network_history:
                self.elderly_monitoring_mode = True
                self.log_activity(
                    f"Включен мониторинг пожилых для {user_id}"
                )
                return True
            return False
        except Exception as e:
    ...
    def disable_elderly_monitoring(self, user_id: str) -> bool:
        ...
        try:
            if user_id in self.family_network_history:
                ...
                self.log_activity(
                    f"Отключен мониторинг пожилых для {user_id}"
                )
                return True
            return False
        except Exception as e:
            self.logger.error(
                f"Ошибка отключения мониторинга пожилых: {e}"
            )
            return False

    def add_family_member(
        self, user_id: str, name: str, age: int, role: str
    ) -> bool:
    ...
    def set_parental_controls(
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка установки родительского контроля: {e}"
            )
            return False
    ...
    def get_parental_controls(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка получения родительского контроля: {e}"
            )
            return {}
    ...
    def block_website(
        ...
        try:
            ...
            self.log_activity(
                f"Заблокирован сайт: {url} (Причина: {reason})"
            )
            return True
        except Exception as e:
    ...
    def get_blocked_websites(self) -> Set[str]:
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка получения заблокированных сайтов: {e}"
            )
            return set()

    def set_time_limits(
        self, user_id: str, limits: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка установки временных ограничений: {e}"
            )
            return False
    ...
    def get_time_limits(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Ошибка получения временных ограничений: {e}"
            )
            return {}
    ...
    def send_parent_notification(
        ...
        try:
            self.log_activity(
                f"Уведомление родителям [{priority}]: {message}"
            )
            return True
        except Exception as e:
            self.logger.error(
                f"Ошибка отправки уведомления родителям: {e}"
            )
            return False
    ...
    def import_family_settings(self, settings: Dict[str, Any]) -> bool:
        ...
        try:
            self.log_activity(
                f"Импортированы семейные настройки: {settings}"
            )
            return True
        except Exception as e:
