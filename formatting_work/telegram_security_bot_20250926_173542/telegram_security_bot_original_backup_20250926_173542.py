...
active_telegram_chats = Gauge(
    ...
    'Number of active Telegram chats'
)
...
class TelegramSecurityBot(SecurityBase):
    ...
    def __init__(self, name: str = "TelegramSecurityBot",
    ...
    async def _load_monitored_chats(self) -> None:
        ...
        try:
            if self.db_session:
                chats = self.db_session.query(TelegramChat).filter(
                    TelegramChat.is_monitored
                ).all()
    ...
    def _get_recommended_action(self, threat_level: ThreatLevel, is_spam: bool,
                                is_bot_activity: bool, is_malicious: bool,
                                is_inappropriate: bool) -> ModerationAction:
            ...
            if chat_id:
                ...
                if self.db_session:
                    chat_messages = self.db_session.query(TelegramMessage).filter(
                        TelegramMessage.chat_id == chat_id).count()
