...
class ThreatIntelligenceMetrics:
    ...
    def __init__(self):
        ...
        self.iocs_analyzed = 0  # Добавлен недостающий атрибут
        self.iocs_by_type = {}
        self.unique_iocs = 0
    ...
    def to_dict(self):
        ...
        return {
            ...
            "iocs_analyzed": self.iocs_analyzed,
...
class ThreatIntelligenceAgent(SecurityBase):
    ...
    async def collect_russian_fraud_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Собрано новостных статей: "
                    f"{len(fraud_data['news_articles'])}"
                )
        ...
        try:
            ...
            for model_name, model in ml_models.items():
                try:
                    ...
                    logger.info(
                        f"Модель {model_name} обучена с точностью: "
                        f"{model_result['accuracy']:.2%}"
                    )
            ...
            self.log_activity(
                f"Обучено ML моделей: "
                f"{models_results['models_trained']}/{len(ml_models)}"
            )
    ...
    async def _train_model(
        ...
        # В реальной реализации здесь будет код обучения с использованием
        # scikit-learn, TensorFlow и т.д.
    ...
    async def generate_russian_fraud_report(
        ...
        try:
            report = {
                "report_id": (
                    f"RUS_FRAUD_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                ),
            ...
            self.log_activity(
                f"Сгенерирован отчет по российскому мошенничеству: "
                f"{report['report_id']}"
            )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        Example:
            ...
            >>> print(f"Угроз собрано: {metrics['threats_collected']}")
        """
        try:
            return {
                "threats_collected": len(self.threats),
                "iocs_analyzed": self.metrics.iocs_analyzed,
                "sources_active": len([
                    s for s in self.threat_sources.values() 
                    if s.get("active", False)
                ]),
                "last_collection_time": getattr(
                    self, "_last_collection_time", None
                ),
                "processing_time_avg": getattr(
                    self, "_avg_processing_time", 0.0
                ),
                "memory_usage_mb": getattr(self, "_memory_usage", 0.0),
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "max_threats": getattr(self, "_max_threats", 10000),
                "collection_interval": getattr(self, "_collection_interval", 3600),
                "analysis_enabled": getattr(self, "_analysis_enabled", True),
                "reporting_enabled": getattr(self, "_reporting_enabled", True),
            }
        except Exception as e:
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        Example:
            ...
            >>> print(f"Валидна: {validation['valid']}")
        """
        try:
            issues = []
            warnings = []
            ...
            if not self.threat_sources:
                issues.append("Отсутствуют источники угроз")

            if not hasattr(self, "_ai_models") or not self._ai_models:
                warnings.append("AI модели не инициализированы")

            if not hasattr(self, "_databases") or not self._databases:
                warnings.append("Базы данных не инициализированы")

            # Проверяем конфигурацию источников
            for source_name, source_config in self.threat_sources.items():
                if not source_config.get("url") and not source_config.get("api_key"):
                    issues.append(f"Источник {source_name}: отсутствует URL или API ключ")
            ...
            return {
                ...
                "models_count": len(getattr(self, "_ai_models", {})),
                "databases_count": len(getattr(self, "_databases", {})),
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_ok = getattr(self, "_memory_usage", 0) < 1000  # MB
            errors_low = getattr(self, "_error_count", 0) < 10

            overall_status = "healthy"
            if not sources_healthy:
                overall_status = "degraded"
            if not memory_ok or not errors_low:
    ...
    def test_connectivity(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Результаты тестов подключения

        Example:
            >>> agent = ThreatIntelligenceAgent()
            >>> connectivity = agent.test_connectivity()
            >>> print(f"Подключений: {connectivity['successful_connections']}")
        """
        try:
            results = {
            ...
            for source_name, source_config in self.threat_sources.items():
                try:
                    ...
                    if url:
                        ...
                        results["connection_details"][source_name] = {
                            "status": "success",
                            "response_time": 0.1,  # Заглушка
                        }
                    else:
    ...
    def export_report(self, report_type: str = "summary") -> Dict[str, Any]:
        """
        Экспорт отчета в различных форматах.

        Args:
            report_type: Тип отчета ("summary", "detailed", "json")
        ...
        Example:
            >>> agent = ThreatIntelligenceAgent()
            >>> report = agent.export_report("summary")
            >>> print(f"Отчет: {report['report_id']}")
        """
        try:
            base_report = {
                "report_id": f"TI_EXPORT_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    def import_config(self, config: Dict[str, Any]) -> bool:
        ...
        Args:
            config: Конфигурация для импорта
        ...
        Example:
            >>> agent = ThreatIntelligenceAgent()
            >>> config = {"sources": {...}}
            >>> success = agent.import_config(config)
        """
        try:
            if "sources" in config:
                self.threat_sources.update(config["sources"])

            if "ai_models" in config:
                if not hasattr(self, "_ai_models"):
                    self._ai_models = {}
                self._ai_models.update(config["ai_models"])

            if "databases" in config:
                if not hasattr(self, "_databases"):
                    self._databases = {}
                self._databases.update(config["databases"])
    ...
    def backup_data(self) -> Dict[str, Any]:
        ...
        Example:
            >>> agent = ThreatIntelligenceAgent()
            >>> backup = agent.backup_data()
            >>> print(f"Backup ID: {backup['backup_id']}")
        """
        try:
            backup_id = f"TI_BACKUP_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            backup_data = {
                ...
                "metrics": self.metrics.to_dict(),
                "sources": self.threat_sources,
                "ai_models": getattr(self, "_ai_models", {}),
                "databases": getattr(self, "_databases", {}),
            }

            logger.info(f"Создана резервная копия: {backup_id}")
            return backup_data
        except Exception as e:
    ...
    def restore_data(self, backup_data: Dict[str, Any]) -> bool:
        ...
        Example:
            >>> agent = ThreatIntelligenceAgent()
            >>> success = agent.restore_data(backup_data)
        """
        try:
            if "threats" in backup_data:
                self.threats = []
                for threat_data in backup_data["threats"]:
                    threat = ThreatIntelligence(**threat_data)
                    self.threats.append(threat)

            if "sources" in backup_data:
                self.threat_sources.update(backup_data["sources"])

            if "ai_models" in backup_data:
                if not hasattr(self, "_ai_models"):
                    self._ai_models = {}
                self._ai_models.update(backup_data["ai_models"])
    ...
    def reset_statistics(self) -> bool:
        ...
        Example:
            >>> agent = ThreatIntelligenceAgent()
            >>> success = agent.reset_statistics()
        """
        try:
            # Сбрасываем метрики
            self.metrics = ThreatIntelligenceMetrics()
