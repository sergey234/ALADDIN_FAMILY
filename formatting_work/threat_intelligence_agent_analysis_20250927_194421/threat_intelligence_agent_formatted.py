        ...
        try:
            return {
                ...
                "sources_active": len(
                    [
                        s
                        for s in self.threat_sources.values()
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for source_name, source_config in self.threat_sources.items():
                if not source_config.get("url") and not source_config.get(
                    "api_key"
                ):
                    issues.append(
                        f"Источник {source_name}: отсутствует URL или API ключ"
                    )
        ...
        try:
            ...
            if report_type == "summary":
                ...
                base_report.update(
                    {
                        "threats": [
                            threat.to_dict() for threat in self.threats
                        ],
                        "metrics": self.metrics.to_dict(),
