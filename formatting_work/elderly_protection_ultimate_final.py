...
class ElderlyProtection(SecurityBase):
    ...
    def analyze_email(
        self, elderly_id: str, sender_email: str, subject: str = "", content: str = ""
    ) -> Tuple[RiskLevel, ProtectionAction, str]:
        """
    ...
    def get_elderly_profile(self, elderly_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            high_risk_activities = sum(1 for a in activities if a.risk_level == RiskLevel.HIGH)
            blocked_activities = sum(1 for a in activities if a.action_taken == ProtectionAction.BLOCK)
            ...
            return {
                ...
                "last_activity": activities[-1].timestamp.isoformat() if activities else None,
                "risk_level": "HIGH" if high_risk_activities > total_activities * 0.3 else "LOW"
            }
        except Exception as e:
            self.logger.error(f"Ошибка получения профиля: {e}")
            return {"error": str(e)}

    def update_elderly_profile(self, elderly_id: str, profile_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            self.logger.info(f"Обновление профиля {elderly_id}: {profile_data}")
            return True
        except Exception as e:
    ...
    def get_threat_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "high_risk_percentage": risk_counts.get("high", 0) / max(sum(risk_counts.values()), 1) * 100
            }
        except Exception as e:
            self.logger.error(f"Ошибка получения статистики: {e}")
            return {"error": str(e)}

    def export_activity_data(self, elderly_id: str, format: str = "json") -> str:
        """
            ...
            if format == "json":
                ...
                for activity in activities:
                    data.append({
                        ...
                        "threat_detected": activity.threat_detected.value if activity.threat_detected else None,
                        "risk_level": activity.risk_level.value,
    ...
    def validate_contact(self, contact_info: str, contact_type: str = "phone") -> bool:
        """
    ...
    def is_blocked_contact(self, contact_info: str, contact_type: str = "phone") -> bool:
        """
    ...
    def get_risk_assessment(self, elderly_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            high_risk_count = sum(1 for a in activities if a.risk_level == RiskLevel.HIGH)
            medium_risk_count = sum(1 for a in activities if a.risk_level == RiskLevel.MEDIUM)
            total_activities = len(activities)
            
            risk_score = (high_risk_count * 3 + medium_risk_count * 1) / max(total_activities, 1)
    ...
    def restore_data(self) -> bool:
        ...
        try:
            ...
            self.elderly_activities = self._backup_data.get("elderly_activities", {})
            self.family_contacts = self._backup_data.get("family_contacts", {})
            self.scam_patterns = self._backup_data.get("scam_patterns", {})
            self.blocked_numbers = set(self._backup_data.get("blocked_numbers", []))
            self.blocked_emails = set(self._backup_data.get("blocked_emails", []))
            self.blocked_websites = set(self._backup_data.get("blocked_websites", []))
            self.trusted_contacts = set(self._backup_data.get("trusted_contacts", []))
            self.elderly_profiles = self._backup_data.get("elderly_profiles", {})
    ...
    def cleanup_old_activities(self) -> int:
        ...
        try:
            ...
            for elderly_id, activities in self.elderly_activities.items():
                ...
                removed_count += original_count - len(self.elderly_activities[elderly_id])
