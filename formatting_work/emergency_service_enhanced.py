...
class EmergencyService(SecurityBase):
    ...
    Основной класс для управления экстренными ситуациями в системе безопасности.
    Обеспечивает полный цикл обработки от создания события до вызова служб.
    ...
    Attributes:
        ...
        notification_manager (EmergencyNotificationManager): Менеджер уведомлений
    ...
    Note:
        Все методы поддерживают как синхронный, так и асинхронный режим работы.
        Рекомендуется использовать асинхронные методы для лучшей производительности.
    """
    ...
    def _validate_emergency_event_params(
        ...
        if not isinstance(emergency_type, EmergencyType):
            raise TypeError(f"emergency_type должен быть EmergencyType, получен {type(emergency_type)}")
        
        # Проверка серьезности
        if not isinstance(severity, EmergencySeverity):
            raise TypeError(f"severity должен быть EmergencySeverity, получен {type(severity)}")
        
        # Проверка местоположения
        if not isinstance(location, dict):
            raise TypeError(f"location должен быть dict, получен {type(location)}")
        ...
        if not isinstance(description, str):
            raise TypeError(f"description должен быть str, получен {type(description)}")
        
        if not description.strip():
            raise ValueError("description не может быть пустым")
        
        if len(description) < 10:
            raise ValueError("description должен содержать минимум 10 символов")
        
        # Проверка user_id (если предоставлен)
        if user_id is not None:
            if not isinstance(user_id, str):
                raise TypeError(f"user_id должен быть str, получен {type(user_id)}")
    ...
    def _validate_contact_params(
        ...
        if email is not None:
            if not isinstance(email, str):
                raise TypeError(f"email должен быть str, получен {type(email)}")
        ...
        if not isinstance(relationship, str):
            raise TypeError(f"relationship должен быть str, получен {type(relationship)}")
        ...
        if not isinstance(priority, int):
            raise TypeError(f"priority должен быть int, получен {type(priority)}")
    ...
    async def create_emergency_event_async(
        ...
        try:
            ...
            self.logger.info(f"Асинхронно обработано экстренное событие {event.event_id}")
            return event

        except Exception as e:
            self.logger.error(f"Ошибка асинхронного создания экстренного события: {e}")
            raise
    ...
    async def _get_contacts_async(self, event: EmergencyEvent) -> List[EmergencyContact]:
    ...
    async def _call_emergency_services_async(self, event: EmergencyEvent) -> List[EmergencyResponse]:
    ...
    def add_emergency_contact(
        ...
        """
        # Валидация параметров
        self._validate_contact_params(name, phone, email, relationship, priority)
