...
# import matplotlib.pyplot as plt  # Не используется в текущей реализации
import numpy as np
import pandas as pd
import requests
...
class AlertManager:
    ...
    def __init__(
        ...
        self.alert_timeout = self.config.get('alert_timeout', 300)  # 5 минут
    ...
    async def send_alert(
        ...
        try:
            ...
            if not isinstance(source, str):
                raise ValueError("source должен быть строкой")
            if recipients is not None and not isinstance(recipients, list):
                raise ValueError("recipients должен быть списком или None")
            if channels is not None and not isinstance(channels, list):
                raise ValueError("channels должен быть списком или None")
            if template_name is not None and not isinstance(template_name, str):
                raise ValueError("template_name должен быть строкой или None")
            if metadata is not None and not isinstance(metadata, dict):
                raise ValueError("metadata должен быть словарем или None")
    ...
    async def add_recipient(self, recipient: AlertRecipient) -> None:
        ...
        try:
            # Валидация параметров
            if not isinstance(recipient, AlertRecipient):
                raise ValueError("recipient должен быть экземпляром AlertRecipient")
            if not recipient.user_id or not isinstance(recipient.user_id, str):
                raise ValueError("recipient.user_id должен быть непустой строкой")
            if not recipient.name or not isinstance(recipient.name, str):
                raise ValueError("recipient.name должен быть непустой строкой")
