...
class ParentalControls(SecurityBase):
    ...
    def emergency_lock_child_device(self, child_id: str, reason: str) -> bool:
        ...
        try:
            ...
            self.logger.critical(
                f"Экстренная блокировка устройства ребенка {child_id}: {reason}"
            )
            return True
    ...
    def activate_kill_switch(
        ...
        try:
            ...
            if settings.get("auto_kill_on_vpn_disconnect", False):
                ...
                self.logger.info(
                    f"Kill Switch активирован для ребенка {child_id}: {reason}"
                )
    ...
    def deactivate_kill_switch(self, child_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Kill Switch деактивирован для ребенка {child_id}"
            )
