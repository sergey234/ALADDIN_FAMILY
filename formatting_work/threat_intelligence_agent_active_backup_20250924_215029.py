...
class ThreatIntelligenceAgent(SecurityBase):
    ...
    async def collect_russian_fraud_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                fraud_data['cbr_reports'] = cbr_collector.collect_fraud_reports(max_pages=5)
                fraud_data['statistics']['cbr'] = cbr_collector.collect_statistics()
                fraud_data['collection_stats']['cbr'] = cbr_collector.get_collection_stats()
                cbr_collector.close()
                
                logger.info(f"Собрано отчетов ЦБ РФ: {len(fraud_data['cbr_reports'])}")
            ...
            try:
                ...
                fraud_data['news_articles'] = news_scraper.collect_news_data(max_sources=3)
                fraud_data['collection_stats']['news'] = news_scraper.get_collection_stats()
                news_scraper.close()
                
                logger.info(f"Собрано новостных статей: {len(fraud_data['news_articles'])}")
                
            except Exception as e:
                logger.error(f"Ошибка сбора новостных данных: {e}")
            
            # Обновление метаданных
            fraud_data['metadata']['total_reports'] = len(fraud_data['cbr_reports'])
            fraud_data['metadata']['total_articles'] = len(fraud_data['news_articles'])
            ...
            raise ThreatIntelligenceError(f"Не удалось собрать российские данные: {e}")

    async def train_russian_ml_models(self, fraud_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            models_results['metadata']['total_training_samples'] = len(training_data)
            ...
            for model_name, model in ml_models.items():
                try:
                    ...
                    model_result = await self._train_model(model, training_data, model_name)
                    models_results['models_status'][model_name] = model_result['status']
                    models_results['accuracy_metrics'][model_name] = model_result['accuracy']
                    models_results['training_stats'][model_name] = model_result['stats']
                    
                    if model_result['status'] == 'success':
                        models_results['models_trained'] += 1
                    
                    logger.info(f"Модель {model_name} обучена с точностью: {model_result['accuracy']:.2%}")
    ...
    def _prepare_training_data(self, fraud_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Подготовлено {len(training_samples)} образцов для обучения")
    ...
    def _determine_severity(self, data: Dict[str, Any]) -> str:
        ...
        fraud_types = data.get('fraud_type', []) or data.get('fraud_indicators', [])
        content = data.get('content', '') or data.get('text', '')
        
        high_severity_keywords = ['критический', 'массовое', 'системное', 'банковское']
    ...
    async def _train_model(self, model: Dict[str, Any], training_data: List[Dict], model_name: str) -> Dict[str, Any]:
        """Обучение ML модели
    ...
    async def _save_ml_models(self, models: Dict[str, Any], results: Dict[str, Any]) -> None:
    ...
    async def analyze_russian_fraud_trends(self, fraud_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            regional_count = {}
            all_data = fraud_data.get('cbr_reports', []) + fraud_data.get('news_articles', [])
            ...
            # Генерация рекомендаций
            trends_analysis['recommendations'] = self._generate_fraud_recommendations(
                fraud_types_count, regional_count
            )
            
            trends_analysis['metadata']['total_records_analyzed'] = len(all_data)
            
            logger.info(f"Анализ трендов завершен. Обработано {len(all_data)} записей")
            
            return trends_analysis
            
        except Exception as e:
            logger.error(f"Ошибка анализа трендов: {e}")
            raise ThreatIntelligenceError(f"Не удалось проанализировать тренды: {e}")

    def _generate_fraud_recommendations(self, fraud_types: Dict[str, int], regions: Dict[str, int]) -> List[str]:
        ...
        if 'banking_fraud' in fraud_types and fraud_types['banking_fraud'] > 15:
            recommendations.append(
                "Улучшить банковскую безопасность: "
                "двухфакторная аутентификация, мониторинг транзакций"
            )
        
        if 'internet_fraud' in fraud_types and fraud_types['internet_fraud'] > 20:
            recommendations.append(
    ...
    async def generate_russian_fraud_report(self, fraud_data: Dict[str, Any], trends: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            report['executive_summary'] = {
                ...
                'top_fraud_type': max(trends['fraud_types_trends'], key=trends['fraud_types_trends'].get) if trends['fraud_types_trends'] else 'неизвестно',
                'most_affected_region': max(trends['regional_trends'], key=trends['regional_trends'].get) if trends['regional_trends'] else 'неизвестно'
            }
            ...
            self.log_activity(f"Сгенерирован отчет по российскому мошенничеству: {report['report_id']}")
            
            return report
            
        except Exception as e:
            logger.error(f"Ошибка генерации отчета: {e}")
            raise ThreatIntelligenceError(f"Не удалось сгенерировать отчет: {e}")
