...
active_instagram_accounts = Gauge(
    ...
    'Number of active Instagram accounts'
)
...
class InstagramSecurityBot(SecurityBase):
    ...
    def __init__(self, name: str = "InstagramSecurityBot",
    ...
    async def _load_monitored_accounts(self) -> None:
        ...
        try:
            if self.db_session:
                accounts = self.db_session.query(InstagramUser).filter(
                    InstagramUser.is_blocked.is_(False)
                ).all()
    ...
    async def analyze_content(
            self, content_data: Dict[str, Any]) -> ContentAnalysisResult:
        ...
        try:
            ...
            recommended_action = self._get_recommended_action(
                threat_level, is_inappropriate, is_bullying, is_fake,
                is_spam, is_deepfake, is_copyright_violation
            )
    ...
    async def _analyze_text_content(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            inappropriate_keywords = [
                "порно", "xxx", "секс", "наркотики", "наркотик",
                "оружие", "взрывчатка", "террор", "экстремизм",
                "насилие", "жестокость", "самоубийство"
            ]
            ...
            bullying_keywords = [
                "тупой", "идиот", "дебил", "урод", "жирный",
                "уродливый", "ненавижу", "убий", "умри"
            ]
            ...
            # Проверка на спам
            spam_indicators = [
                "реклама", "заработок", "быстро", "легко", "бесплатно",
                "срочно", "только сегодня", "не упустите", "гарантия",
                "подписывайтесь", "лайкайте", "репостите"
            ]
            ...
            if positive_count > negative_count:
                ...
                sentiment_score = -0.5 - \
                    (negative_count - positive_count) * 0.1
    ...
    async def _analyze_account(
            self, user_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_recommended_action(
            ...
            is_spam: bool,
            is_deepfake: bool,
            is_copyright_violation: bool) -> ModerationAction:
    ...
    async def _log_content_analysis(
        ...
        try:
            ...
            post = InstagramPost(
                ...
                user_id=content_data.get(
                    "user",
                    {}).get(
                    "id",
                    ""),
                username=content_data.get(
                    "user",
                    {}).get("username"),
                ...
                caption=content_data.get(
                    "caption",
                    ""),
                media_url=content_data.get(
                    "media_url",
                        ""),
                media_type=content_data.get(
                    "media_type",
                    ""),
                threat_level=result.threat_level.value,
                is_deleted=result.recommended_action in [
    ...
    async def add_account_to_monitoring(
            self, user_data: Dict[str, Any]) -> bool:
    ...
    async def get_security_report(
        ...
        try:
            ...
            if user_id:
                ...
                if self.db_session:
                    user_posts = self.db_session.query(InstagramPost).filter(
