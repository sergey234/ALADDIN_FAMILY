...
class DeviceSecurityService(SecurityBase):
    ...
    def measure_performance(func: Callable) -> Callable:
        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                self.logger.debug(f"{func.__name__} выполнен за {execution_time:.4f}с")
                return result
            except Exception as e:
                execution_time = time.time() - start_time
                self.logger.error(f"{func.__name__} завершился с ошибкой за {execution_time:.4f}с: {e}")
                raise
        return wrapper
    ...
    async def scan_device_security_async(self, device_id: str) -> DeviceSecurityReport:
        ...
        try:
            ...
            device.vulnerabilities = [v.vulnerability_id for v in vulnerabilities]
            ...
            # Логируем результат
            self.logger.info(f"Асинхронное сканирование завершено для {device_id}: {security_score:.2f}")
            
            return report
            
        except Exception as e:
            self.logger.error(f"Ошибка асинхронного сканирования {device_id}: {e}")
            raise
    ...
    async def get_performance_metrics_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            device_count_task = loop.run_in_executor(None, self.get_device_count)
            vuln_count_task = loop.run_in_executor(None, self.get_vulnerability_count)
            rules_count_task = loop.run_in_executor(None, self.get_security_rules_count)
            ...
            avg_score_task = loop.run_in_executor(None, self._calculate_average_security_score)
            ...
            return {
                ...
                "last_scan_time": max([p.last_scan for p in self.device_profiles.values() if p.last_scan], default=None),
    ...
    async def register_device_async(
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            system_info_task = loop.run_in_executor(None, self._get_system_device_info)
            ...
            await loop.run_in_executor(None, self._register_device_sync, device_profile)
            
            self.logger.info(f"Устройство {device_name} асинхронно зарегистрировано: {device_id}")
            return device_id
            
        except Exception as e:
            self.logger.error(f"Ошибка асинхронной регистрации устройства: {e}")
            raise
    ...
    def _calculate_average_security_score(self) -> float:
        ...
        if not self.device_profiles:
            return 0.0
        return sum(p.security_score for p in self.device_profiles.values()) / len(self.device_profiles)
    ...
    def register_device(
        ...
        Args:
            ...
            operating_system: Операционная система (например, "iOS", "Android", "Windows")
            os_version: Версия операционной системы (например, "17.0", "13.0", "11")
            user_id: ID пользователя для семейной привязки (опционально)
            family_role: Роль в семье - "parent", "child", "elderly" (опционально)
            
        Returns:
            DeviceProfile: Профиль зарегистрированного устройства с метаданными
    ...
    def scan_device_security(self, device_id: str) -> DeviceSecurityReport:
        ...
        Returns:
            DeviceSecurityReport: Подробный отчет о безопасности устройства, включающий:
        ...
        Example:
            ...
            >>> profile = service.register_device("iPhone 15", DeviceType.MOBILE, "iOS", "17.0")
            >>> # Затем сканируем его
        ...
        Note:
            - Результаты сканирования кэшируются для повышения производительности
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "family_protection_enabled": self.family_protection_enabled,
                "last_scan_time": max([p.last_scan for p in self.device_profiles.values() if p.last_scan], default=None),
                "average_security_score": sum([p.security_score for p in self.device_profiles.values()]) / len(self.device_profiles) if self.device_profiles else 0.0
            }
        except Exception as e:
    ...
    def get_vulnerabilities_by_device(self, device_id: str) -> List[SecurityVulnerability]:
        """Получение уязвимостей устройства"""
        try:
            return [v for v in self.security_vulnerabilities if v.device_id == device_id]
        except Exception as e:
            self.logger.error(f"Ошибка получения уязвимостей устройства {device_id}: {e}")
            return []
    ...
    def get_security_score(self, device_id: str) -> float:
        ...
        try:
            ...
            self.logger.error(f"Ошибка получения балла безопасности {device_id}: {e}")
            return 0.0
    ...
    def get_recommendations(self, device_id: str) -> List[str]:
        ...
        try:
            ...
            if vulnerabilities:
                recommendations.append(f"Обнаружено {len(vulnerabilities)} уязвимостей")
            ...
            self.logger.error(f"Ошибка получения рекомендаций {device_id}: {e}")
            return []
    ...
    def import_config(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            if "family_protection_settings" in config:
                self.family_protection_settings.update(config["family_protection_settings"])
    ...
    def restore_data(self, backup_data: Dict[str, Any]) -> bool:
        ...
        try:
            if "device_profiles" in backup_data:
                ...
                self.logger.info(f"Восстановление {len(backup_data['device_profiles'])} профилей устройств")
            
            if "family_protection_settings" in backup_data:
                self.family_protection_settings.update(backup_data["family_protection_settings"])
    ...
    def send_security_alert(
        ...
        Args:
            alert_type: Тип алерта ("vulnerability", "threat", "compliance", "system")
    ...
    def send_family_notification(
        ...
        Args:
            ...
            notification_type: Тип уведомления ("security", "update", "warning")
            message: Текст сообщения
            device_id: ID устройства (опционально)
        ...
        try:
            notification = {
                ...
                "device_id": device_id,
                "timestamp": datetime.now().isoformat(),
                "family_role": self._get_user_family_role(user_id)
            }
            
            self.logger.info(f"👨‍👩‍👧‍👦 FAMILY NOTIFICATION [{user_id}]: {message}")
    ...
    def get_method_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "total_methods": len([m for m in dir(self) if callable(getattr(self, m)) and not m.startswith('_')]),
