...
class AdvancedMonitoringManager(SecurityBase):
    ...
    производительности и API взаимодействий. Поддерживает асинхронную обработку,
    адаптивные пороги, управление алертами и экспорт данных.
    ...
    def add_metric(
        ...
        try:
            ...
            self.logger.log("ERROR", f"Неожиданная ошибка добавления метрики: {e}")
            return False
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            self.log_activity(
                f"Выход из контекстного менеджера с ошибкой: {exc_type.__name__}",
                "error"
            )
        else:
