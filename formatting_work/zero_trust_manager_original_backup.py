...
class KeyDerivation:
    ...
    def __str__(self) -> str:
        ...
        return f"KeyDerivation(algorithm='{self.algorithm}', length={self.length}, iterations={self.iterations})"

    def __repr__(self) -> str:
        """Представление объекта для отладки"""
        return f"KeyDerivation(algorithm='{self.algorithm}', length={self.length}, salt_length={len(self.salt)}, iterations={self.iterations})"
...
class ZeroTrustManager:
    ...
    def __str__(self) -> str:
        """Строковое представление объекта"""
        return f"ZeroTrustManager(name='{self.name}', running={self.is_running}, devices={len(self.devices)}, users={len(self.users)})"

    def __repr__(self) -> str:
        """Представление объекта для отладки"""
        return f"ZeroTrustManager(name='{self.name}', is_running={self.is_running}, devices_count={len(self.devices)}, users_count={len(self.users)})"
    ...
    def cleanup_expired_requests(self, max_age_hours: int = 24):
        ...
        self.logger.info(f"Очищено {len(expired_requests)} просроченных запросов")
    ...
    def export_data(self) -> Dict[str, any]:
        ...
        return {
            ...
            "policies": {k: v.__dict__ for k, v in self.security_policies.items()},
            "stats": self.stats,
