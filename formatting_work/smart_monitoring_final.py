...
class SmartMonitoringSystem:
    ...
    async def _cleanup_old_data_async(self) -> None:
        ...
        try:
            with self.lock:
                ...
                print(
                    f"Асинхронная очистка: удалено {removed_alerts} старых алертов"
                )
        except Exception as e:
    ...
    def _validate_metric_name(self, metric_name: str) -> None:
        ...
        for keyword in sql_keywords:
            if keyword in metric_upper:
                raise ValueError(
                    f"metric_name содержит потенциально опасный SQL ключевое слово: {keyword}"
                )
    ...
    def _validate_tags(self, tags: Optional[Dict[str, str]]) -> None:
        ...
        for key, value in tags.items():
            ...
            if len(key) > 100 or len(value) > 500:
                raise ValueError(
                    "Ключи тегов не должны превышать 100 символов, значения - 500"
                )
    ...
    def _validate_alert_id(self, alert_id: str) -> None:
        ...
        if "_" not in alert_id:
            raise ValueError(
                "alert_id должен содержать символ '_' для разделения rule_id и timestamp"
            )
    ...
    def _cleanup_memory(self) -> None:
        ...
        try:
            with self.lock:
                ...
                removed_alerts = old_count - len(self.alerts)
                print(
                    f"Принудительная очистка памяти: удалено {removed_alerts} алертов"
                )
