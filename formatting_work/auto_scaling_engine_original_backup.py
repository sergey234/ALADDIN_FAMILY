...
class AutoScalingEngine(SecurityBase):
        ...
        try:
            ...
            if exc_type:
                ...
                if hasattr(exc_val, '__await__'):
                    ...
                    error_msg = str(
                        exc_val) if exc_val else "Неизвестная ошибка"
    ...
    async def make_scaling_decision_async(
            ...
            confidence_threshold: float = 0.7) -> Optional[ScalingDecision]:
        ...
        if not isinstance(
                confidence_threshold, (int, float)) or not (
        ...
        try:
            with self.scaling_lock:
                start_time = time.time()
                service_rules = [r for r in self.scaling_rules.values(
                ) if r.service_id == service_id and r.enabled]
                if not service_rules:
                ...
                if decision:
                    ...
                    self.statistics["last_decision"] = datetime.now()
                    decision_time = time.time() - start_time
                    self.statistics["average_decision_time"] = (self.statistics["average_decision_time"] * (
                        self.statistics["total_decisions_made"] - 1) + decision_time) / self.statistics["total_decisions_made"]
                    self.performance_metrics.average_decision_time = self.statistics[
    ...
    async def _get_service_metrics_async(
            self, service_id: str) -> List[MetricData]:
        ...
        try:
            ...
            for key, metric_list in self.metric_history.items():
                if key.startswith(f"{service_id}_"):
                    recent_metrics = metric_list[-10:] if len(
                        metric_list) > 10 else metric_list
                    metrics.extend(recent_metrics)
    ...
    def _is_cache_valid(self, key: str, ttl_seconds: int = 300) -> bool:
        ...
        return (
            datetime.now() -
            self._cache_ttl[key]).total_seconds() < ttl_seconds
