            ...
            try:
                if self._is_ddos_attack(flow):
                    attempt = IntrusionAttempt(
                        ...
                        threat_level=ThreatLevel.CRITICAL,
                        timestamp=time.time(),
                        payload=f"High packet rate: {flow.packets_sent} "
                        f"packets/sec",
                        user_agent="unknown",
                        session_id="unknown",
                    )
    ...
    def check_rate_limit(
        ...
        try:
            ...
            if not source_ip or not isinstance(source_ip, str):
                print(
                    "⚠️ Предупреждение: Некорректный IP адрес для rate limit"
                )
                return False
    ...
    def is_ip_blocked(self, ip: str) -> bool:
        ...
        try:
            ...
            if not ip or not isinstance(ip, str):
                print(
                    "⚠️ Предупреждение: Некорректный IP адрес для "
                    "проверки блокировки"
                )
                return False
    ...
    def get_recent_attempts(self, hours: int = 24) -> List[IntrusionAttempt]:
        ...
        try:
            ...
            if not isinstance(hours, int) or hours < 0:
                print(
                    "⚠️ Предупреждение: Некорректное количество часов, "
                    "используется значение по умолчанию"
                )
                hours = 24
            ...
            if hours > 8760:  # Максимум 1 год
                print(
                    "⚠️ Предупреждение: Слишком большое количество часов, "
                    "ограничено до 8760 (1 год)"
                )
                hours = 8760
