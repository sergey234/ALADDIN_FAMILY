...
class Incident:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "incident_type": self.incident_type.value if hasattr(
                self.incident_type,
                'value') else str(
                self.incident_type),
            "severity": self.severity.value if hasattr(
                self.severity,
                'value') else str(
                    self.severity),
            "status": self.status.value if hasattr(
                self.status,
                'value') else str(
                self.status),
...
class IncidentResponseAgent(SecurityBase):
    ...
    def _initialize_ai_models(self):
        ...
        try:
            ...
            self.incident_classifier = {
                ...
                "features": [
                    "incident_description", "indicators", "affected_systems",
            ...
            self.severity_predictor = {
                ...
                "features": [
                    ...
                    "attack_complexity", "impact_potential", "temporal_factors"
                ],
                "accuracy": 0.91,
            ...
            self.escalation_predictor = {
                ...
                "features": [
                    "incident_age", "severity", "resolution_progress",
                    "resource_availability", "escalation_history", "sla_status"
                ],
                "accuracy": 0.87,
            ...
            self.impact_analyzer = {
                ...
                "features": [
                    "affected_systems", "data_exposure", "business_impact",
                    "recovery_time", "financial_impact", "reputation_risk"
                ],
                "accuracy": 0.92,
                "confidence_threshold": 0.80,
                "last_trained": datetime.now()
    ...
    def _update_metrics(self, incident, action):
        ...
        try:
            if action == "created":
                ...
                incident_type = incident.incident_type.value if hasattr(
    ...
    def _send_notification(self, incident, recipient, channel):
        ...
        try:
            self.log_activity("Отправка уведомления {} на {}: {}".format(
        ...
        try:
            ...
            if self.metrics.sla_compliance < 0.9:
                recommendations.append({
                    ...
                    "action": "Пересмотреть SLA и улучшить автоматизацию"
                })
            ...
            if self.metrics.false_positives > self.metrics.total_incidents * 0.1:
                recommendations.append({
                    ...
                    "action": "Улучшить алгоритмы классификации инцидентов"
                })
    ...
    def _save_data(self):
        ...
        try:
            ...
            with open(incidents_file, 'w') as f:
                json.dump({iid: incident.to_dict()
