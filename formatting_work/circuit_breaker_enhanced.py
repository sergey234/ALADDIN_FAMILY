...
class SmartCircuitBreaker:
    ...
    def __init__(
        ...
        self.call_history: List[bool] = []  # История вызовов (успех/ошибка)
        self.error_rates: List[float] = []  # История процента ошибок
        self.response_times: List[float] = []  # Время ответа

        # Защита от спама
        self.last_alert_time: Optional[float] = None
        self.alert_cooldown: int = 300  # 5 минут между алертами
    ...
    def __str__(self) -> str:
        ...
        return f"SmartCircuitBreaker(name='{self.name}', state={self.state.value}, failures={self.failure_count}, successes={self.success_count})"

    def __repr__(self) -> str:
        """Детальное представление Circuit Breaker"""
        return f"SmartCircuitBreaker(name='{self.name}', state={self.state.value}, failure_count={self.failure_count}, success_count={self.success_count}, config={self.config})"

    def __eq__(self, other) -> bool:
        """Сравнение Circuit Breaker по имени"""
        if not isinstance(other, SmartCircuitBreaker):
            return False
        return self.name == other.name

    def __hash__(self) -> int:
        """Хеш Circuit Breaker по имени"""
        return hash(self.name)

    def __enter__(self):
        """Вход в контекстный менеджер"""
        self.logger.info(f"Entering circuit breaker context: {self.name}")
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        """Выход из контекстного менеджера"""
        if exc_type is not None:
            self.logger.warning(f"Circuit breaker context exited with exception: {exc_type.__name__}: {exc_val}")
        else:
            self.logger.info(f"Circuit breaker context exited successfully: {self.name}")
    ...
    def _on_failure(self, response_time: float, error: Exception):
        ...
        # Логируем ошибку
        self.logger.warning(f"Circuit breaker failure: {type(error).__name__}: {error} (response_time: {response_time:.3f}s)")
