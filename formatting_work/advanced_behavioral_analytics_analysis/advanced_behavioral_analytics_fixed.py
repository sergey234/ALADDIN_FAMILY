        ...
        if expected_activity > 0:
            ...
            if deviation > 2.0:  # 200% отклонение
                anomaly = AdvancedAnomaly(
                    ...
                    description=(f"Необычная активность в {current_hour}:00. "
                                 f"Ожидалось: {expected_activity}, получено: {current_activity}"),
                ...
                if baseline_value > 0:
                    ...
                    if deviation > 0.5:  # 50% отклонение
                        anomaly = AdvancedAnomaly(
                            ...
                            description=(f"Аномалия производительности {metric_name}: {current_value} "
                                         f"(базовое: {baseline_value})"),
                            detected_at=datetime.now(),
