...
class VPNAnalytics:
    ...
    def _cleanup_old_data(self) -> None:
        ...
        self.analytics_data = [d for d in self.analytics_data if d.timestamp > cutoff_date]
    ...
    async def get_usage_report(self, start_date: datetime, end_date: datetime,
        ...
        report_data["summary"] = {
            ...
            "unique_users": len(set(d.user_id for d in filtered_data if d.user_id)),
            "unique_servers": len(set(d.server_id for d in filtered_data if d.server_id)),
            "unique_sessions": len(set(d.session_id for d in filtered_data if d.session_id))
        }
    ...
    async def get_performance_report(self, start_date: datetime, end_date: datetime,
        ...
        filtered_data = [
            ...
            and d.metric_name in ["server_load", "server_response_time", "server_connections", "server_errors"]
        ]

         # Группировка по серверам
        server_data = defaultdict(lambda: defaultdict(list))
        for data in filtered_data:
            server_data[data.server_id or "unknown"][data.metric_name].append(data.value)
        ...
        for server, metrics in server_data.items():
            ...
            performance_score = self._calculate_performance_score(server_report["metrics"])
    ...
    async def get_revenue_report(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        for revenue_type, values in revenue_by_type.items():
         ...
         # Расчет трендов
        daily_revenue = self._calculate_daily_revenue(revenue_data, start_date, end_date)
    ...
    def _calculate_daily_revenue(self, revenue_data: List[AnalyticsData],
    ...
    async def get_anomaly_detection(self, metric_name: str,
        ...
        metric_data = [d for d in self.analytics_data if d.metric_name == metric_name]
        ...
        for data in metric_data:
            z_score = abs(data.value - mean_val) / std_val if std_val > 0 else 0
    ...
    async def get_recommendations(self) -> List[Dict[str, Any]]:
        ...
        for server_id, server_data in performance_data.get("servers", {}).items():
