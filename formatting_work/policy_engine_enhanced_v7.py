...
class PolicyEngine(SecurityBase):
        ...
        try:
            ...
            backup_data = {
                ...
                "policies": {pid: {
                    ...
                    "actions": [{"action_type": a.action_type.value, "parameters": a.parameters} for a in policy.actions],
                    "target_users": policy.target_users,
                    "target_devices": policy.target_devices,
                    "target_applications": policy.target_applications,
                    "created_at": policy.created_at.isoformat(),
            ...
            return {"success": True, "backup_path": backup_path, "policies_count": len(self.policies)}
    ...
    def export_policies(self, policy_ids: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "policies": exported_policies, "count": len(exported_policies)}
    ...
    def test_policy(self, policy_id: str, test_context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Оцениваем политику
            evaluation_result = self.evaluate_policy(policy_id, "test_user", test_context)
    ...
    def clone_policy(self, policy_id: str, new_policy_id: str, new_name: str = None) -> bool:
        """
        ...
        try:
            ...
            self.logger.info(f"Политика {policy_id} клонирована как {new_policy_id}")
            return True
    ...
    def archive_policy(self, policy_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Ошибка архивирования политики {policy_id}: {e}")
            return False
    ...
    def get_policy_history(self, policy_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Ошибка получения истории политики {policy_id}: {e}")
            return []

    def search_policies(self, query: str, search_fields: List[str] = None) -> List[Dict[str, Any]]:
        """
        Поиск политик
        ...
        try:
            ...
            for policy_id, policy in self.policies.items():
                ...
                for field in search_fields:
                    if field == "name" and query_lower in policy.name.lower():
                        match_found = True
                        break
                    elif field == "description" and query_lower in policy.description.lower():
                        match_found = True
                        break
                    elif field == "policy_id" and query_lower in policy_id.lower():
                        match_found = True
                        break
    ...
    def bulk_update_policies(self, updates: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            if policy_id:
                ...
                return {
                    ...
                    "success_rate": len([e for e in policy_evaluations if e.matched]) / len(policy_evaluations) if policy_evaluations else 0,
                    "average_evaluation_time": sum(e.evaluation_time for e in policy_evaluations) / len(policy_evaluations) if policy_evaluations else 0,
                    "last_evaluation": max(e.timestamp for e in policy_evaluations).isoformat() if policy_evaluations else None
                }
            else:
                # Общие метрики
                return {
    ...
    def optimize_policies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for policy_id, policy in self.policies.items():
                policy_evaluations = [e for e in self.policy_evaluations if e.policy_id == policy_id]
                if not policy_evaluations and policy.status == PolicyStatus.ACTIVE:
                    unused_policies.append(policy_id)
            
            if unused_policies:
                optimizations.append(f"Найдено {len(unused_policies)} неиспользуемых политик")
    ...
    def cleanup_old_policies(self, days_old: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            policies_to_remove = []
            for policy_id, policy in self.policies.items():
                if (policy.status == PolicyStatus.DEPRECATED and 
