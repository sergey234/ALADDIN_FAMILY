...
def measure_time(operation_name: str):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            logger.info(f"{operation_name} выполнена за {execution_time:.3f} секунд")
            return result
        return wrapper
    return decorator
...
def performance_monitor(operation_name: str):
    ...
    try:
        ...
        execution_time = time.time() - start_time
        logger.info(f"Операция {operation_name} завершена за {execution_time:.3f} секунд")
...
def validate_threat_data(threat_data: Dict[str, Any]) -> bool:
    ...
    """
    required_fields = ['threat_id', 'title', 'description', 'threat_type', 'severity']
    
    for field in required_fields:
        if field not in threat_data:
            raise ThreatValidationError(f"Отсутствует обязательное поле: {field}")
    
    if not isinstance(threat_data['threat_id'], str) or not threat_data['threat_id'].strip():
...
class ThreatIntelligence:
    ...
    def add_ioc(self, ioc_type: 'IOCType', value: str, description: str = "") -> None:
        ...
        if not isinstance(ioc_type, IOCType):
            raise ThreatValidationError("ioc_type должен быть экземпляром IOCType")
...
class ThreatIntelligenceAgent(SecurityBase):
    ...
    async def collect_threats_async(self) -> Dict[str, Any]:
        ...
        with performance_monitor("Асинхронный сбор угроз"):
            ...
            return {"collected_threats": collected_threats, "sources": len(tasks)}

    async def _collect_from_source_async(self, source_name: str, source_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def analyze_threats_async(self) -> Dict[str, Any]:
        ...
        with performance_monitor("Асинхронный анализ угроз"):
            ...
            tasks = []
            for threat_id, threat in self.threats.items():
                task = asyncio.create_task(self._analyze_single_threat_async(threat))
                tasks.append(task)
    ...
    async def _analyze_single_threat_async(self, threat: 'ThreatIntelligence') -> Dict[str, Any]:
    ...
    def active_sources_count(self) -> int:
        ...
        return len([source for source in self.sources.values() 
    ...
    def validate_threat_source(source_config: Dict[str, Any]) -> bool:
        ...
        for field in required_fields:
            if field not in source_config:
                raise ThreatValidationError(f"Отсутствует поле {field} в конфигурации источника")
