...
class CDDeploymentManager:
    ...
    def _create_default_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "services": {
                ...
                "vpn_configuration": {"version": "1.0.0", "deployment_order": 2},
    ...
    async def _build_stage(self, config: DeploymentConfig,
    ...
    async def _testing_stage(self, config: DeploymentConfig,
                           record: DeploymentRecord) -> None:
        """Стадия тестирования"""
    ...
    async def _staging_stage(self, config: DeploymentConfig,
                           record: DeploymentRecord) -> None:
        """Стадия staging"""
    ...
    async def rollback(self, deployment_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Откат завершен: {deployment_id} -> {previous_version}")
    ...
    async def get_deployment_history(self, environment: Optional[Environment] = None,
    ...
    async def get_deployment_stats(self) -> Dict[str, Any]:
        ...
        rolled_back_deployments = len([d for d in self.deployment_history
                                      if d.stage == DeploymentStage.ROLLED_BACK])

        if total_deployments > 0:
            success_rate = (successful_deployments / total_deployments) * 100
            avg_duration = sum(d.duration_seconds for d in self.deployment_history) / total_deployments
        else:
            success_rate = 0
            avg_duration = 0
    ...
    async def create_release(self, version: str, services: List[str],
    ...
    async def export_deployment_report(self, deployment_id: str,
