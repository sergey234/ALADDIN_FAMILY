...
class EnhancedDataCollector:
        ...
        self.data_sources = {
            ...
            'news_sources': {
                'rbc': {
                    ...
                    'keywords': ['мошенничество', 'фишинг', 'кибермошенничество', 'банковское мошенничество',
                               'телефонное мошенничество', 'интернет мошенничество', 'обман', 'хищение']
                },
                'ria': {
                ...
                'interfax': {
                    ...
                    'keywords': ['мошенничество', 'фишинг', 'кибермошенничество']
                },
                'tass': {
    ...
    async def collect_comprehensive_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_data['metadata']['total_sources'] += len(self.data_sources['news_sources'])
            ...
            all_data['metadata']['total_sources'] += len(self.data_sources['government_sources'])
            ...
            # Время сбора
            all_data['metadata']['collection_duration'] = time.time() - start_time

            # Сохранение данных
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            data_file = f'data/enhanced_collection/comprehensive_data_{timestamp}.json'

            with open(data_file, 'w', encoding='utf-8') as f:
                json.dump(all_data, f, ensure_ascii=False, indent=2)

            logger.info(f"✅ Комплексный сбор данных завершен: {data_file}")
            logger.info(f"📊 Собрано записей: {all_data['metadata']['total_records']}")
                    ...
                    if response.status_code == 200:
                        ...
                        for link in links[:5]:  # Ограничиваем количество
                            ...
                            if any(keyword in text.lower() for keyword in
                                  ['мошенничество', 'фишинг', 'кибер', 'безопасность']):

                                report_data = {
                                    'id': f"cbr_{len(cbr_data)+1}",
                                    'title': text,
                                    'url': href if href.startswith('http') else cbr_config['base_url'] + href,
                                    'source': 'ЦБ РФ',
    ...
    async def _collect_news_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            for source_name, source_config in self.data_sources['news_sources'].items():
                try:
                    ...
                    for keyword in source_config['keywords']:
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                articles = soup.find_all(['article', 'div'], class_=['news-item', 'article', 'item'])

                                for article in articles[:3]:  # Ограничиваем количество
                                    title_elem = article.find(['h1', 'h2', 'h3', 'a'])
                                    if title_elem:
                                        title = title_elem.get_text(strip=True)

                                        if any(keyword in title.lower() for keyword in
                                              ['мошенничество', 'фишинг', 'кибер', 'обман']):
                                            ...
                                            article_data = {
                                                ...
                                                'date': datetime.now().strftime('%Y-%m-%d'),
                                                'fraud_type': self._classify_fraud_type(title),
                                                'severity': self._determine_severity(title),
                                                'region': self._extract_region(title),
                                                'amount_lost': self._estimate_amount(title),
                                                'keywords': [keyword],
                                                'description': title
                                            }
                            ...
                            logger.warning(f"⚠️ Ошибка поиска по ключевому слову {keyword}: {e}")
                            continue

                except Exception as e:
                    logger.warning(f"⚠️ Ошибка сбора данных из {source_name}: {e}")
                    continue
    ...
    async def _collect_government_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            for source_name, source_config in self.data_sources['government_sources'].items():
                try:
                    ...
                    for endpoint in source_config['endpoints']:
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                items = soup.find_all(['div', 'article'], class_=['news', 'item', 'press'])

                                for item in items[:3]:  # Ограничиваем количество
    ...
    def _classify_fraud_type(self, text: str) -> str:
        ...
        elif any(word in text_lower for word in ['интернет', 'онлайн', 'сеть']):
    ...
    def _determine_severity(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ['критическ', 'массов', 'эпидемия']):
            return 'критическая'
    ...
    def _analyze_fraud_patterns(self, data: Dict[str, Any]) -> Dict[str, Dict[str, int]]:
        ...
        # Анализ всех данных
        all_items = data['cbr_data'] + data['news_data'] + data['government_data']

        for item in all_items:
            # По типам
            fraud_type = item.get('fraud_type', 'неизвестно')
            patterns['by_type'][fraud_type] = patterns['by_type'].get(fraud_type, 0) + 1

            # По регионам
            region = item.get('region', 'неизвестно')
            patterns['by_region'][region] = patterns['by_region'].get(region, 0) + 1
...
async def main():
    ...
    try:
        ...
        # Сохранение отчета
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        report_file = f'data/enhanced_collection/enhanced_report_{timestamp}.json'
