...
def get_test_results(limit: int = 100) -> List[TestResult]:
    """Получение результатов тестов из базы данных"""
    conn = sqlite3.connect("aladdin_dashboard.db")
    cursor = conn.cursor()

    cursor.execute(
        """
        SELECT test_id, test_name, status, duration, timestamp, details, 
...
def save_performance_metric(metric: PerformanceMetric):
...
def get_security_alerts(
    resolved: bool = None, limit: int = 100
) -> List[SecurityAlert]:
    """Получение уведомлений безопасности из базы данных"""
    conn = sqlite3.connect("aladdin_dashboard.db")
    cursor = conn.cursor()
    ...
    if resolved is not None:
        cursor.execute(
            """
            SELECT alert_id, severity, title, description, timestamp, source, resolved, action_required
