...
class LocationMetrics:
    ...
    def record_call(self, execution_time: float, success: bool, cache_hit: bool = False):
...
class RateLimiter:
    ...
    def __init__(self, max_calls: int = LocationConfig.RATE_LIMIT_CALLS, 
...
class LocationDistanceCalculator:
    ...
    def calculate_distance(
        ...
        Example:
            >>> LocationDistanceCalculator.calculate_distance((55.7558, 37.6176), (55.7559, 37.6177))
            0.012782
        ...
        try:
            ...
            if not LocationDistanceCalculator._validate_coordinates(coord1, coord2):
                logger.error(f"Invalid coordinates: {coord1}, {coord2}")
                success = False
                return 0.0
            
            distance = geopy.distance.geodesic(coord1, coord2).kilometers
            logger.debug(f"Calculated distance: {distance:.6f} km between {coord1} and {coord2}")
    ...
    async def calculate_distance_async(
        ...
        try:
            ...
            if not rate_limiter.is_allowed("calculate_distance_async"):
                logger.warning("Rate limit exceeded for calculate_distance_async")
                return 0.0
            
            # Валидация входных параметров
            if not LocationDistanceCalculator._validate_coordinates(coord1, coord2):
                logger.error(f"Invalid coordinates in async calculation: {coord1}, {coord2}")
                success = False
                return 0.0
    ...
    def is_location_in_radius(
        ...
        Example:
            >>> LocationDistanceCalculator.is_location_in_radius((55.7558, 37.6176), (55.7559, 37.6177), 1.0)
            True
        """
        start_time = time.time()
        success = True
        ...
        try:
            # Валидация входных параметров
            if not LocationDistanceCalculator._validate_coordinates(center, point):
                logger.error(f"Invalid coordinates in radius check: {center}, {point}")
                success = False
                return False
            ...
            logger.debug(f"Radius check: {point} in {radius_km}km from {center} = {result}")
            return result
    ...
    @staticmethod
    def _validate_coordinates(coord1: Tuple[float, float], coord2: Tuple[float, float]) -> bool:
        ...
        try:
            for coord in [coord1, coord2]:
                ...
                if not (LocationConfig.MIN_COORDINATE_VALUE <= lat <= LocationConfig.MAX_COORDINATE_VALUE):
                    return False
                
                if not (LocationConfig.MIN_LONGITUDE_VALUE <= lon <= LocationConfig.MAX_LONGITUDE_VALUE):
                    return False
...
class LocationServiceFinder:
    ...
    def find_nearest_services(
        ...
        Example:
            >>> services = [{'name': 'Hospital', 'coordinates': (55.7558, 37.6176)}]
            >>> LocationServiceFinder.find_nearest_services((55.7559, 37.6177), services)
            [{'service': {'name': 'Hospital', 'coordinates': (55.7558, 37.6176)}, 'distance': 0.012782}]
        """
        start_time = time.time()
        success = True
        ...
        try:
            ...
            # Валидация входных параметров
            if not LocationDistanceCalculator._validate_coordinates(location, location):
                logger.error(f"Invalid location: {location}")
                success = False
                return []
            
            if not isinstance(services, list):
                logger.error(f"Services must be a list, got: {type(services)}")
                success = False
                return []
            
            if len(services) > LocationConfig.MAX_SERVICES_LIMIT:
                logger.warning(f"Too many services: {len(services)}, limiting to {LocationConfig.MAX_SERVICES_LIMIT}")
            ...
            for i, service in enumerate(services):
                try:
                    ...
                    if service_coords and LocationDistanceCalculator._validate_coordinates(service_coords, service_coords):
                        distance = LocationDistanceCalculator.calculate_distance_cached(
                            location, service_coords
                        )
                        service_distances.append(
                            {"service": service, "distance": distance, "index": i}
                        )
                    else:
                        logger.warning(f"Invalid service coordinates at index {i}: {service_coords}")
                except Exception as e:
            ...
            logger.debug(f"Found {len(service_distances)} valid services out of {len(services)}")
    ...
    def get_services_in_radius(
        ...
        Example:
            >>> services = [{'name': 'Hospital', 'coordinates': (55.7558, 37.6176)}]
            >>> LocationServiceFinder.get_services_in_radius((55.7559, 37.6177), services, 0.1)
        ...
        try:
            ...
            if not rate_limiter.is_allowed("get_services_in_radius"):
                logger.warning("Rate limit exceeded for get_services_in_radius")
                return []
            ...
            logger.debug(f"Found {len(services_in_radius)} services in {radius_km}km radius")
    ...
    async def find_nearest_services_async(
        ...
        try:
            ...
            if not rate_limiter.is_allowed("find_nearest_services_async"):
                logger.warning("Rate limit exceeded for find_nearest_services_async")
                return []
            ...
            for i, service in enumerate(services):
                try:
                    service_coords = service.get("coordinates")
                    if service_coords and LocationDistanceCalculator._validate_coordinates(service_coords, service_coords):
                        distance = await LocationDistanceCalculator.calculate_distance_async(
                            location, service_coords
                        )
                        service_distances.append(
                            {"service": service, "distance": distance, "index": i}
            ...
            logger.debug(f"Async found {len(service_distances)} valid services")
    ...
    def validate_coordinates(lat: float, lon: float) -> bool:
        ...
        try:
            ...
            if abs(lat) > 90 or abs(lon) > 180:
                logger.warning(f"Suspicious coordinates detected: lat={lat}, lon={lon}")
            
            logger.debug(f"Coordinates validated successfully: lat={lat}, lon={lon}")
            return True
    ...
    def is_location_verified(location_data: Dict[str, Any]) -> bool:
        ...
        try:
            if not isinstance(location_data, dict):
                logger.error(f"Location data must be a dict, got: {type(location_data)}")
                success = False
                return False
...
class LocationClusterAnalyzer:
    ...
    def calculate_cluster_center(
        ...
        try:
            ...
            for i, point in enumerate(points):
                if LocationDistanceCalculator._validate_coordinates(point, point):
                    valid_points.append(point)
                else:
                    logger.warning(f"Invalid point at index {i}: {point}")
            ...
            center = (lat_sum / len(valid_points), lon_sum / len(valid_points))
            logger.debug(f"Calculated cluster center: {center} from {len(valid_points)} points")
            return center
    ...
    def calculate_cluster_radius(
        ...
        Example:
            ...
            >>> LocationClusterAnalyzer.calculate_cluster_radius(points, center)
            0.010000
        """
        start_time = time.time()
        success = True
        ...
        try:
            ...
            if not LocationDistanceCalculator._validate_coordinates(center, center):
            ...
            for i, point in enumerate(points):
                if LocationDistanceCalculator._validate_coordinates(point, point):
                    distance = LocationDistanceCalculator.calculate_distance_cached(center, point)
            ...
            radius = round(max_distance, LocationConfig.DEFAULT_RADIUS_PRECISION)
            logger.debug(f"Calculated cluster radius: {radius}km from {valid_points} points")
            return radius
...
class LocationHealthChecker:
    ...
    def health_check() -> Dict[str, Any]:
        ...
        try:
            ...
            # Сбор метрик
            health_data = {
                "status": "healthy" if geopy_available and rate_limit_status else "degraded",
                "timestamp": time.time(),
                "geopy_available": geopy_available,
                "rate_limit_ok": rate_limit_status,
                "metrics": {
                    "total_calls": metrics.call_count,
                    "average_execution_time": metrics.get_average_time(),
                    "error_rate": metrics.get_error_rate(),
                    "cache_hit_rate": metrics.get_cache_hit_rate()
                },
                "config": {
                    "accuracy_threshold": LocationConfig.DEFAULT_ACCURACY_THRESHOLD,
