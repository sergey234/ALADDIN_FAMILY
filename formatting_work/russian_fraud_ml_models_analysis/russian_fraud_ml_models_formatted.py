...
class RussianFraudMLModels:
    ...
    def _load_data(self) -> Dict[str, Any]:
        ...
        try:
            with open(self.data_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            logger.info(
                f"Загружены данные: {data['metadata']['total_records']} записей"
            )
            return data
    ...
    def _prepare_training_data(self) -> Tuple[np.ndarray, np.ndarray]:
            ...
            records.append(
                {
                    ...
                    "amount_lost": self._estimate_amount_from_description(
                        article["description"]
                    ),
        ...
        df["fraud_type_encoded"] = LabelEncoder().fit_transform(
            df["fraud_type"]
        )
    ...
    def create_severity_predictor(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for report in self.fraud_data.get("cbr_reports", []):
                records.append(
                    {
                        "fraud_type_encoded": LabelEncoder().fit_transform(
                            [report["fraud_type"]]
                        )[0],
                        "region_encoded": LabelEncoder().fit_transform(
                            [report["region"]]
                        )[0],
                        "amount_normalized": report["amount_lost"]
                        / 1000000,  # Нормализация
                        "severity": report["severity"],
                    }
                )
                ...
                records.append(
                    {
                        ...
                        "amount_normalized": self._estimate_amount_from_description(
                            article["description"]
                        )
                        / 1000000,
                        "severity": article["severity"],
            ...
            df = pd.DataFrame(records)
            X_severity = df[
                ["fraud_type_encoded", "region_encoded", "amount_normalized"]
            ].values
            y_severity = (
                df["severity"]
                .map(
            ...
            logger.info(
                f"Предиктор серьезности создан. Точность: {accuracy:.3f}"
            )
    ...
    def create_region_analyzer(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for region, count in (
                self.fraud_data.get("fraud_patterns", {})
                .get("by_region", {})
                .items()
            ):
                regional_data.append(
                    {
                        "region": region,
                        "fraud_count": count,
                        "population_factor": self._get_population_factor(
                            region
                        ),
    ...
    def predict_fraud_type(
        ...
        try:
            ...
            predicted_type = (
                fraud_types[prediction]
                if prediction < len(fraud_types)
    ...
    def generate_model_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "data_summary": {
                "total_records": self.fraud_data.get("metadata", {}).get(
                    "total_records", 0
                ),
                "fraud_types": len(
                    self.fraud_data.get("fraud_patterns", {}).get(
                        "by_type", {}
                    )
                ),
                "regions": len(
...
def main():
    ...
    try:
        ...
        prediction = ml_models.predict_fraud_type(
            severity="высокая", region="Москва", amount=500000, source="банк"
        )

        print(
            f"Предсказанный тип мошенничества: {prediction.get('predicted_type', 'ошибка')}"
        )
