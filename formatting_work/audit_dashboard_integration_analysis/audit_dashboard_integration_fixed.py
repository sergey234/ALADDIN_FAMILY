...
from typing import List, Optional  # Any, Dict не используются

from fastapi import APIRouter, BackgroundTasks, HTTPException
# from fastapi.responses import JSONResponse  # Не используется

from audit_scheduler import AuditScheduler  # AuditSchedule не используется
from automated_audit_system import AuditType, AutomatedAuditSystem  # AuditResult не используется
from compliance_monitor import (
    ComplianceMonitor,
    ComplianceStandard,
)  # ComplianceLevel не используется
...
async def assess_compliance_task(standard: ComplianceStandard):
    ...
    try:
        ...
        print(
            f"✅ Compliance assessment for {standard.value} completed: "
            f"{assessment.overall_level.value} ({assessment.score}%)"
        )
