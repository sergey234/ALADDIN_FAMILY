    ...
    def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            if execution_time > 1.0:  # Более 1 секунды
                self.log_activity(
                    f"Медленная операция {func.__name__}: {execution_time:.2f}s",
                    "warning"
                )
            ...
            execution_time = time.time() - start_time
            self.log_activity(
                f"Ошибка в {func.__name__} за {execution_time:.2f}s: {e}",
                "error"
            )
            raise
    return wrapper
...
class IncidentResponseManager(SecurityBase):
    ...
    def __init__(
        ...
        self._status_index = {}  # Индекс по статусам
        self._priority_index = {}  # Индекс по приоритетам
        self._type_index = {}  # Индекс по типам
        self._created_at_index = {}  # Индекс по дате создания
        self._assigned_to_index = {}  # Индекс по назначенным пользователям
        
        # Rate limiting
        self._rate_limits = {}  # Ограничения скорости запросов
        self._max_requests_per_minute = 100  # Максимум запросов в минуту
    ...
    def _update_indexes(self, incident: "Incident", action: str = "add"):
        """Обновление индексов при изменении инцидента"""
        incident_id = incident.incident_id
        
        if action == "add":
            # Добавление в индексы
            status_key = incident.status.value if hasattr(incident.status, 'value') else str(incident.status)
            priority_key = incident.priority.value if hasattr(incident.priority, 'value') else str(incident.priority)
            type_key = incident.incident_type.value if hasattr(incident.incident_type, 'value') else str(incident.incident_type)
            
            self._status_index.setdefault(status_key, set()).add(incident_id)
            self._priority_index.setdefault(priority_key, set()).add(incident_id)
            self._type_index.setdefault(type_key, set()).add(incident_id)
            
            # Индекс по дате создания (по дням)
            date_key = incident.created_at.date().isoformat()
            self._created_at_index.setdefault(date_key, set()).add(incident_id)
            
            # Индекс по назначенным пользователям
            if hasattr(incident, 'assigned_to') and incident.assigned_to:
                self._assigned_to_index.setdefault(incident.assigned_to, set()).add(incident_id)
                
        elif action == "remove":
            # Удаление из индексов
            for index in [self._status_index, self._priority_index, self._type_index, 
                         self._created_at_index, self._assigned_to_index]:
                for key in index:
                    index[key].discard(incident_id)
                    if not index[key]:  # Удаляем пустые ключи
                        del index[key]
    ...
    def _check_rate_limit(self, user_id: str = "default") -> bool:
        ...
        if len(user_requests) >= self._max_requests_per_minute:
            self.log_activity(f"Rate limit exceeded for user {user_id}", "warning")
            return False
    ...
    def _get_incident_cached(self, incident_id: str):
        """Кэшированное получение инцидента"""
    ...
    def create_incident(
        ...
        try:
            ...
            # Обновление индексов
            self._update_indexes(incident, "add")
    ...
    def _auto_assign_team(self, incident: Incident):
        ...
        try:
            ...
            if (team in self.response_teams and
                    self.response_teams[team]["available"]):
    ...
    def update_incident_status(
        ...
        try:
            ...
            if (old_status == IncidentStatus.OPEN and
        ...
        try:
            if incident.resolved_at and incident.created_at:
                ...
                if self.resolved_incidents == 1:
                    ...
                    total_time = (self.average_resolution_time *
                                  (self.resolved_incidents - 1) +
                                  resolution_time)
                    self.average_resolution_time = (total_time /
    ...
    def _get_incidents_by_priority(self) -> Dict[str, int]:
        ...
        priority_count = {}
        for incident in self.incidents.values():
            if hasattr(incident.priority, 'value'):
    ...
    def get_advanced_analytics(self) -> Dict[str, Any]:
        ...
        return {
            "response_time_metrics": self._calculate_response_times(),
    ...
    def _calculate_response_times(self) -> Dict[str, float]:
        ...
        response_times = []
        for incident in self.incidents.values():
            if hasattr(incident, 'first_response_time') and incident.first_response_time:
                response_times.append(incident.first_response_time)
    ...
    def _analyze_escalation_patterns(self) -> Dict[str, Any]:
        """Анализ паттернов эскалации"""
        escalation_data = {
            "total_escalations": self.escalated_incidents,
            "escalation_rate": self.escalated_incidents / max(self.total_incidents, 1),
            "escalation_by_type": {},
        ...
        for incident in self.incidents.values():
            if incident.is_escalated():
                # Эскалация по типу
                type_key = incident.incident_type.value if hasattr(incident.incident_type, 'value') else str(incident.incident_type)
                escalation_data["escalation_by_type"][type_key] = escalation_data["escalation_by_type"].get(type_key, 0) + 1
                
                # Эскалация по приоритету
                priority_key = incident.priority.value if hasattr(incident.priority, 'value') else str(incident.priority)
                escalation_data["escalation_by_priority"][priority_key] = escalation_data["escalation_by_priority"].get(priority_key, 0) + 1
        
        return escalation_data

    def _calculate_resolution_efficiency(self) -> Dict[str, Any]:
        """Расчет эффективности разрешения"""
        resolved_incidents = [inc for inc in self.incidents.values() if inc.is_resolved()]
        
        if not resolved_incidents:
            return {"efficiency_score": 0.0, "average_resolution_time": 0.0}
        ...
        # Оценка эффективности (чем меньше время разрешения, тем выше оценка)
        efficiency_score = max(0, 100 - (avg_resolution * 2))  # 2 балла за каждый час
    ...
    def _analyze_team_performance(self) -> Dict[str, Any]:
        ...
        for incident in self.incidents.values():
            if hasattr(incident, 'assigned_to') and incident.assigned_to:
                ...
                team_stats[team]["total_incidents"] += 1
                
                if incident.is_resolved():
                    team_stats[team]["resolved_incidents"] += 1
                    if incident.get_resolution_time_hours():
                        team_stats[team]["average_resolution_time"] += incident.get_resolution_time_hours()
                
                if incident.is_escalated():
                    team_stats[team]["escalated_incidents"] += 1
        
        # Расчет среднего времени разрешения для каждой команды
        for team in team_stats:
            if team_stats[team]["resolved_incidents"] > 0:
                team_stats[team]["average_resolution_time"] /= team_stats[team]["resolved_incidents"]
        
        return team_stats

    def _analyze_trends(self) -> Dict[str, Any]:
        """Анализ трендов"""
        # Группировка инцидентов по дням
        daily_incidents = {}
        for incident in self.incidents.values():
            date_key = incident.created_at.date().isoformat()
        ...
        # Расчет тренда
        dates = sorted(daily_incidents.keys())
        if len(dates) < 2:
            return {"trend": "insufficient_data", "daily_average": sum(daily_incidents.values()) / len(daily_incidents)}
        
        recent_avg = sum(list(daily_incidents.values())[-7:]) / min(7, len(daily_incidents))
        overall_avg = sum(daily_incidents.values()) / len(daily_incidents)
        ...
        return {
            ...
            "total_days": len(daily_incidents)
        }
    ...
    def _assess_risk_levels(self) -> Dict[str, Any]:
        ...
        for incident in self.incidents.values():
            if not incident.is_resolved():
                ...
                if severity in risk_incidents:
                    risk_incidents[severity] += 1
        
        total_open = sum(risk_incidents.values())
        risk_percentage = {
            level: (count / total_open * 100) if total_open > 0 else 0
            for level, count in risk_incidents.items()
        }
        
        return {
            "risk_incidents": risk_incidents,
            "risk_percentages": risk_percentage,
            "overall_risk_score": min(100, risk_incidents["critical"] * 4 + risk_incidents["high"] * 2 + risk_incidents["medium"])
        }
    ...
    def _get_rate_limiting_stats(self) -> Dict[str, Any]:
        ...
        total_requests = sum(len(requests) for requests in self._rate_limits.values())
        ...
        return {
            ...
            "rate_limited_users": len([user for user, requests in self._rate_limits.items() 
                                     if len(requests) >= self._max_requests_per_minute])
        }

    async def get_incident_async(self, incident_id: str) -> Optional[Dict[str, Any]]:
        """Асинхронное получение инцидента"""
        await asyncio.sleep(0.001)  # Небольшая задержка для имитации асинхронности
        return self.get_incident(incident_id)

    async def get_incidents_batch_async(self, incident_ids: List[str]) -> Dict[str, Any]:
        """Асинхронное получение нескольких инцидентов"""
        tasks = [self.get_incident_async(incident_id) for incident_id in incident_ids]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        return {incident_id: result for incident_id, result in zip(incident_ids, results)}

    async def create_incidents_batch_async(self, incidents_data: List[Dict[str, Any]]) -> List[Any]:
        """Асинхронное создание нескольких инцидентов"""
        tasks = []
        for data in incidents_data:
            task = self.create_incident_async(
    ...
    def clear_cache(self) -> bool:
        """Очистка кэша"""
        try:
            self._get_incident_cached.cache_clear()
            self.cache_hits = 0
            self.cache_misses = 0
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        cache_info = self._get_incident_cached.cache_info()
        return {
    ...
    def search_incidents(self, query: str) -> List[Incident]:
        ...
        return [
            ...
            if (query_lower in incident.title.lower() or
                ...
                query_lower in incident.incident_id.lower())
        ]
