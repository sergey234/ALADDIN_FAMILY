...
class EnhancedSafeAnalyzer(SafeQualityAnalyzer):
    ...
    def _get_detailed_fix_suggestion(self, error: Dict[str, Any]) -> str:
        ...
        suggestions = {
            ...
            "W292": "Файл не заканчивается переводом строки. Добавьте \\n в конец файла",
        ...
        return suggestions.get(
            code, f"Требует ручного анализа: {error['message']}"
        )
    ...
    def _create_fix_plan(self, errors: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        plan = {
            ...
            "manual_review_needed": len(
                [e for e in errors if e["safety_level"] == SafetyLevel.MANUAL]
            ),
            "dangerous_fixes": len(
                [
                    e
    ...
    def generate_detailed_report(self, analysis_result: Dict[str, Any]) -> str:
        ...
        report.append("=" * 80)
        report.append(f"📁 Файл: {analysis_result.get('file_path', 'Неизвестно')}")
        report.append(f"⏰ Время анализа: {analysis_result.get('timestamp', 'Неизвестно')}")
        report.append(f"📊 Размер файла: {analysis_result.get('file_size', 'Неизвестно')} байт")
        report.append(f"📏 Строк кода: {analysis_result.get('line_count', 'Неизвестно')}")
        report.append("")
        ...
        if "fix_plan" in analysis_result:
            ...
            report.append(
                f"Требуют ручного анализа: {plan['manual_review_needed']}"
            )
            ...
            if plan["steps"]:
                ...
                for step in plan["steps"]:
                    ...
                    report.append(
                        f"   Инструменты: {', '.join(step['tools'])}"
                    )
                    report.append(f"   Риск: {step['risk']}")
                    report.append("")

        # Безопасность
        safety = analysis_result.get("safety_report", {})
        report.append("🛡️ ОТЧЕТ О БЕЗОПАСНОСТИ:")
        report.append("-" * 40)
        report.append(
            f"Функции безопасности сохранены: {'✅' if safety.get('security_functions_preserved', False) else '❌'}"
        )
        report.append(
            f"Шифрование не затронуто: {'✅' if safety.get('encryption_intact', False) else '❌'}"
        )
        report.append(
            f"Аутентификация сохранена: {'✅' if safety.get('authentication_preserved', False) else '❌'}"
        )
        report.append(
            f"Валидация не затронута: {'✅' if safety.get('validation_intact', False) else '❌'}"
        )
        report.append(
            f"Логирование сохранено: {'✅' if safety.get('logging_preserved', False) else '❌'}"
        )
        report.append(
            f"Проблем безопасности найдено: {safety.get('security_issues_found', 0)}"
        )
        report.append(f"Критических проблем: {safety.get('critical_issues', 0)}")
        report.append("")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print(
            "Использование: python3 enhanced_safe_analyzer.py <путь_к_файлу>"
        )
