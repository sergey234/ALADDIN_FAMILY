...
class SecurityAnalyticsManager(SecurityBase):
    ...
    def detect_anomalies(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.log_activity(f"Ошибка анализа метрик производительности: {e}", "error")
            return []
    ...
    def _get_system_trends(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log_activity(f"Ошибка получения метрик по категориям: {e}", "error")
            return {}
    ...
    def restore_data(self, backup_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.log_activity(f"Ошибка получения статистики аналитики: {e}", "error")
            return False
            ...
            integrity_issues.append(
                {
                    ...
                    "message": f"Проверка целостности: {result[status]}",
                }
            )
...
def async_performance_monitor(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if hasattr(args[0], "log_activity"):
                args[0].log_activity(
                    f"Асинхронный метод {func.__name__} выполнен за {execution_time:.3f}с"
                )
            return result
        except Exception as e:
            execution_time = time.time() - start_time
            if hasattr(args[0], "log_activity"):
                args[0].log_activity(
                    f"Ошибка в асинхронном методе {func.__name__} за {execution_time:.3f}с: {e}",
                    "error",
                )
            raise
    ...
    async def _async_analyze_performance(self) -> List[Dict[str, Any]]:
        ...
        if perf_metrics["success_rate"] < 90:
            insight = {
                ...
                "recommendation": "Проверить логи ошибок и улучшить обработку исключений",
                "timestamp": datetime.now().isoformat(),
                "severity": "medium",
            }
            insights.append(insight)
