...
class PasswordSecurityAgent(SecurityBase):
    ...
    def __str__(self) -> str:
        ...
        return f"PasswordSecurityAgent(name='{self.name}', status='{self.status}')"
        ...
        try:
            for key, value in kwargs.items():
                ...
                if isinstance(value, str) and not value.strip():
                    raise ValueError(f"Параметр {key} не может быть пустой строкой")
    ...
    async def async_generate_password(self, length: int = 12, **kwargs) -> str:
        ...
        try:
            ...
            self.log_activity(f"Ошибка асинхронной генерации пароля: {e}", "error")
            return ""

    async def async_analyze_password_strength(self, password: str) -> PasswordStrength:
    ...
    def get_health_status(self) -> dict:
        ...
        return {
            ...
            "system_health": "healthy" if str(self.status) == "ComponentStatus.RUNNING" else "degraded"
        }
