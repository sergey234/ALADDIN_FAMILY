...
class SecurityPolicy:
    ...
    def __str__(self) -> str:
        ...
        return f"SecurityPolicy(id='{self.policy_id}', name='{self.name}', level={self.min_trust_level.value})"

    def __repr__(self) -> str:
        """Представление для отладки"""
        return (
            f"SecurityPolicy(policy_id='{self.policy_id}', name='{self.name}', "
            f"min_trust_level={self.min_trust_level}, required_verifications={self.required_verifications})"
        )
...
class ZeroTrustManager:
    ...
    async def async_verify_access(
        self, request_id: str, verification_type: str, verification_data: Dict[str, any]
    ) -> bool:
        ...
        return self.verify_access(request_id, verification_type, verification_data)
    ...
    def validate_parameters(self, **kwargs) -> bool:
        ...
        try:
            ...
            if 'device_id' in kwargs and not isinstance(kwargs['device_id'], str):
    ...
    def get_family_security_summary(self) -> Dict[str, any]:
        ...
        """
        return {
            "статус_защиты": "🛡️ Активна" if self.is_running else "⚠️ Неактивна",
            "защищенные_устройства": len(self.devices),
            "пользователи": len(self.users),
            "активные_запросы": len(self.list_active_requests()),
            "уровень_безопасности": "Высокий" if self.stats["approved_requests"] > 0 else "Средний",
            "последняя_активность": self.stats.get("last_cleanup", "Неизвестно"),
            "рекомендации": [
    ...
    def get_child_safety_status(self) -> Dict[str, any]:
        ...
        """
        child_devices = [dev for dev in self.devices.values() if 'child' in dev.device_type.lower()]
        
        return {
            "детские_устройства": len(child_devices),
            "статус_родительского_контроля": "Активен" if child_devices else "Не настроен",
