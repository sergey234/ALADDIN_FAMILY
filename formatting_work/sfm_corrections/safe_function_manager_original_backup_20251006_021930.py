...
from security.security_analytics import SecurityAnalytics, SecurityAnalyticsManager
from security.incident_response import IncidentResponseSystem, IncidentResponseManager
from security.health_check_system import HealthCheckSystem, HealthCheckManager
...
class SafeFunctionManager(SecurityBase):
    ...
    def execute_function(self, function_id: str, params: Optional[Dict[str, Any]] = None) -> Tuple[bool, Any, str]:
        ...
        try:
            ...
            self.log_activity(
                f"Системная ошибка выполнения функции {function_id}: {e}", "error"
            )
    ...
    def search_functions(self, query: str, category: Optional[str] = None, status: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for function_id, function in self.functions.items():
                ...
                matches_query = (
                    query_lower in function.name.lower() or
                    query_lower in function.description.lower() or
                    query_lower in function_id.lower() or
                    query_lower in function.function_type.lower()
                )
                
                # Проверка фильтра по категории
                matches_category = (
                    category is None or 
                    function.function_type.upper() == category.upper()
                )
                
                # Проверка фильтра по статусу
                matches_status = (
                    status is None or 
                    function.status.value.lower() == status.lower()
                )
    ...
    def search_functions_advanced(self, 
        ...
        try:
            ...
            for function_id, function in self.functions.items():
                ...
                if query:
                    query_lower = query.lower()
                    if not (query_lower in function.name.lower() or
                           query_lower in function.description.lower() or
                           query_lower in function_id.lower() or
                           query_lower in function.function_type.lower()):
                        matches = False
    ...
    async def get_monitoring_dashboard_data(self) -> Dict[str, Any]:
        """Получение данных для dashboard мониторинга"""
        try:
            dashboard_data = {
                "sfm_status": {
                    "status": self.status.value,
                    ...
                    "enabled_functions": self.functions_enabled,
                    "disabled_functions": self.functions_disabled,
                    "sleeping_functions": self.functions_sleeping
                },
                "security_monitoring": await self._get_security_monitoring_data(),
                "analytics": await self._get_analytics_data(),
                "incident_response": await self._get_incident_response_data(),
                "health_check": await self._get_health_check_data(),
                "timestamp": datetime.now().isoformat()
            }
            return dashboard_data
        except Exception as e:
            self.log_activity(f"Ошибка получения данных dashboard: {e}", "error")
            return {"error": str(e)}

    async def _get_security_monitoring_data(self) -> Dict[str, Any]:
        """Получение данных мониторинга безопасности"""
        try:
            if self.security_monitoring:
                return await self.security_monitoring.get_monitoring_data()
    ...
    async def _get_analytics_data(self) -> Dict[str, Any]:
        ...
        try:
            if self.security_analytics:
                await self.security_analytics.initialize()
                return await self.security_analytics.get_dashboard_data()
            return {"status": "not_available"}
        except Exception as e:
            return {"status": "error", "error": str(e)}

    async def _get_incident_response_data(self) -> Dict[str, Any]:
        """Получение данных реагирования на инциденты"""
        try:
            if self.incident_response:
                await self.incident_response.initialize()
                return await self.incident_response.get_all_incidents()
            return {"status": "not_available"}
        except Exception as e:
            return {"status": "error", "error": str(e)}

    async def _get_health_check_data(self) -> Dict[str, Any]:
        """Получение данных Health Check"""
        try:
            if self.health_check_system:
                await self.health_check_system.initialize()
                return await self.health_check_system.get_health_status()
            return {"status": "not_available"}
        except Exception as e:
            return {"status": "error", "error": str(e)}
    ...
    def _initialize_pagination_system(self):
        ...
        try:
            if self.pagination_enabled:
                self.pagination_system = get_pagination_system()
                success = initialize_pagination_system()
                if success:
    ...
    def search_functions_advanced(self, query: str, search_fields: List[str] = None) -> Dict[str, Any]:
        """Расширенный поиск функций"""
        try:
            if self.search_indexer:
                results = search_functions(query, search_fields)
                return {
                    "query": query,
                    "results": results,
                    "total_results": sum(len(field_results) for field_results in results.values())
                }
            return {"error": "Search system not initialized"}
        except Exception as e:
            return {"error": str(e)}

    def paginate_functions(self, page: int = 1, per_page: int = 10, 
