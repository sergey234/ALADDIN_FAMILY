...
class SecurityValidator:
    ...
    def validate_emergency_description(description: Union[str, None]) -> bool:
        ...
        try:
            ...
            if not isinstance(description, str):
                logger.warning(
                    f"validate_emergency_description: Expected string, got {type(description)}"
                )
                return False
    ...
    def validate_contains_suspicious_content(text: Union[str, None]) -> bool:
        ...
        try:
            ...
            if not isinstance(text, str):
                logger.warning(
                    f"validate_contains_suspicious_content: Expected string, got {type(text)}"
                )
                return False
    ...
    def generate_event_hash(event_data: Union[Dict[str, Any], None]) -> str:
        ...
        try:
            ...
            if not isinstance(event_data, dict):
                logger.warning(
                    f"generate_event_hash: Expected dict, got {type(event_data)}"
                )
                return ""
    ...
    def generate_contact_hash(
        ...
        try:
            ...
            if not isinstance(contact_data, dict):
                logger.warning(
                    f"generate_contact_hash: Expected dict, got {type(contact_data)}"
                )
                return ""
    ...
    def validate_emergency_request(
        ...
        try:
            ...
            if not isinstance(request_data, dict):
                logger.warning(
                    f"validate_emergency_request: Expected dict, got {type(request_data)}"
                )
                return False
    ...
    def run_comprehensive_tests() -> Dict[str, Any]:
        ...
        validator_tests = [
            ...
            (
                ...
                lambda: not SecurityValidator.validate_emergency_description(
                    "test"
                ),
            ),
            (
                "validate_emergency_description_none",
                lambda: not SecurityValidator.validate_emergency_description(None),
            ),
            (
            ...
            (
                ...
                lambda: not SecurityValidator.validate_input_length("", 5, 10),
            ),
            (
            ...
            (
                ...
                lambda: not SecurityValidator.validate_contains_suspicious_content(
                    "Hello world"
                ),
            ),
        ]
        ...
        utils_tests = [
            ...
            (
                ...
                lambda: not EmergencySecurityUtils.validate_emergency_request(
                    {"emergency_type": "fire"}
                ),
            ),
            (
                "validate_emergency_request_none",
                lambda: not EmergencySecurityUtils.validate_emergency_request(None),
            ),
        ]
...
if __name__ == "__main__":
    ...
    print(
        f"📊 Синхронные тесты: {sync_results['passed_tests']}/{sync_results['total_tests']} пройдено"
    )
    ...
    print(
        f"📊 Асинхронные тесты: {async_results['passed_tests']}/{async_results['total_tests']} пройдено"
    )
    ...
    print(
        f"🎯 Общая статистика: {total_passed}/{total_tests} тестов пройдено ({success_rate:.1f}%)"
    )
    ...
    if success_rate == 100:
        ...
        print(
            f"⚠️  {total_tests - total_passed} тестов не пройдено. Требуется доработка."
        )
