...
class NotificationsViewModel: ObservableObject {
    ...
    func loadNotifications() async {
        ...
        case .failure:
            ...
            await MainActor.run {
                ...
                errorMessage = "Не удалось загрузить уведомления. Используются сохраненные данные."
                isLoading = false
            }
        }
    }
    ...
    private func loadMockNotifications() {
        notifications = [
            AppNotification(id: UUID().uuidString, icon: "🛡️", title: "Угроза заблокирована", message: "Заблокирован вредоносный сайт", time: "5 мин назад", isRead: false, type: .threat),
            AppNotification(id: UUID().uuidString, icon: "✅", title: "VPN подключён", message: "Ваше соединение защищено", time: "1 час назад", isRead: true, type: .success),
            AppNotification(id: UUID().uuidString, icon: "⚠️", title: "Подозрительная активность", message: "Обнаружена попытка доступа", time: "2 часа назад", isRead: true, type: .warning),
            AppNotification(id: UUID().uuidString, icon: "ℹ️", title: "Обновление доступно", message: "Доступна новая версия ALADDIN", time: "Вчера", isRead: true, type: .info)
        ]
        updateUnreadCount()
    }
