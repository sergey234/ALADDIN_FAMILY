    ...
    private var contactMethods: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 8) {
                ...
                contactButton(icon: "🤖", title: localizationManager.localized("support_ai_assistant"), subtitle: localizationManager.localized("support_ai_assistant_subtitle"), color: .green)
                contactButton(icon: "📱", title: localizationManager.localized("support_phone"), subtitle: AppConfig.supportPhone, color: .orange)
            }
            .padding(.horizontal, 20)
        }
        .accessibilityElement(children: .contain)
    ...
    private func contactButton(icon: String, title: String, subtitle: String, color: Color) -> some View {
        Button(action: {
            if title == localizationManager.localized("support_chat") {
                // Открыть чат с AI поддержкой в Telegram
                if let url = URL(string: AppConfig.supportTelegramURL) {
                    UIApplication.shared.open(url)
                }
            } else if title == localizationManager.localized("support_ai_assistant") {
                // Открыть AI ассистента в приложении для отправки пожеланий и рекомендаций
                navigationManager.navigateTo(.aiAssistant)
            } else if title == localizationManager.localized("support_phone") {
                // Позвонить
                let phoneNumber = AppConfig.supportPhone.replacingOccurrences(of: " ", with: "").replacingOccurrences(of: "(", with: "").replacingOccurrences(of: ")", with: "").replacingOccurrences(of: "-", with: "")
