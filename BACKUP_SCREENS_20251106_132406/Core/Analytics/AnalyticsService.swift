...
struct ThreatTypeCount: Codable, Identifiable {
    let id: UUID
    let type: String // "Фишинговые сайты", "Вредоносные файлы", etc.
    let count: Int
    let icon: String
    
    init(id: UUID = UUID(), type: String, count: Int, icon: String) {
        self.id = id
        self.type = type
        self.count = count
        self.icon = icon
...
final class LocalAnalyticsService: AnalyticsService {
    ...
    func fetchSecurityAnalytics(period: String) async throws -> SecurityAnalytics {
        ...
        return SecurityAnalytics(
            blockedThreats: [
                ThreatTypeCount(type: "Фишинговые сайты", count: 542, icon: "🌐"),
                ThreatTypeCount(type: "Вредоносные файлы", count: 318, icon: "📁"),
                ThreatTypeCount(type: "Подозрительные приложения", count: 187, icon: "📱"),
                ThreatTypeCount(type: "Опасные ссылки", count: 200, icon: "🔗")
            ],
            recentThreats: [
