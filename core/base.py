...
class SecurityBase(CoreBase):
    ...
    def add_security_rule(self, operation: str, rule: Dict[str, Any]) -> bool:
        ...
        self.log_activity(
            f"Добавлено правило безопасности для операции {operation}")
        return True
    ...
    def _initialize_security_rules(self):
        ...
        for rule in basic_rules:
            self.add_security_rule(rule["name"], rule)
    ...
    def add_security_event(self,
                           ...
                           metadata: Optional[Dict[str,
                                                   Any]] = None):
        """
        Добавление события безопасности в журнал
        ...
        self.log_activity(
            f"Событие безопасности: {event_type} - {description}")

    def get_security_events(self,
                            event_type: Optional[str] = None,
                            severity: Optional[str] = None) -> List[Dict[str,
                                                                         Any]]:
        """
        Получение событий безопасности с фильтрацией
    ...
    def clear_security_events(self, older_than_days: Optional[int] = None):
        """
        Очистка старых событий безопасности

        Args:
            older_than_days: Удалить события старше указанного количества дней
        """
        if older_than_days is None:
            self.activity_log.clear()
            self.log_activity("Очищен весь журнал событий безопасности")
        else:
            cutoff_date = datetime.now() - timedelta(days=older_than_days)
            cutoff_iso = cutoff_date.isoformat()

            initial_count = len(self.activity_log)
            self.activity_log = [
                event for event in self.activity_log
                if event["timestamp"] > cutoff_iso
            ]

            removed_count = initial_count - len(self.activity_log)
            self.log_activity(
                f"Удалено {removed_count} старых событий безопасности")
