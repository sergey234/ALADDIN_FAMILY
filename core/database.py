...
class DatabaseManager(CoreBase):
    ...
    def add_security_event(
        ...
        return self.execute_update(
            query,
            (event_type, severity, description, source, event_data_json)
        )
    ...
    def get_security_events(
        ...
        if resolved is not None:
            query = (
                "SELECT * FROM security_events WHERE resolved = ? "
    ...
    def add_threat(
        ...
        query = """
        INSERT INTO threats
        (threat_type, threat_name, description, severity, confidence,
         threat_data)
        VALUES (?, ?, ?, ?, ?, ?)
        """
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        # Безопасный список разрешенных таблиц для предотвращения SQL инъекций
        allowed_tables = [
            "users",
            "security_events",
            "threats",
        ...
        for table in allowed_tables:
            if not self._is_valid_table_name(table):
                self.log_activity(
                    f"Попытка доступа к недопустимой таблице: {table}",
                    "warning"
                )
                continue
    ...
    def _is_valid_table_name(self, table_name: str) -> bool:
        ...
        Returns:
            bool: True если имя таблицы безопасно, False иначе
        """
        ...
        sql_keywords = {
            ...
            'function', 'union', 'join', 'where', 'from', 'group', 'order',
            'having', 'limit', 'offset', 'distinct', 'as', 'in', 'exists',
            'null', 'not', 'and', 'or', 'like', 'between', 'is'
        }
