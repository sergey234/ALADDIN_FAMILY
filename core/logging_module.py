...
class LoggingManager(CoreBase):
            ...
            for log_entry in self.log_entries:
                if any(
                    [
                        ...
                        "auth" in log_entry.component.lower(),
                        "security" in log_entry.message.lower(),
                    ]
                ):
    ...
    def get_logging_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "oldest_log": (
                self.log_entries[0].timestamp.isoformat()
                if self.log_entries else None
            ),
            "newest_log": (
                self.log_entries[-1].timestamp.isoformat()
                if self.log_entries else None
            ),
        }
