...
# -*- coding: utf-8 -*-
"""
АВТОМАТИЧЕСКАЯ МИГРАЦИЯ БЕЗ ИНТЕРАКТИВНОГО ВВОДА
Строго по 1 файлу за раз с полной проверкой
"""
...
def main():
    ...
    print("⚠️  После каждого файла - полная проверка!")
    print("⚠️  В случае ошибки - автоматический откат!")
    
    # Список файлов для перемещения (осталось 5 файлов)
    files_to_move = [
        {
    ...
    print(f"\n📋 ПЛАН МИГРАЦИИ (осталось {len(files_to_move)} файлов):")
    print("-" * 50)
    ...
    for i, file_info in enumerate(files_to_move, 1):
        ...
        print(f"УСПЕШНО: {success_count}/{total_files} файлов перемещено")
        print(f"{'='*50}")
