...
# -*- coding: utf-8 -*-
"""
Быстрый анализ flake8 для всех 326 функций SFM системы
Анализирует файлы пакетами и создает индивидуальные отчеты
"""
...
def create_individual_report(function_num, file_path, category, total_errors, errors, status, full_output):
    """Создание индивидуального отчета"""
    os.makedirs('INDIVIDUAL_REPORTS', exist_ok=True)
    
    safe_filename = str(file_path).replace('/', '_').replace('\\', '_').replace('.py', '')
    ...
    with open(report_filename, 'w', encoding='utf-8') as f:
        ...
        if total_errors == 0:
            ...
            if full_output:
                ...
                f.write("```\n")
                f.write(full_output[:1000] + ("..." if len(full_output) > 1000 else ""))
                f.write("\n```\n")
        
        f.write("\n---\n")
        f.write(f"**Отчет создан:** AI Security Assistant  \n")
        f.write(f"**Дата:** {datetime.now().isoformat()}  \n")
        f.write(f"**Функция #{function_num}**\n")
...
def get_error_description(error_code):
    ...
    descriptions = {
        ...
        'E305': 'Ожидается 2 пустые строки после определения класса или функции'
    }
...
def get_error_fix_recommendation(error_code):
    ...
    recommendations = {
        ...
        'E305': 'Добавить 2 пустые строки после определения класса или функции'
    }
    return recommendations.get(error_code, f'Исправить ошибку {error_code}')

def main():
    """Основная функция"""
    print("🔍 БЫСТРЫЙ АНАЛИЗ FLAKE8 ДЛЯ ВСЕХ 326 ФУНКЦИЙ SFM СИСТЕМЫ")
    print("=" * 80)
    ...
    # Анализируем файлы пакетами по 50 штук
    batch_size = 50
    for batch_start in range(0, len(all_python_files), batch_size):
        batch_end = min(batch_start + batch_size, len(all_python_files))
        batch_files = all_python_files[batch_start:batch_end]
        
        print(f"\n📦 ПАКЕТ {batch_start//batch_size + 1}: файлы {batch_start + 1}-{batch_end}")
        print("-" * 60)
        
        for i, filepath in enumerate(batch_files, batch_start + 1):
            category = categorize_file(filepath)
            
            print(f"[{i:3d}/{len(all_python_files)}] {filepath}")
            
            # Запуск flake8
            returncode, stdout, stderr = run_flake8_quick(filepath)
            ...
            # Создание индивидуального отчета
            create_individual_report(i, filepath, category, total_errors, errors, status, stdout)
        ...
        # Сохраняем промежуточные результаты после каждого пакета
        ...
        with open(intermediate_file, 'w', encoding='utf-8') as f:
            json_results = {
                ...
                'results_so_far': {
                    'total_files_analyzed': len(results['individual_reports']),
        ...
        print(f"💾 Промежуточные результаты сохранены в {intermediate_file}")
    
    # Создание общего отчета
    create_summary_report(results)
    
    # Сохранение финальных JSON данных
