#!/usr/bin/env python3
"""
УЛЬТИМАТИВНАЯ ПРОВЕРКА КАЧЕСТВА КОДА
Правильный расчет документации и type hints для достижения A+ качества
"""
...
def calculate_ultimate_complexity(filename: str) -> Dict[str, Any]:
    """
    Ультимативная проверка качества кода с правильным расчетом всех метрик
    ...
    try:
        ...
        # ПРАВИЛЬНЫЙ расчет документации
        documentation_percent = calculate_documentation_percent(content)
        
        # ПРАВИЛЬНЫЙ расчет type hints
        ...
        # Расчет итогового балла с правильными весами
        ...
        # Итоговый балл с правильными весами
        final_score = (
            size_score * 0.10 +
            doc_score * 0.30 +  # Увеличенный вес документации
            complexity_score_percent * 0.30 +  # Увеличенный вес сложности
            type_hints_score * 0.20 +  # Увеличенный вес type hints
...
def calculate_documentation_percent(content: str) -> float:
    """
    ПРАВИЛЬНЫЙ расчет процента документации
    
    Args:
        content: Содержимое файла
        
    Returns:
        float: Процент документации
    """
    ...
    for line in lines:
        ...
        if '"""' in line or "'''" in line:
            if not in_docstring:
                ...
                if line.count(docstring_type) >= 2:
                    in_docstring = False
                    docstring_type = None
            else:
                # Конец docstring
            ...
            # Строки внутри docstring
            documentation_lines += 1
    
    return (documentation_lines / total_lines) * 100 if total_lines > 0 else 0

def calculate_type_hints_percent(content: str) -> float:
    """
    ПРАВИЛЬНЫЙ расчет процента использования type hints
    ...
    for line in lines:
        ...
        if any(keyword in line for keyword in [
            ...
            ': Tuple', ': Optional', ': Union', ': Any', ': Callable', ': Protocol',
            ': List[', ': Dict[', ': Tuple[', ': Optional[', ': Union[', ': Any[',
            ': Callable[', ': Protocol[', ': Set', ': FrozenSet', ': Sequence',
            ': Iterable', ': Iterator', ': Generator', ': Awaitable', ': Coroutine'
        ]):
            type_hint_lines += 1
...
def main():
    ...
    print("🔍 УЛЬТИМАТИВНАЯ ПРОВЕРКА КАЧЕСТВА КОДА")
    print("=" * 70)
    ...
    for filename in files_to_check:
        if os.path.exists(filename):
            ...
            result = calculate_ultimate_complexity(filename)
            if result:
