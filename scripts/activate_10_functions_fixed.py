...
# -*- coding: utf-8 -*-
"""
ИСПРАВЛЕННЫЙ скрипт активации 10 функций с мониторингом "было-стало"
Сохраняет состояние между запусками и показывает реальную накопительную статистику
"""
...
STATE_FILE = "/Users/sergejhlystov/ALADDIN_NEW/scripts/activation_state.pkl"
...
def print_comparison_with_state(before_stats, after_stats, before_sys, after_sys, before_perf, after_perf, activated_count, state):
    """Вывод сравнения до и после активации с учетом накопительного состояния"""
    print("=" * 80)
    ...
    # НАКОПИТЕЛЬНАЯ статистика активации
    total_activated_this_session = state.get('total_activated_this_session', 0) + activated_count
    total_sleeping_remaining = after_stats['sleeping_count']
    initial_sleeping = state.get('initial_sleeping_count', after_stats['sleeping_count'] + activated_count)
    progress_percent = (total_activated_this_session / initial_sleeping * 100) if initial_sleeping > 0 else 0
    
    print(f"\n📈 НАКОПИТЕЛЬНЫЙ ПРОГРЕСС АКТИВАЦИИ:")
    print(f"   Активировано в этом запуске: {activated_count}")
    print(f"   Активировано в этой сессии: {total_activated_this_session}")
    print(f"   Прогресс сессии: {progress_percent:.1f}% ({total_activated_this_session}/{initial_sleeping})")
    print(f"   Осталось спящих: {total_sleeping_remaining}")
    
    print("=" * 80)

def main():
    print("🚀 ИСПРАВЛЕННАЯ АКТИВАЦИЯ 10 ФУНКЦИЙ С НАКОПИТЕЛЬНОЙ СТАТИСТИКОЙ")
    print()
    ...
    if state is None:
        state = {
            'total_activated_this_session': 0,
            'initial_sleeping_count': None,
            'session_start_time': time.time()
        }
        print("🆕 Начало новой сессии активации")
    else:
        print(f"📊 Продолжение сессии: уже активировано {state.get('total_activated_this_session', 0)} функций")
    ...
    # Сохраняем начальное состояние для новой сессии
    if state['initial_sleeping_count'] is None:
        state['initial_sleeping_count'] = before_stats['sleeping_count']
    ...
    # Показываем сравнение до/после с накопительной статистикой
    print_comparison_with_state(
        before_stats, after_stats,
        before_sys, after_sys,
        before_perf, after_perf,
        activated_count, state
    )
    
    # Обновляем состояние
    state['total_activated_this_session'] += activated_count
    save_state(state)
    
    print(f"\n🎉 ЗАВЕРШЕНО!")
    print(f"📊 Активировано: {activated_count} из {len(sleeping_functions)} функций")
    print(f"📈 Всего активировано в сессии: {state['total_activated_this_session']}")
