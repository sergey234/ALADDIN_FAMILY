#!/usr/bin/env python3
"""
Анализ ВСЕХ backup файлов включая повторения и дубликаты
"""
...
def analyze_all_backups():
    ...
    print("🔍 ПОЛНЫЙ АНАЛИЗ ВСЕХ 40 BACKUP ФАЙЛОВ")
    print("=" * 80)
    ...
    for i, backup_file in enumerate(backup_files, 1):
        ...
        duplicates_analysis[base_name].append({
            ...
            'backup_lines': comparison['size_comparison']['backup_lines'],
            'original_lines': comparison['size_comparison']['original_lines']
        })
    ...
    # Анализ дубликатов
    print(f"\n🔄 АНАЛИЗ ДУБЛИКАТОВ И ПОВТОРЕНИЙ")
    print("=" * 80)
    ...
    for base_name, duplicates in duplicates_analysis.items():
        if len(duplicates) > 1:
            ...
            for dup in duplicates:
                print(f"  • {dup['backup_file']}")
                print(f"    Размер: {dup['backup_size']:,} байт, Строк: {dup['backup_lines']}")
                print(f"    Различия: {dup['diff_ratio']:.1f}%")
            
            # Анализируем размеры
            sizes = [dup['backup_size'] for dup in duplicates]
            lines = [dup['backup_lines'] for dup in duplicates]
            
            if len(set(sizes)) == 1 and len(set(lines)) == 1:
                print(f"  🔍 ВСЕ ВЕРСИИ ИДЕНТИЧНЫ - это временные копии одного файла")
            elif len(set(sizes)) == 2:
                print(f"  🔍 ДВА РАЗНЫХ РАЗМЕРА - возможно разные версии функциональности")
                unique_sizes = list(set(sizes))
                print(f"    Размеры: {unique_sizes[0]:,} и {unique_sizes[1]:,} байт")
            else:
                print(f"  🔍 РАЗНЫЕ РАЗМЕРЫ - разные версии функциональности")
                print(f"    Размеры: {', '.join([f'{s:,}' for s in sorted(set(sizes))])} байт")
    
    # Сохраняем полный отчет
    report_file = backup_dir / f"ALL_BACKUP_ANALYSIS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
