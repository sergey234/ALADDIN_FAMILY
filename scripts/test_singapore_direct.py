...
# -*- coding: utf-8 -*-
"""
Прямой тест Singapore VPN без веб-интерфейса
"""
...
async def test_singapore_direct():
    """Прямой тест Singapore VPN"""
    print("📱 ПРЯМОЙ ТЕСТ SINGAPORE VPN")
    print("=" * 40)
    
    try:
        # Создание VPN системы
        print("1. Создание VPN Security System...")
        vpn_system = VPNSecuritySystem("DirectTestVPN")
        ...
        test_user = f"direct_test_user_{int(time.time())}"
        ...
        print("\n🎉 ПРЯМОЙ ТЕСТ SINGAPORE VPN ЗАВЕРШЕН УСПЕШНО!")
...
async def main():
    """Основная функция"""
    print("📱 VPN SECURITY SYSTEM - ПРЯМОЙ ТЕСТ SINGAPORE")
    print("=" * 60)
    print(f"Время начала: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
    
    # Тестирование VPN
    success = await test_singapore_direct()
    
    print("\n" + "=" * 60)
    print(f"Время завершения: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    if success:
        print("✅ ПРЯМОЙ ТЕСТ SINGAPORE VPN ЗАВЕРШЕН УСПЕШНО!")
        print("\n🎯 ВАРИАНТЫ ДЛЯ ТЕЛЕФОНА:")
        print("1. 📱 vpn_test_fixed.html - исправленная версия")
        print("2. 📱 vpn_test_simple_no_js.html - без JavaScript")
        print("3. 📱 QR-код - отсканируйте камерой")
        print("4. 📱 Прямое подключение - введите URL")
    else:
        print("❌ ПРЯМОЙ ТЕСТ SINGAPORE VPN ЗАВЕРШЕН С ОШИБКАМИ!")
    print("=" * 60)
