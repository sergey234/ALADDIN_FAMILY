#!/usr/bin/env python3
"""
Полная таблица сравнения всех 40 backup файлов с оригиналами
"""
...
def create_full_comparison_table():
    ...
    print('📊 ПОЛНАЯ ТАБЛИЦА СРАВНЕНИЯ: 40 BACKUP ФАЙЛОВ vs ОРИГИНАЛЫ')
    print('=' * 140)
    print(f'{"№":<3} | {"BACKUP ФАЙЛ":<45} | {"РАЗМЕР":<8} | {"ОРИГИНАЛ":<45} | {"РАЗМЕР":<8} | {"СТАТУС":<12}')
    print('-' * 140)
    ...
    for i, backup_file in enumerate(backup_files, 1):
        ...
        original_display = original_name[:42] + '...' if len(original_name) > 45 else original_name
        backup_display = backup_name[:42] + '...' if len(backup_name) > 45 else backup_name
        
        print(f'{i:<3} | {backup_display:<45} | {backup_size:<8} | {original_display:<45} | {original_size:<8} | {status:<12}')
