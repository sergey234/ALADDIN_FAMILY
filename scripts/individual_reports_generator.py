...
# -*- coding: utf-8 -*-
"""
Генератор индивидуальных отчетов по каждой функции
Создает отчеты с нумерацией 1-2-3-4-5...
"""
...
def create_individual_report(function_num, file_path, category, total_errors, errors, status, full_output):
    ...
    with open(report_filename, 'w', encoding='utf-8') as f:
        ...
        if total_errors == 0:
            ...
            if full_output and len(full_output) > 0:
                f.write("### 📝 Детальный вывод flake8:\n\n")
                f.write("```\n")
                # Ограничиваем вывод первыми 1000 символами
                output_preview = full_output[:1000]
                if len(full_output) > 1000:
                    output_preview += "\n... (показаны первые 1000 символов)"
                f.write(output_preview)
                f.write("\n```\n")
...
def main():
    """Основная функция"""
    print("🔍 СОЗДАНИЕ ИНДИВИДУАЛЬНЫХ ОТЧЕТОВ ПО ВСЕМ ФУНКЦИЯМ")
    print("=" * 70)
    print("Создаем отчеты с нумерацией 1-2-3-4-5...")
    print("=" * 70)
    ...
    print(f"\n🔍 СОЗДАНИЕ ОТЧЕТОВ ДЛЯ {len(all_python_files)} ФАЙЛОВ:")
    print("-" * 70)
    ...
    for i, filepath in enumerate(all_python_files, 1):
        ...
        print(f"     {status} ({category}) - Отчет создан")
