...
# -*- coding: utf-8 -*-
"""
Скрипт перевода SpeechRecognitionEngine в спящий режим
Создан: 2024-09-05
Версия: 1.0.0
"""
...
def put_speech_recognition_to_sleep():
    """Перевод SpeechRecognitionEngine в спящий режим"""
    print("😴 ПЕРЕВОД SPEECHRECOGNITIONENGINE В СПЯЩИЙ РЕЖИМ")
    print("=" * 60)
    ...
    try:
        ...
        print(f"📊 Текущий статус: {speech_engine.status}")
        
        # Переводим в спящий режим
        speech_engine.status = "SLEEP"
        speech_engine.last_update = datetime.now()
        
        print(f"✅ Статус изменен на: {speech_engine.status}")
        print(f"🕐 Время перевода: {speech_engine.last_update}")
        ...
        config_file = "data/sleep_mode/speech_recognition_sleep_config.json"
        ...
        # Создаем лог
        log_file = f"logs/speech_recognition_sleep_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        os.makedirs("logs", exist_ok=True)
        
        with open(log_file, 'w', encoding='utf-8') as f:
            f.write(f"SpeechRecognitionEngine переведен в спящий режим\n")
            f.write(f"Время: {datetime.now().isoformat()}\n")
            f.write(f"Качество: {quality_score:.1f}/100\n")
            f.write(f"Статус: {speech_engine.status}\n")
        
        print(f"📝 Лог создан: {log_file}")
        
        print(f"\n🎉 SPEECHRECOGNITIONENGINE УСПЕШНО ПЕРЕВЕДЕН В СПЯЩИЙ РЕЖИМ!")
        print(f"   📊 Качество: {quality_score:.1f}/100")
        print(f"   😴 Статус: SLEEP")
        print(f"   🕐 Время: {speech_engine.last_update}")
...
if __name__ == "__main__":
    success = put_speech_recognition_to_sleep()
    if success:
