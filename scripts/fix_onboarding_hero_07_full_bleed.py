#!/usr/bin/env python3
"""OB_07 full-bleed like OB_03: content width ~369, L/R≈12, top≈70.

Rebuild 393×852 PNG from zone, then iOS uses scaledToFill + zoom 1.09 (same as OB_02/03).
"""
...
IMAGESET = IOS / "Assets.xcassets" / "OnboardingHero_07.imageset" / "OnboardingHero_07.png"
W, H = 393, 852
TARGET_CONTENT_W = 369
TARGET_TOP = 70
PASTE_X = 12
BG = (10, 17, 40)
...
def build_ob07_full_bleed(zone_path: Path) -> Image.Image:
...
def main() -> None:
    zone = HERO / "OnboardingHero_07_figma_zone_361x460.png"
    if not zone.exists():
        raise SystemExit(f"missing zone: {zone}")

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    for src in (IMAGESET, HERO / "OnboardingHero_07.png"):
        if src.exists():
            shutil.copy2(src, HERO / f"OnboardingHero_07_BEFORE_FULL_BLEED_{stamp}.png")
            break

    frame = build_ob07_full_bleed(zone)
    l, t, r, b = bbox_report(frame)
    print(f"bbox L={l} R={r} top={t} w={r - l + 1} (target L={PASTE_X} w={TARGET_CONTENT_W} top={TARGET_TOP})")

    master = HERO / "OnboardingHero_07.png"
    frame.save(master, "PNG", optimize=True)
    IMAGESET.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(master, IMAGESET)
    print(f"OK OB_07 full-bleed -> {IMAGESET}")
