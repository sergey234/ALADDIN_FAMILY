    ...
    private func tariffCard(_ tariff: TariffType) -> some View {
        VStack(alignment: .leading, spacing: Spacing.m) {
            ...
            // Кнопка выбора/оплаты
                ...
                addLog("Выбран тариф: \(tariff.title(localizationManager: localizationManager))")
                ...
                print("🔍 Выбран тариф: \(tariff.title(localizationManager: localizationManager))")
                ...
                let tariffObj: Tariff = {
                    ...
                    let safeTitle = tariff.title(localizationManager: localizationManager).isEmpty ? "Тариф \(tariffId)" : tariff.title(localizationManager: localizationManager)
                    let safePrice = tariff.price.isEmpty ? "0 ₽" : tariff.price
                    let safePeriod = tariff.period(localizationManager: localizationManager).isEmpty ? localizationManager.localized("tariffs_period_month") : tariff.period(localizationManager: localizationManager)
                    let safeFeatures = tariff.features(localizationManager: localizationManager).isEmpty ? [localizationManager.localized("tariffs_free_features_1")] : tariff.features(localizationManager: localizationManager)
                ...
                if AppConfig.useAlternativePayments {
                    ...
                    print("🇷🇺 Регион: Россия → Открываем QR-оплату для тарифа: \(tariff.title(localizationManager: localizationManager))")
            ...
            Button(action: {
                ...
                HStack(spacing: Spacing.m) {
                    ...
                    VStack(alignment: .leading, spacing: Spacing.xxs) {
                        Text(localizationManager.localized("tariffs_ai_protection"))
                            .font(.h3)
                            .foregroundColor(.textPrimary)
                            .multilineTextAlignment(.leading)
                        
                        Text(localizationManager.localized("tariffs_ai_protection_desc"))
                            .font(.caption)
                            .foregroundColor(.textSecondary)
                            .multilineTextAlignment(.leading)
                    }
...
struct TariffComparisonModal: View {
    ...
    private var comparisonTable: some View {
        VStack(spacing: Spacing.s) {
            ...
            comparisonRow(title: localizationManager.localized("tariffs_comparison_vpn"), values: ["50МБ/дн", "∞", "∞", "∞"])
