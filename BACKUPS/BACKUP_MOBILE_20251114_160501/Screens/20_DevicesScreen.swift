...
struct AddDeviceView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .onAppear {
                loadFamilyMembers()
                // ✅ ДИАГНОСТИКА: Проверяем состояние после загрузки
                print("🔍 AddDeviceView.onAppear:")
                print("   - Члены семьи загружены: \(familyMembers.count)")
                print("   - Выбранный владелец: '\(selectedOwner)'")
                print("   - Форма валидна: \(isFormValid)")
            }
