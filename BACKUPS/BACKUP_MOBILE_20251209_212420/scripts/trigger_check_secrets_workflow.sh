...
# Скрипт для автоматического запуска workflow "Build and Upload to App Store Connect"
...
WORKFLOW_FILE="check-secrets.yml"
BRANCH="master"
...
echo -e "${GREEN}🚀 Запуск workflow для сборки и загрузки IPA в App Store Connect${NC}"
echo ""
...
if [ -z "$GITHUB_TOKEN" ]; then
    ...
    echo "  ./scripts/trigger_check_secrets_workflow.sh"
    echo ""
    echo "Или откройте страницу вручную:"
    echo "  https://github.com/sergey234/ALADDIN_FAMILY/actions/workflows/check-secrets.yml"
    echo ""
    exit 1
fi
...
if [ "$HTTP_CODE" = "204" ]; then
    ...
    echo ""
    echo "⏳ Workflow выполняется (~10-15 минут)"
