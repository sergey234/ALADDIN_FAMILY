...
class IoTSecurityModuleTests: XCTestCase {
    ...
    func testAlertCompromised() {
        ...
        // Act
        module.alertCompromised(device)
        
        // Assert
        // Проверяем, что уведомление было отправлено
        // (в реальном тесте можно проверить NotificationManager)
        XCTAssertTrue(true, "Уведомление должно быть отправлено")
    }
...
class MockAPIService: APIService {
    ...
    override func blockIoTDevice(deviceId: String) async throws -> APIResponse<Bool> {
        ...
        return APIResponse(success: shouldBlockDevice, data: shouldBlockDevice, message: nil)
    }
}
