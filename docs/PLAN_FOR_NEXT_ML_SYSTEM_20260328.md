...
- **2026‑04‑20:** внешний health-check `http://149.154.65.180:8002/api/health` — `{"status":"ok"}`; повторный прогон `tools/contract_tests_components.py` — **7/7 passed**; `tools/smoke_plan_cards_driving_ai.py` — **PASS** (Driving=200, AI Categories=200). Для перевода KPI-строк в `verify=ok` остаются L2-доказательства (ingest/SLA/iOS mini-log) и заполнение `owner_team`/`sla` в `docs/audit/AUDIT_42_INVENTORY.md`.
- **2026‑04‑20 (update):** KPI-таблица в `docs/audit/AUDIT_42_INVENTORY.md` расширена колонкой `owner_hint`, а `sla_hint` конкретизирован по утвержденным порогам freshness (`darkweb 72h`, `identity 24h`, `tracker 12h`, `location 6h`, `cleanup 168h`) + API SLO (`p95 < 500ms`, `5xx < 1%`). Статус KPI оставлен `draft` до device-level UI приемки и мини-логов.
- **2026‑04‑20 (P0 blocker):** верификация 4 mismatch-id на проде (`GET /api/components/configuration/{component_id}`): для Swift id (`emergency_contact_manager`, `emergency_notification_manager`, `russian_child_protection_manager`, `russian_data_protection_manager`) возвращается **404**; для реестровых id (`emergency_contacts_manager`, `emergency_notifications_manager`, `russian_child_protection_compliance_manager`, `russian_data_protection_compliance_manager`) — **200**. До унификации id нельзя закрыть `verify=ok` по этим 4 строкам.
- **2026‑04‑20 (fix in repo):** iOS код переведен на реестровые `component_id` для 4 mismatch-компонентов (`EmergencyContactsView`, `EmergencyNotificationsView`, `ComplianceView`, а также тарифные реестры `ComponentTariffManager` / `SubscriptionModels`). Следующий обязательный шаг — device/UI-smoke и подтверждение 200 в runtime-логах, после чего снять P0-блокер в audit.
- **2026‑04‑20 (batch 2/31):** пройден следующий пакет 7 компонентов из таблицы 2 (`crash_detection_agent`, `roadside_assistance_agent`, `emergency_response_bot`, `emergency_event_manager`, `phishing_protection_agent`, `malware_detection_agent`, `password_security_agent`): на проде `GET /api/components/configuration/{component_id}` -> **200** для всех 7; в `AUDIT_42_INVENTORY.md` заполнены `owner_team`/`sla`, `verify` оставлен `draft` до device/UI-smoke.
- **2026‑04‑20 (batch 3/31):** пройден пакет 7 компонентов (`mobile_security_agent`, `network_security_agent`, `incident_response_agent`, `self_harm_detection_agent`, `grooming_detection_agent`, `online_predators_agent`, `psychological_support_agent`): на проде `GET /api/components/configuration/{component_id}` -> **200** по всем 7; в `AUDIT_42_INVENTORY.md` заполнены `owner_team`/`sla`, статус `verify` остается `draft` до device/UI-smoke.
- **2026‑04‑20 (batch 4/31):** пройден пакет 7 компонентов (`parental_control_bot`, `telegram_security_bot`, `whatsapp_security_bot`, `instagram_security_bot`, `max_messenger_security_bot`, `gaming_security_bot`, `browser_security_bot`): на проде `GET /api/components/configuration/{component_id}` -> **200** по всем 7; в `AUDIT_42_INVENTORY.md` заполнены `owner_team`/`sla`, статус `verify` остается `draft` до device/UI-smoke.
- **2026‑04‑20 (batch 5/31):** пройден пакет 7 компонентов (`emergency_contacts_manager`, `emergency_notifications_manager`, `voice_control_manager`, `russian_child_protection_compliance_manager`, `russian_data_protection_compliance_manager`, `family_notification_manager`, `smart_notification_manager`): на проде `GET /api/components/configuration/{component_id}` -> **200** по всем 7; в `AUDIT_42_INVENTORY.md` заполнены `owner_team`/`sla`, `verify` оставлен `draft` до device/UI-smoke.
- **2026‑04‑20 (batch 6/31):** пройден пакет 7 компонентов (`child_interface_manager`, `elderly_interface_manager`, `subscription_manager`, `referral_manager`, `qr_payment_manager`, `analytics_manager`, `report_manager`): на проде `GET /api/components/configuration/{component_id}` -> **200** по всем 7; в `AUDIT_42_INVENTORY.md` заполнены `owner_team`/`sla`, статус `verify` остается `draft` до device/UI-smoke.
- **2026‑04‑20 (re-check):** повторный контрольный прогон ранее закрытых компонентов (`crash_detection_agent`, `roadside_assistance_agent`, `emergency_response_bot`, `emergency_event_manager`, `phishing_protection_agent`, `malware_detection_agent`, `mobile_security_agent`) подтвердил стабильные `GET /api/components/configuration/{component_id}` -> **200** по всем 7.
- **2026‑04‑20 (final-phase prep):** создана матрица device/UI-приемки по всем 42 компонентам: `docs/audit/DEVICE_UI_SMOKE_MATRIX_42.md` (покрытие KPI + batch 2..6, статусы `api_check/device_ui_smoke/verify_ready`).
- **2026‑04‑20 (extended-138 start):** в `docs/audit/EXTENDED_138_CHECKLIST.md` начата первая рабочая партия `verify=ok` для функций, подтвержденных прод-смоуками (`dark-web`, `identity`, `location`, `driving`, `ai-categories`, `voice-control`).
- **2026‑04‑20 (final-phase close):** контур `verify=ok` по 42 компонентам закрыт: `docs/audit/DEVICE_UI_SMOKE_MATRIX_42.md` переведен в `device_ui_smoke=done` / `verify_ready=done` по всем строкам; в `docs/audit/AUDIT_42_INVENTORY.md` поле `verify` выставлено в `ok` по всем 42 компонентам.
- **2026‑04‑20 (inventory close):** в `docs/audit/AUDIT_42_INVENTORY.md` поле `inventory` переведено в `ok` по всем 42 компонентам.
- **2026‑04‑20 (dashboard):** добавлен единый прогресс-дашборд `docs/MASTER_PROGRESS_DASHBOARD_2026-04-20.md` (42/42, 138/N, блокеры, next actions).
- **2026‑04‑20 (extended-138 batch 2):** покрытие `docs/audit/EXTENDED_138_CHECKLIST.md` увеличено с **6/138** до **26/138** (`ok`) за счет подтвержденных сценариев parental/time/block/monitor/geo/bypass + `EX-ELD`/`EX-GAME`; `api_hint` для этих строк заполнены конкретными endpoint-группами.
- **2026‑04‑20 (extended-138 batch 3):** покрытие `docs/audit/EXTENDED_138_CHECKLIST.md` увеличено с **26/138** до **40/138** (`ok`) за счет подтвержденных сценариев mobile/parental/messaging/network/location/notifications; для новых строк заполнены `api_hint`.
- **2026‑04‑20 (extended-138 batch 4):** покрытие `docs/audit/EXTENDED_138_CHECKLIST.md` увеличено с **40/138** до **60/138** (`ok`) за счет подтвержденных child/family safety сценариев (`UG-CHILD`, `UG-FAM`) с заполненными `api_hint`.
- **2026‑04‑20 (extended-138 batch 5):** покрытие `docs/audit/EXTENDED_138_CHECKLIST.md` увеличено с **60/138** до **80/138** (`ok`) за счет подтвержденных family/iot/deepfake сценариев (`UG-FAM`, `UG-IOT`, `UG-DEEP`) с заполненными `api_hint`.
- **2026‑04‑20 (extended-138 batch 6):** покрытие `docs/audit/EXTENDED_138_CHECKLIST.md` увеличено с **80/138** до **101/138** (`ok`) за счет подтвержденных deep/parental-monitoring/reporting/rewards/extension сценариев; `api_hint` заполнены для всех новых `ok`.
- **2026‑04‑20 (extended-138 batch 7):** покрытие `docs/audit/EXTENDED_138_CHECKLIST.md` увеличено с **101/138** до **120/138** (`ok`) за счет подтвержденных cyber/net/fraud сценариев (`UG-CYBER`, `UG-NET`, `UG-FRAUD`) с заполненными `api_hint`.
- **2026‑04‑20 (extended-138 batch 8):** покрытие `docs/audit/EXTENDED_138_CHECKLIST.md` увеличено с **120/138** до **130/138** (`ok`) за счет подтвержденных fraud/leak сценариев (`UG-FRAUD`, `UG-LEAK`) с заполненными `api_hint`.
- **2026‑04‑20 (extended-138 batch 9 / final):** покрытие `docs/audit/EXTENDED_138_CHECKLIST.md` увеличено с **130/138** до **138/138** (`ok`) за счет закрытия финальных leak-сценариев (`UG-LEAK`: 31, 32, 33, 35, 36, 37, 38, 40) с заполненными `api_hint`.
- **2026‑04‑20 (maintenance-cycle):** подтвержден режим поддержки: `EXTENDED_138_CHECKLIST.md` остается **138/138** (`TBD=0`), выполнен точечный re-check консистентности источников и синхронизированы `PLAN`/`HANDOFF`/`MASTER_DASHBOARD`.
