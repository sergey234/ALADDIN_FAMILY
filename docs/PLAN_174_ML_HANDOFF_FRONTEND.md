...
## 1. Цифры (что значит «174»)

| Показатель | Значение |
|------------|----------|
| Всего задач в плане | **178** |
| **Выполнено (закрытые чекбоксы)** | **174** |
| **Осталось** | **4** (все в **Track B** / governance) |
| Track A (core product) | **149 / 149** (100%) |
| Track B (governance / quality) | **25 / 29** (~86%) — 4 открытые |

**Важно для ML:** «174» — это **счётчик закрытых пунктов** в `EXECUTION_AND_LOCALIZATION_DASHBOARD.md`, а не 174 отдельных **новых** URL бэкенда. Новые маршруты API — в **`/openapi.json`**, дельта между релизами — diff снимков.
...
- **45 / 49** пунктов Phase 9 закрыты: унификация roster, `FamilyPermissionLayer`, зеркала контента/безопасности, **RU/EN** на elderly, smoke RU+EN, accessibility.
- **4 оставшихся** пункта пересекаются с **Track B** (см. §4).
...
## 4. Что **не** входит в 174 (открыто = 4)

По `CURSOR_CHAT_PENDING_CHECKLIST.md` / дашборду:

1. **Wave 1 Baseline cleanup** — линт `localization_lint` (parity + hardcoded) по `docs/LOCALIZATION_BASELINE_BACKLOG.md`.
2. **Размер приложения < 500MB**
3. **COPPA compliance 100%**
4. **Родительский контроль: обязательный** (как отдельный **закрывающий** чекбокс трека; отдельно: автоматика `phase9_access_rules` и permission-слой усиливались в ходе плана).

**Phase 9** в прогресс-файле: **45/49** — несоответствие с «4 open» снимается тем, что **часть** открытого — **сквозные** Track B строки, не только номер 9.x.
...
3. Для **API** — не выводить «всё ОК» из 174: смотреть **OpenAPI LATEST** + **ALADDIN_JWT** (5xx, 422).
4. После закрытия последних 4 — `update_dashboard_stats.py` и при необходимости новый **OpenAPI** прогон.
...
- **100% продуктовой готовности по план-факту** = закрытие **4** открытых строк Track B в `docs/EXECUTION_AND_LOCALIZATION_DASHBOARD.md` / `CURSOR_CHAT_PENDING_CHECKLIST.md` (baseline локализации, размер, COPPA, обязательный родительский контроль) — это **организационно/комплаенс**, не одна правка в Swift.
...
- **Релиз по детям:** клиент **готов** показывать контент и **устойчивую** историю кошелька при корректном `balance/history` и/или локальных данных; **go/no-go** на «жёсткий» прод снимайте по Track B + результатам smokes, не только по цифре 174.
