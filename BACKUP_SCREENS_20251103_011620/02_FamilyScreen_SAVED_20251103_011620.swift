...
struct FamilyScreen: View {
    ...
    private func navigateToMemberScreen(role: FamilyMemberCard.FamilyRole) {
        ...
        // ✅ ИСПРАВЛЕНИЕ: Явное обновление на главном потоке для гарантированного обновления UI
        DispatchQueue.main.async {
            // Navigate based on role
            switch role {
            case .parent:
                print("🔍 DEBUG: Переход к .profile (профиль родителя)")
                print("🔍 DEBUG: Текущий экран ДО перехода: \(self.navigationManager.currentScreen)")
                self.navigationManager.navigateTo(.profile)
                print("🔍 DEBUG: Текущий экран ПОСЛЕ перехода: \(self.navigationManager.currentScreen)")
                print("🔍 DEBUG: Проверка - должен быть .profile")
                
                // ✅ Дополнительная проверка через небольшую задержку
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    print("🔍 DEBUG: Проверка через 0.1 сек - currentScreen = \(self.navigationManager.currentScreen)")
                }
            case .child:
            ...
            case .teenager:
                ...
                print("🔍 DEBUG: Текущий экран ДО перехода: \(self.navigationManager.currentScreen)")
                self.navigationManager.navigateTo(.childInterface) // Simplified interface
                print("🔍 DEBUG: Текущий экран ПОСЛЕ перехода: \(self.navigationManager.currentScreen)")
            case .elderly:
    ...
    private var isFamilyCreator: Bool {
        ...
        if memberCount <= 1 {
            let currentUserName = UserDefaults.standard.string(forKey: currentUserNameKey) ?? "Вы"
            if let firstMember = familyMembers.first,
               firstMember.name == currentUserName || firstMember.name == "Вы" {
                return true
            }
        }
                            ...
                            // Пустое состояние
                                ...
                                VStack(spacing: Spacing.m) {
                                    ...
                                    Button(action: {
                                        ...
                                        Text("➕ Добавить первого участника")
                                            .font(.subheadline)
                                            .fontWeight(.semibold)
