...
struct AnalyticsScreen: View {
    ...
    private var navigationHeader: some View {
        ALADDINNavigationBar(
            ...
            rightButtons: [
                .init(icon: "line.3.horizontal.decrease.circle", accessibilityLabel: localizationManager.localized("analytics_filters_button")) {
                    showFilters = true
                }
            ],
            onBack: {
        ...
        VStack(spacing: Spacing.m) {
            ...
            HStack(spacing: Spacing.s) {
                ForEach(AnalyticsViewModel.TimePeriod.allCases, id: \.self) { period in
                    ...
                    .accessibilityLabel(
                        String(
                            format: localizationManager.localized("analytics_accessibility_period"),
                            periodTitle(period)
                        )
                    )
    ...
    private var detailedStats: some View {
        VStack(spacing: Spacing.m) {
            ...
            VStack(spacing: Spacing.s) {
                detailRow(
                    icon: "globe",
                    title: localizationManager.localized("analytics_web_threats"),
                ...
                detailRow(
                    icon: "doc",
                    title: localizationManager.localized("analytics_file_threats"),
                ...
                detailRow(
                    icon: "network",
                    title: localizationManager.localized("analytics_network_threats"),
                ...
                detailRow(
                    icon: "app",
                    title: localizationManager.localized("analytics_app_threats"),
...
struct DetailedStatsModal: View {
    ...
    var body: some View {
        ...
        .id("detailed_stats_modal_lang_\(localizationManager.currentLanguage.rawValue)")
    }
    ...
    private var tabsView: some View {
        HStack(spacing: Spacing.xs) {
            ForEach(AnalyticsScreen.StatsType.allCases, id: \.self) { type in
                ...
                .accessibilityLabel(
                    String(
                        format: localizationManager.localized("analytics_accessibility_tab"),
                        type.title(localizationManager: localizationManager)
                    )
                )
    ...
    private var securityVPNStats: some View {
        VStack(alignment: .leading, spacing: Spacing.m) {
            ...
            if let vpn = viewModel.securityAnalytics?.vpnStats {
                statRow(title: localizationManager.localized("analytics_today"), value: vpn.today, icon: "📊")
                statRow(title: localizationManager.localized("analytics_week"), value: vpn.week, icon: "📊")
                statRow(title: localizationManager.localized("analytics_protected"), value: vpn.protection, icon: "✅")
            } else {
                Text(localizationManager.localized("analytics_loading"))
                    .foregroundColor(.textSecondary)
                    .padding(.vertical, Spacing.s)
    ...
    // Локализация типов угроз
    private func localizedThreatType(_ type: String) -> String {
        switch type {
        case "Фишинговые сайты": return localizationManager.localized("analytics_phishing_site")
        case "Вредоносные файлы": return localizationManager.localized("analytics_malicious_file")
        case "Подозрительные приложения": return localizationManager.localized("analytics_suspicious_app")
        case "Опасные ссылки": return localizationManager.localized("analytics_dangerous_link")
        default: return type
        }
    }
    
    // Локализация текстов угроз
    private func localizedThreatText(_ text: String) -> String {
        switch text {
        case "Фишинговый сайт": return localizationManager.localized("analytics_phishing_site")
        case "Подозрительное приложение": return localizationManager.localized("analytics_suspicious_app")
        case "Вредоносный файл": return localizationManager.localized("analytics_malicious_file")
        default: return text
        }
    }
    
    // Локализация имен членов семьи
    private func localizedFamilyMemberName(_ name: String) -> String {
        switch name {
        case "Александр": return localizationManager.localized("analytics_family_member_alexander")
        case "Елена": return localizationManager.localized("analytics_family_member_elena")
        case "Алексей": return localizationManager.localized("analytics_family_member_alexey")
        case "Бабушка": return localizationManager.localized("analytics_family_member_grandma")
        case "Мария": return localizationManager.localized("analytics_family_member_maria")
        case "Мама": return localizationManager.localized("analytics_family_member_mom")
        default: return name
        }
    }
    
    // Локализация периодов использования
    private func localizedUsagePeriod(_ period: String) -> String {
        switch period {
        case "Утро (6-12)": return localizationManager.localized("analytics_usage_morning")
        case "День (12-18)": return localizationManager.localized("analytics_usage_day")
        case "Вечер (18-24)": return localizationManager.localized("analytics_usage_evening")
        default: return period
        }
    }
    
    // Локализация активностей семьи
    private func localizedFamilyActivity(_ activity: String) -> String {
        switch activity {
        case "безопасная игра": return localizationManager.localized("analytics_activity_safe_game")
        case "урок безопасности": return localizationManager.localized("analytics_activity_safety_lesson")
        case "родительский контроль": return localizationManager.localized("analytics_activity_parental_control")
    ...
    private func statRow(title: String, value: String, icon: String) -> some View {
        HStack(alignment: .firstTextBaseline, spacing: 8) {
            Text(icon)
                .frame(width: 24)
            Text(localizedThreatType(title))
                .font(.system(size: 13))
                .foregroundColor(.textPrimary)
                .lineLimit(1)
    ...
    private func familyMemberRow(emoji: String, name: String, time: String, percent: Int) -> some View {
        HStack {
            Text(emoji)
            Text(localizedFamilyMemberName(name))
                .foregroundColor(.textPrimary)
            Spacer()
            Text(time)
                .foregroundColor(.textPrimary)
    ...
    private func recentActivityRow(emoji: String, member: String, activity: String, time: String) -> some View {
        HStack {
            Text(emoji)
            Text("\(localizedFamilyMemberName(member)) → \(localizedFamilyActivity(activity))")
                .foregroundColor(.textPrimary)
            Spacer()
            Text(time)
                .foregroundColor(.textSecondary)
    ...
    private func usageTimeRow(emoji: String, period: String, time: String, percent: Int) -> some View {
        HStack {
            Text(emoji)
            Text(localizedUsagePeriod(period))
                .foregroundColor(.textPrimary)
            Spacer()
            Text(time)
                .foregroundColor(.textPrimary)
...
struct AnalyticsFiltersSheet: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                Section(header: Text(localizationManager.localized("analytics_filters"))) {
                    Toggle(localizationManager.localized("analytics_filter_only_blocked"), isOn: $showOnlyBlocked)
                    Toggle(localizationManager.localized("analytics_filter_include_family"), isOn: $includeFamily)
                    Toggle(localizationManager.localized("analytics_filter_include_devices"), isOn: $includeDevices)
                }
            }
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button(localizationManager.localized("analytics_filter_cancel")) { isPresented = false }
                }
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(localizationManager.localized("analytics_filter_apply")) {
        ...
        .id("analytics_filters_lang_\(localizationManager.currentLanguage.rawValue)")
    }
}
