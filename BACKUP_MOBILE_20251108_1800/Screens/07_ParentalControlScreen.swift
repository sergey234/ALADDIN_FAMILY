...
struct ParentalControlScreen: View {
    ...
    var body: some View {
        ...
        .onAppear {
            // Инициализация selectedChild если пусто
            if selectedChild.isEmpty {
                selectedChild = "Маша" // Default значение, в реальности должно быть из данных
            }
            
            // Инициализация locationStatus если пусто
            ...
            let currentRole = UserDefaults.standard.string(forKey: "current_user_role") ?? localizationManager.localized("parental_role_not_set")
    ...
    private var navigationHeader: some View {
        ALADDINNavigationBar(
            ...
            subtitle: "\(localizationManager.localized("parental_control_subtitle")) \(selectedChild)",
            showBackButton: true,
        ...
        VStack(spacing: Spacing.s) {
            ...
            HStack(spacing: Spacing.s) {
                ForEach(["Маша", "Петя", "Аня"], id: \.self) { child in
                    // TODO: Имена детей должны быть из данных пользователя
                    Button(action: {
    ...
    private var parentalControlCards: some View {
        VStack(alignment: .leading, spacing: Spacing.m) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: Spacing.s), count: 2), spacing: Spacing.s) {
                ...
                // 7. Защита от обхода (НОВАЯ) - ✅ Синий цвет как у "Управление временем"
                ParentalControlCard(
                    icon: "🚨",
                    ...
                    cardColor: Color.blue.opacity(0.2),  // ✅ Синий цвет как у "Управление временем"
