...
struct FamilyContentBlockModal: View {
    ...
    var body: some View {
        FamilyModalBaseView(
            title: localizationManager.localized("family_content_block_modal_title"),
        ...
        .onChange(of: isWebsiteBlockingEnabled) { newValue in
            ...
            print("✅ Website blocking: \(newValue ? "ON" : "OFF")")
        ...
        .onChange(of: isAppBlockingEnabled) { newValue in
            print("✅ App blocking: \(newValue ? "ON" : "OFF")")
            manager.applyContentBlocking(
                childId: selectedChild,
                websiteBlocking: isWebsiteBlockingEnabled,
                appBlocking: isAppBlockingEnabled,
        ...
        .onChange(of: isSearchBlockingEnabled) { newValue in
            print("✅ Search blocking: \(newValue ? "ON" : "OFF")")
            manager.applyContentBlocking(
                childId: selectedChild,
                websiteBlocking: isWebsiteBlockingEnabled,
                appBlocking: isAppBlockingEnabled,
        ...
        .onChange(of: isSafeSearchEnabled) { newValue in
            print("✅ SafeSearch: \(newValue ? "ON" : "OFF")")
            manager.applyContentBlocking(
                childId: selectedChild,
                websiteBlocking: isWebsiteBlockingEnabled,
                appBlocking: isAppBlockingEnabled,
        ...
        FamilyModalBaseView(
            ...
            VStack(spacing: Spacing.m) {
                ...
                FamilyBadgeItem(
                    ...
                    description: "\(browserSitesCount) \(localizationManager.localized("family_sites")) per week",
                ...
                FamilyBadgeItem(
                    ...
                    description: "\(appsUsedCount) \(localizationManager.localized("family_apps")) used",
                    badgeText: localizationManager.localized("family_active_badge"),
                    badgeColor: .successGreen,
...
struct FamilyLocationModal: View {
    ...
    // Mock-данные (загружаются из UserDefaults)
    @State private var locationStatus: String = "🏠 Home (Lenina St. 42)"
    @State private var locationLastUpdate: String = "2 min ago"
        ...
        FamilyModalBaseView(
            ...
            VStack(spacing: Spacing.m) {
                ...
                FamilyConfigButtonItem(
                    ...
                    description: "\(geofencesCount) active: \(geofencesList)",
                    buttonTitle: localizationManager.localized("family_configure"),
    ...
    private func loadLocationStatistics() {
        ...
        if let stats = UserDefaults.standard.dictionary(forKey: statsKey) {
            ...
            locationStatus = stats["locationStatus"] as? String ?? "🏠 \(home) (\(street))"
            locationLastUpdate = stats["locationLastUpdate"] as? String ?? "2 min ago"
            geofencesCount = stats["geofencesCount"] as? Int ?? 2
            geofencesList = stats["geofencesList"] as? String ?? "\(home), \(localizationManager.localized("geofences_school"))"
        }
...
struct FamilyReportsModal: View {
        ...
        FamilyModalBaseView(
            ...
            VStack(spacing: Spacing.m) {
                ...
                FamilyBadgeItem(
                    ...
                    description: "\(suspiciousActivityCount) \(localizationManager.localized("family_new")) warnings",
    ...
    private func loadReportsStatistics() {
        ...
        if warnings.isEmpty {
            warnings = [
                ReportWarning(text: "Attempted access to blocked site", color: .dangerRed),
                ReportWarning(text: "Screen time exceeded by 15 minutes", color: .warningOrange)
            ]
        }
    }
}
...
struct FamilyAdditionalModal: View {
    ...
    @State private var deviceStatus: String = "Online"
    ...
    private func loadAdditionalStatistics() {
        if let stats = UserDefaults.standard.dictionary(forKey: statsKey) {
            ...
            deviceStatus = stats["deviceStatus"] as? String ?? "Online"
        ...
        if requests.isEmpty {
            requests = [
                AccessRequest(app: "Instagram", time: "10 min ago", reason: "Want to check messages", limit: "30 min/day (used)"),
                AccessRequest(app: "YouTube", time: "5 min ago", reason: "Need to review lesson", limit: "45 min/day (12 min remaining)")
            ]
        }
    }
}
...
struct BrowserHistoryDetailModal: View {
    ...
    @State private var browserHistory: [BrowserHistoryItem] = [
        BrowserHistoryItem(site: "youtube.com", visits: 142, time: "8h 24m", category: "Video", color: .red),
        BrowserHistoryItem(site: "instagram.com", visits: 89, time: "4h 12 min", category: "Social networks", color: .purple),
        BrowserHistoryItem(site: "vk.com", visits: 67, time: "2h 45m", category: "Social networks", color: .blue),
        BrowserHistoryItem(site: "google.com", visits: 45, time: "1h 15m", category: "Search", color: .blue),
        BrowserHistoryItem(site: "tiktok.com", visits: 34, time: "3h 20 min", category: "Video", color: .black)
    ]
                ...
                HStack(spacing: Spacing.m) {
                    ...
                    VStack(alignment: .trailing) {
                        Text(localizationManager.localized("browser_history_total_time"))
                            .font(.caption)
                            .foregroundColor(.textSecondary)
                        Text("20\(localizationManager.localized("analytics_hour")) 16\(localizationManager.localized("analytics_min"))")
                            .font(.h3)
                            .foregroundColor(.secondaryGold)
                    }
                }
...
struct AppHistoryDetailModal: View {
    ...
    @State private var appHistory: [AppHistoryItem] = [
        AppHistoryItem(app: "Instagram", time: "8h 24m", limit: "30 min/day", exceeded: true, exceededBy: "7h 54m", color: .purple),
        AppHistoryItem(app: "TikTok", time: "4h 12 min", limit: "20 min/day", exceeded: true, exceededBy: "3h 52 min", color: .black),
        AppHistoryItem(app: "YouTube", time: "3h 45 min", limit: "45 min/day", exceeded: true, exceededBy: "3h 0 min", color: .red),
        AppHistoryItem(app: "WhatsApp", time: "1h 30 min", limit: "60 min/day", exceeded: false, exceededBy: nil, color: .green),
        AppHistoryItem(app: "VK", time: "45 min", limit: "30 min/day", exceeded: true, exceededBy: "15 min", color: .blue)
    ]
                    ...
                    HStack(spacing: Spacing.m) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            HStack(spacing: Spacing.s) {
                                ...
                                Text("\(localizationManager.localized("app_history_limit")) \(item.limit)")
                                    .font(.caption)
                                    .foregroundColor(.textSecondary)
                            }
                            ...
                            if item.exceeded, let exceededBy = item.exceededBy {
                                HStack {
                                    ...
                                    Text("\(localizationManager.localized("app_history_exceeded_by")) \(exceededBy)")
                                        .font(.captionSmall)
                                        .foregroundColor(.dangerRed)
                                        .fontWeight(.semibold)
                                }
           ...
           !decoded.isEmpty {
            ...
            contacts = decoded.prefix(5).map { contact in
                ContactItem(
                    ...
                    lastContact: "Recently", // TODO: Загрузить из API мониторинга
                    color: .successGreen
                )
            }
            return
...
struct SuspiciousActivityDetailModal: View {
    ...
    @State private var warnings: [SuspiciousWarning] = [
        SuspiciousWarning(text: "Attempted access to blocked site", level: .high, time: "2 hours ago"),
        SuspiciousWarning(text: "Screen time exceeded by 15 minutes", level: .medium, time: "5 hours ago"),
        SuspiciousWarning(text: "Unknown contact in messages", level: .high, time: "1 day ago"),
        SuspiciousWarning(text: "Attempted installation of restricted app", level: .high, time: "2 days ago")
    ]
    
    var body: some View {
        FamilyModalBaseView(
            title: localizationManager.localized("suspicious_activity_title"),
            isPresented: $isPresented
        ) {
            VStack(spacing: Spacing.m) {
                ForEach(warnings) { warning in
...
struct TopSitesDetailModal: View {
    ...
    @State private var topSites: [TopSiteItem] = [
        TopSiteItem(site: "youtube.com", visits: 142, time: "8h 24m", category: "Video", color: .red),
        TopSiteItem(site: "instagram.com", visits: 89, time: "4h 12 min", category: "Social networks", color: .purple),
        TopSiteItem(site: "vk.com", visits: 67, time: "2h 45m", category: "Social networks", color: .blue),
        TopSiteItem(site: "google.com", visits: 45, time: "1h 15m", category: "Search", color: .blue),
        TopSiteItem(site: "tiktok.com", visits: 34, time: "3h 20 min", category: "Video", color: .black)
    ]
...
struct TopAppsDetailModal: View {
    ...
    @State private var topApps: [TopAppItem] = [
        TopAppItem(app: "Instagram", time: "8h 24m", limit: "30 min/day", exceeded: true, color: .purple),
        TopAppItem(app: "TikTok", time: "4h 12 min", limit: "20 min/day", exceeded: true, color: .black),
        TopAppItem(app: "YouTube", time: "3h 45 min", limit: "45 min/day", exceeded: true, color: .red),
        TopAppItem(app: "WhatsApp", time: "1h 30 min", limit: "60 min/day", exceeded: false, color: .green),
        TopAppItem(app: "VK", time: "45 min", limit: "30 min/day", exceeded: true, color: .blue)
    ]
                    ...
                    HStack(spacing: Spacing.m) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            HStack(spacing: Spacing.s) {
                                ...
                                Text("\(localizationManager.localized("top_apps_limit")) \(app.limit)")
                                    .font(.caption)
                                    .foregroundColor(.textSecondary)
                            }
            ...
            VStack(spacing: Spacing.l) {
                ...
                VStack(alignment: .leading, spacing: Spacing.m) {
                    ...
                    if isAgeRestrictionEnabled {
                        VStack(alignment: .leading, spacing: Spacing.s) {
                            Text(localizationManager.localized("youtube_age_restriction_value"))
                                .font(.body)
                                .foregroundColor(.textSecondary)
